...
class CetStressPauseResumeCetTest(ContentBaseTestCase):
    """
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        super(CetStressPauseResumeCetTest, self).__init__(test_log, arguments, cfg_opts)
    ...
    def save_and_restore_thread_fun(self):
        ...
        resume_vm_cmd_1 = self._cet_common_obj.RESUME_VM_CMD.format(self.windows_vm_2.get('vm_name'))
        self._cet_common_obj._os.execute(resume_vm_cmd_1, self._command_timeout)
        self._log.info("Resuming Second VM")
        resume_vm_cmd_2 = self._cet_common_obj.RESUME_VM_CMD.format(self.windows_vm_2.get('vm_name'))
        self._cet_common_obj._os.execute(resume_vm_cmd_2, self._command_timeout)
        time.sleep(self.WAIT_TIME)

    def execute(self):
        """
        This program will execute CET- stress test by pausing and resuming Hyper-V vms \
        while running CETtest in an infinite loop for 2 hours

        """
        thread_1 = threading.Thread(target=self.root_thread, args=(), daemon=True)
        thread_2 = threading.Thread(target=self.vm_thread, args=(self._cet_common_obj.WINDOWS_GEN2_LEVEL1_VM,),
                                    daemon=True)
        thread_3 = threading.Thread(target=self.vm_thread, args=(self._cet_common_obj.WINDOWS_GEN1_LEVEL1_VM,),
                                    daemon=True)
        ...
        thread_3.start()
        thread_4.start()
        # wait for 2hours while running STRESS test
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if CetStressPauseResumeCetTest.main() else Framework.TEST_RESULT_FAIL)
