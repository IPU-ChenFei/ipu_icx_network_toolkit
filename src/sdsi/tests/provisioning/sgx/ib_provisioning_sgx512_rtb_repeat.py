...
class IbProvisioningSgx512RTBRepeat(InBandSdsiSgxCommon):
    """
    Glasgow_ID: NA
    Phoenix_ID: NA - OOB: 18014074924
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create an instance of IbProvisioningSgx512RTB
        :param cfg_opts: Configuration Object of provider
        :param test_log: Log object
        :param arguments: None
        """
        super(IbProvisioningSgx512RTBRepeat, self).__init__(test_log, arguments, cfg_opts)

    def prepare(self):
        # type: () -> None
        """preparing the setup"""
        super(IbProvisioningSgx512RTBRepeat, self).prepare()
        ...
        memory_required = self.prm_size_to_apply[FeatureNames.SG40] * 1.5
    ...
    def execute(self):
        ...
        # Repeat Test 2 times for extra validation
        for _test_repeat in range(2):
            # Provision CPU with SG40 and validate PrmSgxSize increase
            self._log.info("Perform SGX operation for SG40 and validate.")
            self.perform_and_validate_sgx_operation(FeatureNames.SG40)
            if not self.set_and_validate_prm_size(self.prm_size_to_apply[FeatureNames.SG40]):
            ...
            for cpu in range(self._sdsi_obj.number_of_cpu):
                self._sdsi_obj.return_to_base(FeatureNames.BASE, cpu)
            self.perform_graceful_g3()
    ...
    def cleanup(self, return_status):
        # type: (bool) -> None
        """DTAF cleanup"""
        super(IbProvisioningSgx512RTBRepeat, self).cleanup(return_status)

if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if IbProvisioningSgx512RTBRepeat.main()
