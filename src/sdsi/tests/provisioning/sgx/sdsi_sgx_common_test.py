...
class InBandSdsiSgxCommon(ContentBaseTestCase):
    """
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        """
        super(InBandSdsiSgxCommon, self).__init__(test_log, arguments, cfg_opts, self.SGX_BIOS_DEFAULT)
        ...
        self.prm_size_to_apply = {"SG00": self.get_prm_default_max(),
                                  "SG01": 8,
                                  "SG04": 32,
                                  "SG08": 64,
                                  "SG10": 128,
    ...
    def prepare(self):
        ...
        """
        super(InBandSdsiSgxCommon, self).prepare()
        self._log.info("Clear any existing payloads from the CPU")
        self._sdsi_obj.erase_payloads_from_nvram()
        self.initial_memory_available = self._sdsi_obj.get_total_memory_in_gb()
        self._log.info("Initial memory available on platform is {}".format(self.initial_memory_available))
    ...
    def cleanup(self, return_status):
        ...
        """
        super(InBandSdsiSgxCommon, self).cleanup(return_status)
    ...
    def perform_and_validate_sgx_operation(self, feature_name):
        ...
        for cpu_counter in range(self._sdsi_obj.number_of_cpu):
            ...
            if payload_fail_count_before != payload_fail_count_after:
                error_msg = f"Error counter increased for CPU {cpu_counter} during provisioning"
                self._log.error(error_msg)
                raise SDSiExceptions.ProvisioningError(error_msg)
            if max_ssku_updates_after != max_ssku_updates_before - 1:
                error_msg = f"Max available SSKU updates did not decrease for CPU {cpu_counter} after provisioning"
                self._log.error(error_msg)
                raise SDSiExceptions.ProvisioningError(error_msg)
    ...
    def set_and_validate_prm_size(self, size_in_gb) -> bool:
        ...
        self._log.info("Set the PrmSgxSize to {}GB".format(size_in_gb / self._sdsi_obj.number_of_cpu))
        ...
        prm_set_success = (size_in_gb / self._sdsi_obj.number_of_cpu == current_prm_size_in_gb)
        ...
        prm_set_success = (prm_set_success and isclose(memory_difference, size_in_gb, rel_tol=memory_deviation))
        return prm_set_success

    def get_prm_default_max(self):
        return 256


class OutOfBandSdsiSgxCommon(InBandSdsiSgxCommon):
    def _get_common_lib(self):
