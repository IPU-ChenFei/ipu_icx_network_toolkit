...
class SDSiFunctionalCyclingAC(OutOfBandSdsiSgxCommon):
    """
    Glasgow_ID: NA
    Phoenix_ID: NA - IB: 22012935895
    ...
    def execute(self):
        ...
        self._log.info('THIS TEST CURRENTLY REQUIRES MANUAL PROVISION BEFORE CYCLING TEST.')

        for cycle in range(self.AC_CYCLES):
            self._log.info(f"Functional testing, reboot {cycle + 1}.")

            if not self.set_and_validate_prm_size(self.prm_size_to_apply["SG20"]):
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if SDSiFunctionalCyclingAC.main() else Framework.TEST_RESULT_FAIL)
