...
class InBandTwoIAAFuncProvisionAndReturnToBase(ContentBaseTestCase):
    """
    Glasgow_ID: 74642
    Phoenix_ID: 22014826713
    Expectation is that IAA devices should works only after applying the IAA2 licenses (except default devices)
    """
    IAA_PAYLOAD_NAME = 'IAA2'
    RTB_PAYLOAD_NAME = 'BASE'
    ROOT_FOLDER = '/'
    IAA_DEVICES = {0: ['79:02.0', '6f:02.0', '74:02.0'],
                   1: ['f6:02.0', 'ec:02.0', 'f1:02.0']}
    ...
    def execute(self):
        """
            ...
            - Verify any IAA devices enumerated without IAA2 payload
            - Apply IAA2 payload
        ...
        self._log.info("#5 Verify non expected IAA devices are enumerated without IAA2 capability payload")
        ...
        self._log.info("#6 - Apply IAA2 payload on CPUs and start cold reboot")
        for socket in range(self._sdsi_obj.number_of_cpu):
        ...
        for socket in range(self._sdsi_obj.number_of_cpu):
            ...
            for default_device in self.verify_default_device_list_status(self.DLB_DEFAULT_DEVICES[socket]):
                self.default_devices_found.add(default_device)
            # Only first device in list will be enumerated after provisioning
            self.verify_device_list_status([self.IAA_DEVICES[socket][0]], True)
            self.verify_device_list_status(self.IAA_DEVICES[socket][1:], False)
            self.verify_device_list_status(self.DSA_DEVICES[socket], False)
            self.verify_device_list_status(self.DLB_DEVICES[socket], False)
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if InBandTwoIAAFuncProvisionAndReturnToBase.main()
