...
class SystemHang(ContentBaseTestCase):
    """
    Glasgow_ID: 69434
    Phoenix_ID: 18014074887
    This test case is to verify the OOB provisioning of the SSKU enabled CPU by applying the license key certificate,
    and a capability activation payload and verify it is available after the system hang.
    """
    ENABLE_SYSRQ = 'sysctl -w kernel.sysrq=1'
    CHECK_IF_SYSRQ_ENABLED = 'cat /proc/sys/kernel/sysrq'
    EXECUTE_HANG = 'echo c > /proc/sysrq-trigger'

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create an instance of SystemHang

        :param cfg_opts: Configuration Object of provider
        :param test_log: Log object
        :param arguments: None
        """
        super(SystemHang, self).__init__(test_log, arguments, cfg_opts)
    ...
    def execute(self):
        """
            ...
            6. Execute OS hang command
            7. Verify the failure counter and max ssku updates counter values.
        """
        self.bios_util.set_single_bios_knob("PpinControl", "0x1")

        payload_info = self.provision_cpu()
        if not self.execute_hang_command_and_verify_that_payloads_applied(payload_info):
            raise RuntimeError("Your system was not properly crashed")
        return True

    def execute_hang_command_and_verify_that_payloads_applied(self, payload_info):
        execution_status = False
        self._log.info("Checking if sysrq is enabled")
        returned_value = self.os.execute(self.CHECK_IF_SYSRQ_ENABLED, self._command_timeout)
        if not int(returned_value.stdout) == 1:
            self._log.info("Sysrq was enabled for this boot")
            self.os.execute(self.ENABLE_SYSRQ, self._command_timeout)

        self._log.info("Executing OS command to crash the system")
        try:
            self.os.execute(self.EXECUTE_HANG, self._command_timeout)
        except OsCommandTimeoutException:
            self._log.info("Rebooting the system after it hang up")
    ...
    def provision_cpu(self):
        for cpu_counter in range(self._sdsi_obj.number_of_cpu):
            ...
            time.sleep(self._read_execution_delay)
    ...
    def verify_provisioning(self, payload_info):
        for cpu_counter in range(self._sdsi_obj.number_of_cpu):
            ...
            assert self._sdsi_obj.is_payload_available(
                ...
                .format(cpu_counter, payload_info[1])
    ...
    def cleanup(self, return_status):
        ...
        self.perform_graceful_g3()
        super(SystemHang, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if SystemHang.main()
