...
from src.sdsi.tests.provisioning.in_band_provisioning_test import InBandProvisioningTest


class InBandProvisioningTestUnderStress(InBandProvisioningTest):
    """
    Glasgow_ID: 75095
    Phoenix_ID: 22013674015
    This test case is to verify the OOB provisioning of the SSKU enabled CPU by applying
    the license key certificate, and a capability activation payload and
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create an instance of InBandProvisioningTestUnderStress
        ...
        super(InBandProvisioningTestUnderStress, self).__init__(self._test_log, arguments, cfg_opts)
        self._install_collateral = InstallCollateral(self._test_log, self.os, cfg_opts)

    def prepare(self):
        # type: () -> None
        """preparing the setup"""
        super(InBandProvisioningTestUnderStress, self).prepare()
    ...
    def execute(self):
        ...
        super(InBandProvisioningTestUnderStress, self).execute()
    ...
    def cleanup(self, return_status):
        # type: (bool) -> None
        """DTAF cleanup"""
        super(InBandProvisioningTestUnderStress, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if InBandProvisioningTestUnderStress.main()
