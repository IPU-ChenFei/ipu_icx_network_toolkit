...
class InBandProvisioningUsingPrevioslyUsedFeatureActivationPayload(ContentBaseTestCase):
    """
    Glasgow_ID: 75093
    Phoenix_ID: 22013642500
    This test is to verify platform behavior when a used Feature Activation Payload is applied to
    a previously provisioned system.
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        """
        super(InBandProvisioningUsingPrevioslyUsedFeatureActivationPayload, self).__init__(test_log, arguments,
                                                                                              cfg_opts)
    ...
    def prepare(self):
        # type: () -> None
        """preparing the setup"""
        super(InBandProvisioningUsingPrevioslyUsedFeatureActivationPayload, self).prepare()
    ...
    def execute(self):
        ...
        for cpu_counter in range(self._sdsi_obj.number_of_cpu):
            ...
            self._log.info(
                "Apply the same CAP config that previosly applied({}) for CPU{}.".format(payload_information[1], cpu_counter + 1))
    ...
    def cleanup(self, return_status):
        ...
        self.perform_graceful_g3()
        super(InBandProvisioningUsingPrevioslyUsedFeatureActivationPayload, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if InBandProvisioningUsingPrevioslyUsedFeatureActivationPayload.main()
