...
class InBandProvisioningTestGlobalReset(ContentBaseTestCase):
    """
    Glasgow_ID: 75421
    Phoenix_ID: 22013725176
    This test case is to verify the In Band provisioning of the SSKU enabled CPU by applying the license key certificate,
    and a capability activation payload and verify it is available with Global reset enabled.
    """

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create an instance of InBandProvisioningTestGlobalReset

        :param cfg_opts: Configuration Object of provider
        :param test_log: Log object
        :param arguments: None
        """
        super(InBandProvisioningTestGlobalReset, self).__init__(test_log, arguments, cfg_opts)
    ...
    def prepare(self):
        # type: () -> None
        """preparing the setup"""
        super(InBandProvisioningTestGlobalReset, self).prepare()
    ...
    def cleanup(self, return_status):
        ...
        if int(current_global_reset, 16) != int(global_reset_value, 16):
            ...
            self._log.error(log_error)
            raise RuntimeError(log_error)
        super(InBandProvisioningTestGlobalReset, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if InBandProvisioningTestGlobalReset.main()
