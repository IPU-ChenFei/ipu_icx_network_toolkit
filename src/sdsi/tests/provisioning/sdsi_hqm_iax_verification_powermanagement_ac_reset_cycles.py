...
class SdsiHqmIaxVerificationPowerManagementAcResetCycles(ContentBaseTestCase):
    """
    Glasgow_ID: 73960
    Phoenix_ID: 22012935895
    This TestCase is Used to Verify MCE Logged Status, HQM by AC cycling and verifying the status every cycle.
    """
    _BIOS_CONFIG_FILE = "cycling_bios_knobs.cfg"

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new SdsiHqmIaxVerificationPowerManagementAcResetCycles object

        :param test_log: Used for debug and info messages
        :param arguments:
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(SdsiHqmIaxVerificationPowerManagementAcResetCycles, self).__init__(test_log, arguments, cfg_opts, self._BIOS_CONFIG_FILE)
    ...
    def prepare(self):
        ...
        super(SdsiHqmIaxVerificationPowerManagementAcResetCycles, self).prepare()
    ...
    def execute(self):
        """
        This test is Used to verify status of MCE, HQM, and IAX in Every AC Reset of 50 Cycles'
        ...
        self._log.info("Perform {} AC Reset cycles and verify the HQM4 data and MCE error in each cycles.".format(self.reboot_cycles))
        for cycle_number in range(self.reboot_cycles):
            if cycle_number % 2:
                self._sdsi_obj.set_peci_mode_mctp()
            else:
                self._sdsi_obj.set_peci_mode_wire()
            self._log.info("AC Reset cycle #{}.".format(cycle_number + 1))
            self.perform_graceful_g3()
            if self._common_content_lib.check_if_linux_mce_errors():
                log_error = "Machine Check errors are Logged in AC Reset Cycle '{}'".format(cycle_number + 1)
                self._log.error(log_error)
                raise RuntimeError(log_error)
            self._log.info("No Machine Check Errors are Logged in AC Reset Cycle '{}'.".format(cycle_number + 1))
            self._log.info("Verify HQM4 data is available on the SUT in AC Reset Cycle '{}'.".format(cycle_number + 1))
            assert self._sdsi_obj.verify_hqm_dlb_kernel("HQM4"), "HQM4 device information is not found in AC Reset Cycle '{}'.".format(cycle_number + 1)
        self._log.info("Machine Check Events Status, HQM, IAX status is Verified in all {} AC Reset Cycles.".format(self.reboot_cycles))
    ...
    def cleanup(self, return_status):
        ...
        self.perform_graceful_g3()
        super(SdsiHqmIaxVerificationPowerManagementAcResetCycles, self).cleanup(return_status)

if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if SdsiHqmIaxVerificationPowerManagementAcResetCycles.main() else Framework.TEST_RESULT_FAIL)
