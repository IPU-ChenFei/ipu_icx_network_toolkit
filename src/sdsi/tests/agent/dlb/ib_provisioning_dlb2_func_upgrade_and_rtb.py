...
class IbDlb2FuncProvisionAndRTB(SDSiBaseTestCase):
    """
    Verify that DLB devices work only after applying the DLB2 licenses (except default devices)
    """
    ...
    def execute(self) -> None:
        ...
        # Install DLB2 capability.
        active_features = []
        for socket in range(self.sdsi_agent.num_sockets):
            active_features.append(self.sdsi_agent.apply_lac_by_name(socket, [FeatureNames.DLB2.value]))
        self.cycling_tool.perform_ac_cycle()
        for socket in range(self.sdsi_agent.num_sockets):
            self.sdsi_agent.validate_active_feature_set(socket, active_features[socket])

        # Verify DLB2 devices are numerated.
        self._log.info("Verify DLB2 devices are enumerated after applying capability payload")
        self.sdsi_feature_lib.verify_dlb_device_count(self.sdsi_agent.num_sockets * 2)
...
if __name__ == "__main__":
    sys.exit(not IbDlb2FuncProvisionAndRTB.run())
