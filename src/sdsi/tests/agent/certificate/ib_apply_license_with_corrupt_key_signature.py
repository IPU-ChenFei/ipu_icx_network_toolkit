...
class IbApplyLicenseWithCorruptKeySignature(SDSiBaseTestCase):
    """
    ...
    def execute(self) -> None:
        ...
        for socket in range(self.sdsi_agent.num_sockets):
            try:
                self.sdsi_agent.apply_corrupt_certificate(socket)
                error_msg = 'Applying corrupt AKC succeeded.'
                self._log.error(error_msg)
                raise SDSiExceptions.LicenseKeyError(error_msg)
            except SDSiExceptions.ProvisioningError:
                self._log.info('Applying corrupt AKC failed as expected.')

if __name__ == "__main__":
    sys.exit(not IbApplyLicenseWithCorruptKeySignature.run())
