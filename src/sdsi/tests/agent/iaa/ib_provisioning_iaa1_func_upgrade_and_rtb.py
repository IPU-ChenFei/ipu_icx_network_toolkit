...
class IbIAA1FuncProvisionAndRTB(SDSiBaseTestCase):
    """
    Verify that IAA devices work only after applying the IAA1 licenses (except default devices)
    """
    ...
    def execute(self) -> None:
        ...
        self.sdsi_feature_lib.verify_iaa_device_count(0)

        # Install IAA1 capability.
        active_features = []
        for socket in range(self.sdsi_agent.num_sockets):
            active_features.append(self.sdsi_agent.apply_lac_by_name(socket, [FeatureNames.IAA1.value]))
        self.cycling_tool.perform_ac_cycle()
        for socket in range(self.sdsi_agent.num_sockets):
            self.sdsi_agent.validate_active_feature_set(socket, active_features[socket])

        # Verify IAA1 devices are numerated.
        self._log.info("Verify IAA1 devices are enumerated after applying capability payload")
...
if __name__ == "__main__":
    sys.exit(not IbIAA1FuncProvisionAndRTB.run())
