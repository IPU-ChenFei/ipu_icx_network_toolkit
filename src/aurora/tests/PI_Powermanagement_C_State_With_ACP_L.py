...
CASE_DESC = [
    'https://hsdes.intel.com/appstore/article/#/18016910048'
]
...
def test_steps(sut, my_os):
    ...
    Case.step('Check ACP enabled')
    itp.execute('unlock()')
    itp.execute('sls()')
    out = itp.execute('print(sv.sockets.uncore.core_pmsb.core_pmsbs.pma_debug2.acp_enabled)')
    Case.expect('ACP enabled', '0x00000001' in out)
    ...
    Case.step('Check C6 residency in idle mode again')
    Case.sleep(10 * 60, 60)
    ret, _, _ = sut.execute_shell_cmd('./ptu -mon -filter 0x0F -l 2 -t 60 -log -logdir . -logname idle2', 90,
                                      cwd=PTU_PATH)
    Case.expect('run ptu tool successfully', ret == 0)
    sut.download_to_local(remotepath=f'{PTU_PATH}/idle2_ptumon.txt')
    sut.download_to_local(remotepath=f'{PTU_PATH}/idle2_ptumsg.txt')
    ptu_mon_log_check(os.path.join(LOG_PATH, 'idle_ptumon.txt'), idle_check_callback)
