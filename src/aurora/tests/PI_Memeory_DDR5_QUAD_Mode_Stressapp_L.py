...
CASE_DESC = [
    'https://hsdes.intel.com/appstore/article/#/16014658638'
]
...
def test_steps(sut, my_os):
    ...
    set_bios_knobs_step(sut, *bios_knob('disable_snc_xmlci'), *bios_knob('enable_UMA_Based_clustering_quad_xmlci'))
    itp = PythonsvSemiStructured(sut.socket_name, globals(), locals())

    Case.step('Verify Quad mode is enabled')
    itp.execute('unlock()')
    ...
    for snc_config in out:
        ...
        Case.expect('Result should be 0xe for both S0 and S1', snc_config == 0xe)
