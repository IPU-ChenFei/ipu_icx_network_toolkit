...
class PiSpareModeCheckW(HsioUpiCommon):
    """
    Phoenix 1509350487, PI_spare_mode_check_W
    PCH IO Devices S5 cycling test
    """

    TEST_CASE_ID = ["1509350487", "PI_spare_mode_check_W"]
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create an instance PiSpareModeCheckW
        ...
        super(PiSpareModeCheckW, self).__init__(test_log, arguments, cfg_opts)
    ...
    def prepare(self):
        ...
        no_of_sockets = int(no_of_sockets_from_pysv)
        ...
        if mode != "Spare Mode":
            raise content_exceptions.TestSetupError("System is not in Spare mode please perform the require jumper "
                                                    "settings")
        super(PiSpareModeCheckW, self).prepare()
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if PiSpareModeCheckW.main() else Framework.TEST_RESULT_FAIL)
