...
class PiAcCyclingSanitySpareMode(HsioUpiCommon):
    """
    Phoenix 18014075145, PI_ACCycling_Sanity_SpareMode
    PCH IO Devices S5 cycling test
    """

    TEST_CASE_ID = ["18014075145", "PI_ACCycling_Sanity_SpareMode"]
    step_data_dict = {1: {'step_details': 'System should be in Spare Mode.',
                      ...
                      4: {'step_details': 'Perform AC cycle ',
                          'expected_results': 'AC Cycle should happen properly...'},
                      5: {'step_details': 'check for any errors or hang conditions',
                          'expected_results': 'There should not be any hang/error...'}
                      }

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create an instance PiAcCyclingSanitySpareMode
        ...
        super(PiAcCyclingSanitySpareMode, self).__init__(test_log, arguments, cfg_opts)
    ...
    def prepare(self):
        ...
        if mode != "Spare Mode":
            raise content_exceptions.TestSetupError("System is not in Spare mode please perform the require jumper "
                                                    "settings")
        super(PiAcCyclingSanitySpareMode, self).prepare()
    ...
    def execute(self):
        ...
        for cycle_number in range(self._common_content_configuration.get_num_of_ac_cycles()):
            ...
            if not self.verify_rx_ports_l0_state():
                raise content_exceptions.TestFail(" verification of rx port lo state failed")
            if not self.verify_tx_ports_l0_state():
                raise content_exceptions.TestFail(" verification of tx port lo state failed")
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if PiAcCyclingSanitySpareMode.main() else Framework.TEST_RESULT_FAIL)
