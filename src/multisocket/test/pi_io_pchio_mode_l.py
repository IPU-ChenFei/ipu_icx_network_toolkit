...
class PCHIoModeL(StorageCommon):
    """
    Phoenix 18014075337, PI_IO_PCHIO_mode_L

    This test cases to Check for IO devices in PCH-IO mode for Linux..
    """
    TEST_CASE_ID = ["18014075337", "PI_IO_PCHIO_mode_L"]
    step_data_dict = {1: {'step_details': 'boot to OS',
                      ...
                      3: {'step_details': 'verify the basic checks for IO',
                          'expected_results': 'successfully verified the basic checks for IO ...'}
                     }

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create an instance PCHIoModeL
        ...
        super(PCHIoModeL, self).__init__(test_log, arguments, cfg_opts)
    ...
    def execute(self):
        ...
        self.check_storage_device_work_fine_after_boot(boot_option="", storage_devices=['sata', 'usb'])
        sut_ip = self._common_content_lib.get_sut_ip()
        self._log.info("SUT IP is  {}".format(sut_ip))
        output = self._common_content_lib.execute_sut_cmd("hostname -I", "Getting IP", 10.0)
        ip_values = str(output).split()
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if PCHIoModeL.main() else Framework.TEST_RESULT_FAIL)
