...
class PiIoAcCycledeviceCheckL(ContentBaseTestCase):
    """
    Phoenix 18014074373, PI_IO_ACcycle_device_check_L
    PCH IO Devices Reset cycling test on Linux
    """

    TEST_CASE_ID = ["18014074373", "PI_IO_ACcycle_device_check_L"]
    step_data_dict = {1: {'step_details': 'To clear OS logs and load default bios settings ...',
                      ...
                      2: {'step_details': 'Check all IO devices and reboot no of ac cycles ',
                          'expected_results': 'Display all IO devices and rebooted successfully ...'}}

    LSSCSI = "lsscsi | grep -e ATA -e Generic"
    LSPCI = "lspci | grep Ethernet"
    LSUSB = "lsusb"

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create an instance PiIoAcCycledeviceCheckL
        ...
        super(PiIoAcCycledeviceCheckL, self).__init__(test_log, arguments, cfg_opts)
    ...
    def prepare(self):
        ...
        super(PiIoAcCycledeviceCheckL, self).prepare()
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if PiIoAcCycledeviceCheckL.main() else Framework.TEST_RESULT_FAIL)
