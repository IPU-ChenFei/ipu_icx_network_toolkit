...
import IFWI_BIOS_Knob_Retriever
...
def GenerateBIOSKnobDIFFReport(IFWIBIOSBinary1, IFWIBIOSBinary2, ConfigPath, XLSXFileName, XMLCLIBaseFolder):
	print("GenerateBIOSKnobReportOut: BIOS/IFWI BIOS Setup options report out started")
	jsondata = {}
	with open(ConfigPath+r'\BIOS_Knob_Diff_Report.json', "r", newline = '') as jsonfile:
	...
	if (os.path.isfile(IFWIBIOSBinary1)):
		...
		KnobList1 = IFWI_BIOS_Knob_Retriever.GenerateBIOSSetupOptionXMLFile(IFWIBIOSBinary1, XMLCLIBaseFolder, jsondata['Header'])
	print (len(KnobList1), KnobList1[0]['prompt'])
	if (os.path.isfile(IFWIBIOSBinary2)):
		print("GenerateBIOSKnobReportOut: IFWIBIOSBinary Continuing further")
		KnobList2 = IFWI_BIOS_Knob_Retriever.GenerateBIOSSetupOptionXMLFile(IFWIBIOSBinary2, XMLCLIBaseFolder, jsondata['Header'])
	...
	XLSXFile.close()
	return "BIOS Knob DIFF Report is generated and stored in the XLSX File"
