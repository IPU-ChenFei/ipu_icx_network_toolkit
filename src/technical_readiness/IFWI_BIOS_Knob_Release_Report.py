...
def GenerateXLSXFileBaseOnXML(xp, jsondata, XLSXFileName):
...
def GenerateBIOSKnobReportOut(IFWIBIOSBinary, ConfigPath, XLSXFileName, XMLCLIBaseFolder):
	print("GenerateBIOSKnobReportOut: BIOS/IFWI BIOS Setup options report out started")
	jsondata = {}
	KnobList =[]
	with open(ConfigPath+r'\BIOS_Knob_Release_Report.json', "r", newline = '') as jsonfile:
		jsonstring = jsonfile.read()
		jsondata = json.loads (jsonstring)
	if (os.path.isfile(IFWIBIOSBinary)):
		print("GenerateBIOSKnobReportOut: IFWIBIOSBinary Continuing further")
		KnobList = IFWI_BIOS_Knob_Retriever.GenerateBIOSSetupOptionXMLFile(IFWIBIOSBinary, XMLCLIBaseFolder, jsondata['Header'])
		if (len(KnobList) > 0):
			XLSXFileReportOut = GenerateXLSXFileBaseOnXML(KnobList, jsondata, XLSXFileName)
	return "BIOS Knob Report is generated and stored in the XLSX File"
