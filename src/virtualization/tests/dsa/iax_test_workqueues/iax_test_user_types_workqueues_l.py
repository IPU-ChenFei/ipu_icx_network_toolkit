...
class SprIaxTestUserTypeWorkQueues(DsaBaseTest):
    """
    Phoenix ID : 16014809930-PI_SPR_IAX_Test_User_Type_WorkQueues

    This Test case is to Test the IAX user Types work queues in the Linux SUT
    """
    TEST_CASE_ID = ["16014809930", "PI_SPR_IAX_Test_User_Type_WorkQueues"]
    BIOS_CONFIG_FILE = "../vtd_bios_config.cfg"
    IAX_USER_MODE_SCRIPT = "./setup_iax_user_mode.sh"
    EXEC_DMA_TEST_SCRIPT = "./Setup_Randomize_IAX_Conf.sh -r"
    ...
    STEP_DATA_DICT = {1: {'step_details': 'Enable Intel VT for Directed I/O (VT-d), Interrupt Remapping, PCIe ENQCMD '
                      ...
                      5: {'step_details': 'Execute basic user mode tests',
                          'expected_results': 'Executed basic user mode tests successfully'},
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create instance of SprIaxTestUserTypeWorkQueues
        ...
        super(SprIaxTestUserTypeWorkQueues, self).__init__(test_log, arguments, cfg_opts, self.dsa_bios_enable)
        ...
        if self.os.os_subtype.upper() != LinuxOsTypes.CENTOS:  # check if the Linux sub type is CentOS
            raise content_exceptions.TestFail("IAX Test User Type WorkQueues not supported in the OS Type : {}".format(
                self.os.os_subtype))
    ...
    def prepare(self):
        ...
        super(SprIaxTestUserTypeWorkQueues, self).prepare()
    ...
    def execute(self):
        ...
        4. Execute basic user mode tests
        ...
        result = self.execute_shell_script(self.IAX_USER_MODE_SCRIPT, self.TEST_CASE_CONTENT_PATH)
        self._log.debug("enabled result {}".format(result))
        ...
        # Execute basic user mode tests
        self._test_content_logger.start_step_logger(5)
        result = self.execute_shell_script(self.EXEC_DMA_TEST_SCRIPT, self.SPR_ACC_PATH)
        self._log.debug("Basic user mode result : {}".format(result))
        test_passed_output = re.findall(self.REGEX_TEST_PASSED, result)[0]
        test_passed_list = test_passed_output.split("of")
        total_tests = int(test_passed_list[0])
        total_passed = int(test_passed_list[1])

        self._log.debug("Total number of Tests : {} Total Tests passed : {}".format(total_tests, total_passed))
        if total_tests != total_passed:
            raise content_exceptions.TestFail("IAX Basic user Mode Test  failed. Mismatch between Total Tests {} vs "
                                              "Tests passed {}".format(total_tests, total_passed))
        ...
        self._log.debug("disable script result: {}".format(result))
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if SprIaxTestUserTypeWorkQueues.main() else Framework.TEST_RESULT_FAIL)
