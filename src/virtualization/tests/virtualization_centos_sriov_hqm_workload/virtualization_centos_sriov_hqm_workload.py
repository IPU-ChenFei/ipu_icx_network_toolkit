...
class VirtualizationSRIOVHQMVFVMWorkLd(VirtualizationCommon):
    """
    Phoenix ID : 16013929517
    ...
    VM = [VMs.CENTOS] * 1
    VM_TYPE = "CENTOS"
    TEST_CASE_ID = ["p16013929517", "Virtualization_SRIOV_Dlb_guestVM"]
    BIOS_CONFIG_FILE = "virtualization_sriov_dlb_bios_knobs.cfg"
    STORAGE_VOLUME = ["/home"]
    STEP_DATA_DICT = {
        1: {'step_details': 'Install and verify Bios knobs for HQM',
        ...
        4: {'step_details': 'Verify the HQM in kernel',
            'expected_results': 'HQM verified successfully'},
        5: {'step_details': 'Install HQM and dpdk, verify it installed or not',
            'expected_results': 'HQM is installed successfully and DPDK folders are present'},
        6: {'step_details': 'Create the VF function to be created',
            'expected_results': 'VFs are created'},
        ...
        11: {'step_details': 'Attach the created vf device to VM',
             'expected_results': 'Created vf device attached to VM'},
        12: {'step_details': 'Enabling iommu on VM',
        ...
        14: {'step_details': 'Install HQM and libdlb, verify it installed or not',
             'expected_results': 'HQM is installed successfully and DPDK folders are present'},
        15: {'step_details': 'Execute dlb work load in VM',
             'expected_results': 'dlb workload executed successfully in VM'}
    }
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        """
        self.virtualization_sriov_dlb_bios_knobs = os.path.join(os.path.dirname(os.path.abspath(__file__)), self.BIOS_CONFIG_FILE)
        super(VirtualizationSRIOVHQMVFVMWorkLd, self).__init__(test_log, arguments, cfg_opts, self.virtualization_sriov_dlb_bios_knobs)
    ...
    def prepare(self):
        ...
        super(VirtualizationSRIOVHQMVFVMWorkLd, self).prepare()
    ...
    def execute(self):
        """
        This method installs the HQM on sut and verify if it is installed properly.
        Then execute DLB workload inside guest
        """
        """
        ...
        for vm_name in self.LIST_OF_VM_NAMES:
            ...
            self.create_virtual_function_dlb(self.NUMBER_OF_VMS, vm_index)
            ...
            devlist_mdev = []
            # in case of more than one device below 3 lines have to be called in loop
            domain, bus, device, function = self._vm_provider.create_sriov_dlb2_instance(vm_index)
            cmand = "lspci -Dd :2711"
            ...
            host_dev = ("{}".format(dlb_vf_dev_bdf_list[0]))
            devlist_mdev.append("pt=={}".format(host_dev))

            self.create_vm_qemu_generic(vm_name, self.VM_TYPE, vm_create_async=None, mac_addr=True,
                                        pool_id=free_storage_pool,pool_vol_id=None,
                                        nw_bridge="bridge", devlist=devlist_mdev)
            ...
            #self.attach_vfiopci_device_using_dbdf_to_vm(vm_name, domain, bus, device, function)
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if VirtualizationSRIOVHQMVFVMWorkLd.main() else Framework.TEST_RESULT_FAIL)
