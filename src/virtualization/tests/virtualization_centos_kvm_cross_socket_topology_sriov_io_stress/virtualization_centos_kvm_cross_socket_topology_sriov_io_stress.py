...
class VirtualizationCentOsKvmCrossSocketTopologySriovIoStress(VirtualizationCommon):
    """
    Phoenix ID: 16014124071
    ...
    """
    TEST_CASE_ID = ["P16014124071", "Virtualization-CentOS-KVM-NIC-SRIOV-stress"]
    STEP_DATA_DICT = {
        ...
        16: {'step_details': "Execute the iPerf test from SUT to VM's",
    ...
    BIOS_CONFIG_FILE = "virtualization_centos_kvm_cross_socket_topology_sriov_stress.cfg"
    NUMBER_OF_VMS = 2
    VM = [VMs.CENTOS]*2
    IPERF_EXEC_TIME = 5
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        super(VirtualizationCentOsKvmCrossSocketTopologySriovIoStress, self).__init__(test_log, arguments, cfg_opts)
    ...
    def prepare(self):
        ...
        super(VirtualizationCentOsKvmCrossSocketTopologySriovIoStress, self).prepare()
    ...
    def execute(self):
        ...
        for index in range(self.NUMBER_OF_VMS):
            nw_adapter_name = nw_device_list[index]
        ...
        for index in range(self.NUMBER_OF_VMS):
            ...
            vf_mac_id_dict.extend(mac_id_list)
        ...
        vm_sut_obj_list = []
        self.STORAGE_VOLUME = ["/home"]
        self.LIST_OF_POOL = []
        self.LIST_OF_MOUNT_DEVICE =[]
        ...
        for index in range(len(vfs_data_list)):
            num_range = index % 2
            # attaching the VF device to the VM
            self._test_content_logger.start_step_logger(10)
            self.attach_pci_nw_device_to_vm(vfs_data_list[index], self.LIST_OF_VM_NAMES[num_range])
            self._test_content_logger.end_step_logger(10, True)
            # create VM os object
            vm_os_obj = self.create_vm_host(self.LIST_OF_VM_NAMES[num_range], self.VM[num_range])
            ...
            vf_adapter_name = self.get_vf_adapter_name_in_vm(self.LIST_OF_VM_NAMES[num_range], vf_mac_id_dict[num_range])
            self._test_content_logger.end_step_logger(12, True)
            # assign the IP to the VM
            self._test_content_logger.start_step_logger(13)
            ip, subnet = opt_dict[nw_device_list[index]]
            ip = ip[:-2] + str(int(ip[-2:]) + 5 + index)
            self.assign_ip_to_vf_in_vm(vm_os_obj, ip, subnet, vf_adapter_name, self.LIST_OF_VM_NAMES[num_range])
        ...
        for index in range(self.EXEC_IPERF_INSTANCE):
            ...
            self.get_yum_repo_config(vm_os_obj, common_content_lib_vm, os_type="centos", machine_type="vm")
        ...
        for index in range(len(vfs_data_list)):
            num_range = index % 2
            # Creates VM names dynamically according to the OS and its resources
            vm_name = self.VM[num_range] + "_" + str(num_range)
            ...
            self.remove_virtual_function(nw_device_list[index])
            self._test_content_logger.end_step_logger(19, True)

        return True

    def cleanup(self, return_status):  # type: (bool) -> None
        super(VirtualizationCentOsKvmCrossSocketTopologySriovIoStress, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if VirtualizationCentOsKvmCrossSocketTopologySriovIoStress.main()
