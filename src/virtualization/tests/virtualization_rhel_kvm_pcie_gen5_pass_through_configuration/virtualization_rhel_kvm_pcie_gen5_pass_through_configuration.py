...
class VirtualizationRhelKvmPcieGen5PassThroughConfiguration(VirtualizationCommon):
    """
    PHOENIX ID: 18014075457 - ["Virtualization_RHEL_KVM_PCIe_Gen_5.0_Pass_Through_Configuration"]
    ...
    """
    VM = [VMs.RHEL]
    BIOS_CONFIG_FILE = "virtualization_rhel_kvm_pcie_gen5_pass_through_configuration"
    REGEX_CMD_FOR_VERIFYING_VIRTUAL_ADAPTER = r".*Ethernet\scontroller.*810.*"
    STATIC_IP_FOR_VIRTUAL_ADAPTER = "20.20.20.2{}"

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create a new VirtualizationRhelKvmPcieGen5PassThroughConfiguration object.
        ...
        super(VirtualizationRhelKvmPcieGen5PassThroughConfiguration, self).__init__(test_log, arguments, cfg_opts)
    ...
    def execute(self):
        """
        1.get the PCie Gen5 device name
        ...
        nw_device_list = self.get_nw_adapter_details(gen5_device_name)
        ...
        for index in range(len(self.VM)):
            ...
            nw_adapter_name_list = self.get_nw_adapters_in_vm(vm_os_obj, gen5_device_name)
    ...
    def cleanup(self, return_status):  # type: (bool) -> None
        super(VirtualizationRhelKvmPcieGen5PassThroughConfiguration, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if VirtualizationRhelKvmPcieGen5PassThroughConfiguration.main()
