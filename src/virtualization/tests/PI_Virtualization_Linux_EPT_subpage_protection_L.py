...
CASE_DESC = [
    "Verify that EPT subpage protection (SPP) feature is working fine as expected"
]
...
def test_steps(sut, my_os):
    ...
    set_bios_knobs_step(sut, *bios_knob("enable_vmx_xmlcli"), *bios_knob("enable_vtd_xmlcli"))
    ...
    Case.step("./configure")
    err = sut.execute_shell_cmd("./configure", cwd=sut_tool("VT_KVM_UNIT_TESTS_UNZIP_L"))[2]
    Case.expect('No Error, the log is below', err == '')

    Case.step("make standalone")
    sut.execute_shell_cmd("make standalone", cwd=sut_tool("VT_KVM_UNIT_TESTS_UNZIP_L"), timeout=60 * 2)

    Case.step("./tests/vmx")
    _, out, _ = sut.execute_shell_cmd("./tests/vmx", timeout=60 * 2, cwd=sut_tool("VT_KVM_UNIT_TESTS_UNZIP_L"))
    res = find_lines("EPT violation", str(out))
    count = 0
    for out in res:
        if out[:4] == "PASS":
            count = count + 1
