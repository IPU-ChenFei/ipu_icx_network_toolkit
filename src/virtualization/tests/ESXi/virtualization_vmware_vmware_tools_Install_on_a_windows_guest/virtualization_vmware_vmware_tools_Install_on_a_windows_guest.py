...
class VirtualizationVmwareToolsInstallWindowsGuest(VirtualizationCommon):
    """
    Phoenix ID: 18014072044
    The purpose of this test case is making sure the creation of VMs guests on VMware ESXi.
    1. Enable VT-d bios on ESXi sut.
    2. Copy WINDOWS ISO image to ESXi SUT under 'vmfs/volumes/datastore1'.
    4. Create VM.
    ...
    VM = [VMs.WINDOWS]
    VM_TYPE = "WINDOWS"
    TEST_CASE_ID = ["P18014072044", "Virtualization_VMware_Tools_Install_on_a_Windows_Guest"]
    STEP_DATA_DICT = {
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create a new VirtualizationVmwareToolsInstallWindowsGuest object.

        """
        super(VirtualizationVmwareToolsInstallWindowsGuest, self).__init__(test_log, arguments, cfg_opts)
    ...
    def cleanup(self, return_status):  # type: (bool) -> None
        super(VirtualizationVmwareToolsInstallWindowsGuest, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if VirtualizationVmwareToolsInstallWindowsGuest.main()
