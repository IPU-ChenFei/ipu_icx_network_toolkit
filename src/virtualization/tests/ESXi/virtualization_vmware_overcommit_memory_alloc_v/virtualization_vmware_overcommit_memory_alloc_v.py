...
class VirtualizationVmwareOverCommitMemoryAllocIOWrkLd(VirtualizationCommon):
    """
    Phoenix ID: 18014073349
    The purpose of this test is to verify if all the memory in the system can be allocated and to test ESX's ability to
    overcommit a certain amount of memory.
.
    ...
    VM = [VMs.RHEL]*2
    VM_TYPE = "RHEL"
    TEST_CASE_ID = ["P18014073349", "Virtualization_VMware_OverCommitMemory_IOWorkLd"]
    STEP_DATA_DICT = {
    ...
    VM_MEMORY = 70
    LOGFILE = ['log_5_m.txt', 'log_10_m.txt']
    vm_bytes = ['25G', '24G']
    vm_time = ['5m', '10m']
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        """
        super(VirtualizationVmwareOverCommitMemoryAllocIOWrkLd, self).__init__(test_log, arguments, cfg_opts)
    ...
    def execute(self):
        ...
        for index in range(len(self.VM)):
            ...
            result = self.virt_execute_host_cmd_esxi(f'Get-VM {vm_name} | Set-VM -MemoryGB {vm_memory} -Confirm:$false')
            ...
            vm_stat = common_content_lib_vm_obj.execute_sut_cmd('vmware-toolbox-cmd stat balloon',
                                                                'vmware-toolbox-cmd stat balloon',
                                                                execute_timeout=self._command_timeout,
                                                                cmd_path = self.ROOT_PATH)
            if re.search("0 MB", vm_stat):
                self._log.info('The returned value is 0 MB', vm_stat)

            self._log.info('Begin to memory stress the virtual machine')
            self._log.info(
                'Allow the system to stress for a few hours to ensure that the balloon driver is properly working')

            common_content_lib_vm_obj.execute_sut_cmd_async(
                                                'stress --vm 2 --vm-bytes {} --timeout {}'.format(self.vm_bytes[index],
                                                                                                   self.vm_time[index]),
                                                'stress --vm 2 --vm-bytes {} --timeout {}'.format(self.vm_bytes[index],
                                                                                                   self.vm_time[index]),
                                                execute_timeout= 5 * 60, cmd_path = self.ROOT_PATH)

            res = common_content_lib_vm_obj.execute_sut_cmd('./check_ballon.sh {}'.format(self.LOGFILE[index]),
                                                            './check_ballon.sh {}'.format(self.LOGFILE[index]),
                                                            execute_timeout=5 * 60, cmd_path = self.ROOT_PATH)
            time.sleep(350 + 300*index)
            if not re.search("0 MB", res):
                self._log.info('stress test successfully : No 0 MB, {}', res)
            cat_result = common_content_lib_vm_obj.execute_sut_cmd('cat {}'.format(self.LOGFILE[index]),
                                                                   'cat {}'.format(self.LOGFILE[index]),
                                                                   execute_timeout=self._command_timeout,
                                                                   cmd_path = self.ROOT_PATH)
            if not re.search("0 MB", vm_stat):
                self._log.info('test pass : No 0 MB {}', cat_result)
    ...
    def cleanup(self, return_status):  # type: (bool) -> None
        super(VirtualizationVmwareOverCommitMemoryAllocIOWrkLd, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if VirtualizationVmwareOverCommitMemoryAllocIOWrkLd.main()
