...
class VirtualizationVmwareVMGuestOsInstall(VirtualizationCommon):
    """
    ...
    4. Create VM and imstall OS.
    5. Verify VM is running.
    6. Verify VMware tool installed on VM or not.
    """
    VM = [VMs.RHEL, VMs.WINDOWS]
    VM_TYPE = ["RHEL", "WINDOWS"]

    TEST_CASE_ID = ["P18014073884", "Virtualization_VMware_VM_Guest_OS_Install"]
    STEP_DATA_DICT = {
        1: {'step_details': "Install and verify Bios knobs for VT-d",
            'expected_results': "Bios knobs installed and verified successfully'"},
        2: {'step_details': "Create RHEL/WINDOWS VM on ESXi SUT",
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        """
        super(VirtualizationVmwareVMGuestOsInstall, self).__init__(test_log, arguments, cfg_opts)
    ...
    def cleanup(self, return_status):  # type: (bool) -> None
        super(VirtualizationVmwareVMGuestOsInstall, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if VirtualizationVmwareVMGuestOsInstall.main()
