...
class VirtualizationVmwareQATSriovMultiVMMultiPFDataMov(BaseQATUtil):
    """
    Phoenix ID: 16013374116
    The purpose of this test case is to execute the scenario -
    Virtualization - Vmware Esxi - QAT_SRIOV_Multi_VM_Multi_PF_Data_Movement_L.
    1. Enable VT-d bios on ESXi sut.
    2. Install the QAT driver
    3. Enable SRIOV and Create 16 VFs for each QAT PFs
    ...
    8. Create/Enable the Passthrough for each of the 2 VFs created from each of the PF in host
    9. Attach 2 VFs created from each of the PF to single VM
    ...
    no_of_qat_devices = 0
    TEST_CASE_ID = ["P16013374116", "VirtualizationVmwareQATSriovMultiVMMultiPFDataMov"]
    STEP_DATA_DICT = {
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        """
        super(VirtualizationVmwareQATSriovMultiVMMultiPFDataMov, self).__init__(test_log, arguments, cfg_opts)
    ...
    def add_qat_devices_to_vm_with_allvf_enabled(self, sut_ip, vm_name, start_index, no_of_dev):
        index = 0
        ...
        cmand = "$devs=Get-PassthroughDevice -VMHost {} | Where-Object Name -like '*QAT VF*';" \
                ...
                "   }}; " \
                "}};".format(sut_ip, start_index, self.no_of_qat_devices, no_of_dev, vm_name)
    ...
    def execute(self):
        ...
        self.no_of_qat_devices = hostsocket * self.QAT_DEVICE_NUM
        self._test_content_logger.end_step_logger(4, return_val=True)
        max_vf_per_qat = 16
        qat_vf_per_pf_per_vm = int(max_vf_per_qat/self.NUM_VM)
        qat_bdf_list = []
        self._test_content_logger.start_step_logger(5)
        for qat_index in range(0, self.no_of_qat_devices):
            qat_bdf_value = self.get_qat_device_bdf_value(qat_index)
            qat_bdf_list.append(qat_bdf_value)
            self.enable_SRIOV(qat_bdf_value, self.sut_ip, num=max_vf_per_qat)
        ...
        for vm_index, vm_name in enumerate(self.LIST_OF_VM_NAMES):
            for qat_pf_index in range(self.no_of_qat_devices):
                #assign 2 VFs from each PF to each VM
                start_qat_vf_index = (vm_index*2) + (max_vf_per_qat*qat_pf_index)
                for qat_vf_index in range(start_qat_vf_index, start_qat_vf_index + 2):
                    try:
        ...
        for vm_index, vm_name in enumerate(self.LIST_OF_VM_NAMES):
            ...
            self.add_qat_devices_to_vm_with_allvf_enabled(self.sut_ip, vm_name, vm_index, max_vf_per_qat)

            qat_vf_index_used = qat_vf_index_used + ((vm_index+1)*qat_vf_per_pf_per_vm)
        ...
        for vm_name, vm_os_obj in zip(self.LIST_OF_VM_NAMES, vm_sut_obj_list):
            common_content_obj_vm = CommonContentLib(self._log, vm_os_obj, self._cfg_opts)
            cpa_sample_command = "./cpa_sample_code; sync"  # data_movment
        ...
        total_qat_vf_used = self.no_of_qat_devices * max_vf_per_qat
        for qat_vf_index in range(0, total_qat_vf_used):
            try:
    ...
    def cleanup(self, return_status):  # type: (bool) -> None
        super(VirtualizationVmwareQATSriovMultiVMMultiPFDataMov, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if VirtualizationVmwareQATSriovMultiVMMultiPFDataMov.main()
