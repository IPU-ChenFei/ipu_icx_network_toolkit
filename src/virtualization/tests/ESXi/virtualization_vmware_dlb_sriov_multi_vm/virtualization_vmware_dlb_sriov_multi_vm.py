...
class VirtualizationVmwareDLBSRIOVMultiVM(VirtualizationCommon):
    """
    ...
    """
    NUMBER_OF_VMS = 8
    VM = [VMs.CENTOS] * 8 # tested for 2 vms
    VM_TYPE = "CENTOS"
    test_time = 15*60
    TEST_CASE_ID = ["P16013527924", "Virtualization_VMware_DLB_SRIOV_Multi_VM"]
    STEP_DATA_DICT = {
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        """
        super(VirtualizationVmwareDLBSRIOVMultiVM, self).__init__(test_log, arguments, cfg_opts)
    ...
    def add_dlb_drivers_to_vm(self,sut_ip, vm_name,index):
        ...
        for dlb_index in range(0, no_of_dlb_devices):
            cmand = "$devs=Get-PassthroughDevice -VMHost {} | Where-Object Name -like '*DLB VF*';" \
                     ...
                     "Add-PassthroughDevice -VM {} -PassthroughDevice $devs[{}];" \
                     "Add-PassthroughDevice -VM {} -PassthroughDevice $devs[{}];"
            self.virt_execute_host_cmd_esxi(cmand.format(sut_ip,vm_name,2+index, vm_name,1+index, vm_name,0+index,
                                                         vm_name,3+index, vm_name,4+index))
            time.sleep(60)
        return
    ...
    def cleanup(self, return_status):  # type: (bool) -> None
        super(VirtualizationVmwareDLBSRIOVMultiVM, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if VirtualizationVmwareDLBSRIOVMultiVM.main()
