...
class VirtualizationRhelKvmVmScalableIovRunDSADMAStress(VirtualizationCommon, DsaBaseTest_virtualization):
    """
    HPALM ID: 16013929455
    ...
    7. Run DMA workload on the VM and host
    """
    ...
    TEST_CASE_ID = ["H16013929455", "DSA_TEST"]
    STORAGE_VOLUME = ["/home"]
    DSA_VM_LOAD_TEST_TIME = 1000
    ...
    STEP_DATA_DICT = {1: {'step_details': 'Enable Intel VT for Directed I/O (VT-d), Interrupt Remapping, PCIe ENQCMD '
                      ...
                      2: {'step_details': 'IOMMU scalable mode need to be enabled in', 'expected_results': 'Should be '
                                                                                                 'successfull'},
                      3: {'step_details': 'Configure Work Queue and DSA', 'expected_results': 'Fail if not found the '
                                                                                             'devices'},
                      4: {'step_details': 'Enable DSA Device', 'expected_results': 'Verify DSA devices '
                                                                                               'should be dsa0 to dsa7 devices should be available'},
                      5: {'step_details': 'Enable Workqueue and associate UUID to WQ', 'expected_results': 'Should be successfull'},
                      6: {'step_details': 'Configure DSA on guest', 'expected_results': 'Should be successfull'},
                      7: {'step_details': 'Run DMA workload on the VM and host', 'expected_results': 'Should be successfull'},
                      }
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        super(VirtualizationRhelKvmVmScalableIovRunDSADMAStress, self).__init__(test_log, arguments, cfg_opts,
    ...
    def prepare(self):
        ...
        super(VirtualizationRhelKvmVmScalableIovRunDSADMAStress, self).prepare()
    ...
    def execute(self):
        ...
        for index in range(self.NUMBER_OF_VMS):
            ...
            time.sleep(150)
            uid_list = self.get_dsa_uuid()
            uuid = uid_list[index]
            self._log.info("uuid info:{}".format(uid_list))

            self.attach_mdev_instance_to_vm(vm_name, uuid)
            self._log.info("uuid info:{}".format(uuid))
            start_time = time.time()
            seconds = self.DSA_VM_LOAD_TEST_TIME
            while True:
                current_time = time.time()
                elapsed_time = current_time - start_time

                if elapsed_time > seconds:
                    self._log.info("Finished DSA stress test after: " + str(int(elapsed_time)) + " seconds")
                    break
                else:
                    install_collateral_vm_obj = InstallCollateral(self._log, vm_os_obj, self._cfg_opts)
    ...
    def cleanup(self, return_status):  # type: (bool) -> None
        super(VirtualizationRhelKvmVmScalableIovRunDSADMAStress, self).cleanup(return_status)

if __name__ == "__main__":
    sys.exit(
        Framework.TEST_RESULT_PASS if VirtualizationRhelKvmVmScalableIovRunDSADMAStress.main() else Framework.TEST_RESULT_FAIL)
