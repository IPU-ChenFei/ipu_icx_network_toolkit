...
class VirtualizationHyperVConfigureTestSriov(VirtualizationCommon):
    """
    Phoenix ID: 16013171686-Virtualization - Hyper-V - Configure and test SR-IOV with 3rd party Mellanox Network Card
    """
    TEST_CASE_ID = ["p16013171686",
                    "Virtualization_Hyper-V_Configure_and_test_SR-IOV_with_3rd_party_Mellanox_Network_Card"]
    STEP_DATA_DICT = {
        ...
        4: {'step_details': "Assign Static IP to the mellanox card ethernet adapter",
            'expected_results': "Successfully Assigned Static IP to the mellanox card ethernet adapter"},
        5: {'step_details': "Execute the PING test from SUT to mellanox card",
            'expected_results': "PING test passed"},
    }
    BIOS_CONFIG_FILE = "virtualization_hyper_v_configure_and_test_sriov_mellanox_card.cfg"
    NUMBER_OF_VMS = 1
    VM = [VMs.WINDOWS]
    VM_TYPE = "RS5"
    NETWORK_ASSIGNMENT_TYPE = "SRIOV"
    VSWITCH_NAME_MELLANOX = "MELLANOX_SRIOV_VSWITCH"
    ...
    def execute(self):
        ...
        4. Assign the SRIOV to the mellanox card network adapter of the hyper-v module
        ...
        for index in range(self.NUMBER_OF_VMS):
            ...
            self._vm_provider.get_adapter_name_and_create_vswitch_for_sriov(
                self.VSWITCH_NAME_MELLANOX, vm_physical_adapter, index+1)
            mac_id = self._vm_provider.configure_mac_id_for_sriov_vf()
            self._vm_provider.add_sriov_ethernet_adapter_to_vm(vm_name, self.VSWITCH_NAME_MELLANOX, mac_id[index])
            ...
            self._log.info("Assigning static ip to mellanox card:")
