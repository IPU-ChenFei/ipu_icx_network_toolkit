...
class VirtualizationRhelKvmVmScalableIovAndDsa_stress(VirtualizationCommon,DsaBaseTest_virtualization):
    """
    ...
    6. Then restore the snapshot of the DSA.
    7. Run workload to attached device as MDEV.
    8. Continously stress the system
    """
    ...
    STEP_DATA_DICT = {1: {'step_details': 'Enable Intel VT for Directed I/O (VT-d), Interrupt Remapping, PCIe ENQCMD '
                      ...
                      7: {'step_details': 'Run workload to attached device as MDEV',
                          'expected_results': 'Should be successfull'},
                      8: {'step_details': 'Run workload on Host and VM for continue 3 iterations',
                          'expected_results': 'Should be successfull'},
                      }
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        super(VirtualizationRhelKvmVmScalableIovAndDsa_stress, self).__init__(test_log, arguments, cfg_opts, self.dsa_bios_enable)
    ...
    def prepare(self):
        ...
        super(VirtualizationRhelKvmVmScalableIovAndDsa_stress, self).prepare()
    ...
    def execute(self):
        """
         ...
         7. Run workload to attached device as MDEV.
         8. Run workload on Host and VM for continue 3 iterations
        ...
        for index in range(self.NUMBER_OF_VMS):
            ...
            #elf.attach_multi_mdev_instance_to_vm(vm_name, uuid_list)
            ...
            while no_attempts < 3:
                ...
                install_collateral_vm_obj.run_workload_on_vm(spr_dir_path, common_content_lib_vm, vm_os_obj)
    ...
    def cleanup(self, return_status):  # type: (bool) -> None
       super(VirtualizationRhelKvmVmScalableIovAndDsa_stress, self).cleanup(return_status)

if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if VirtualizationRhelKvmVmScalableIovAndDsa_stress.main() else Framework.TEST_RESULT_FAIL)
