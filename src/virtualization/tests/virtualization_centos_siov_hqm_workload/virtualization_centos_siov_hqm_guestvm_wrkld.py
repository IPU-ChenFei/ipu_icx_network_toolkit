...
class VirtualizationSIOVHQMVDevVMWorkLd(VirtualizationCommon):
    """
    Phoenix ID : 16013929532
    ...
    VM = [VMs.CENTOS] * 1
    VM_TYPE = "CENTOS"
    TEST_CASE_ID = ["P16013929532", "Virtualization_SIOV_Dlb_guestVM"]
    BIOS_CONFIG_FILE = "virtualization_siov_dlb_bios_knobs.cfg"
    STORAGE_VOLUME = ["/home"]
    STEP_DATA_DICT = {
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        """
        self.virtualization_siov_dlb_bios_knobs = os.path.join(os.path.dirname(os.path.abspath(__file__)), self.BIOS_CONFIG_FILE)
        super(VirtualizationSIOVHQMVDevVMWorkLd, self).__init__(test_log, arguments, cfg_opts, self.virtualization_siov_dlb_bios_knobs)
    ...
    def prepare(self):
        ...
        super(VirtualizationSIOVHQMVDevVMWorkLd, self).prepare()
    ...
    def execute(self):
        ...
        for vm_name in self.LIST_OF_VM_NAMES:
            ...
            mdev_dev = "/sys/bus/mdev/devices/{}".format(uuid)
            devlist_mdev.append("mdev=={}".format(mdev_dev))

            self.create_vm_qemu_generic(vm_name, self.VM_TYPE, vm_create_async=None, mac_addr=True, pool_id=free_storage_pool,
                                        pool_vol_id=None, nw_bridge="bridge", devlist=devlist_mdev)
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if VirtualizationSIOVHQMVDevVMWorkLd.main() else Framework.TEST_RESULT_FAIL)
