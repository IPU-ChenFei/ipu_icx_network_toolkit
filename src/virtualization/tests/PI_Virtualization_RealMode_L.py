"""
 @Case Link: https://hsdes.intel.com/appstore/article/#/1509883673
 @Author:Liu, JianjunX
 @Prerequisite:
    1. SW Configuration
        1. QEMU is installed:
...
CASE_DESC = [
    "The system checks Arch extentions that enable guest's software to access Real Mode"
]
...
def test_steps(sut, my_os):
    ...
    tool_name_master = f"{sut_tool('VT_TOOLS_L')}/kvm-unit-tests-master/"
    ...
    sut.execute_shell_cmd('./configure', cwd=tool_name_master)

    Case.step("make standalone")
    sut.execute_shell_cmd('make standalone', cwd=tool_name_master)

    Case.step("./tests/realmode")
    _, out, _ = sut.execute_shell_cmd("./tests/realmode", cwd=tool_name_master)
    # The successful code need to the last line of log has string "PASS realmode "
    res = log_check.find_lines("PASS", out)
    result = log_check.find_keyword("realmode ", res[-1])
