...
class VirtualizationCreateVMRhelCentOSWinIOStress(VirtualizationCommon):
    ...
    """
    TEST_CASE_ID = ["p16014379584", "Virtualization-CentOS-CreateVM-Rhel-CentOS-Win-IO-Stress"]
    STEP_DATA_DICT = {
    ...
    BIOS_CONFIG_FILE = "virtualization_centos_sriov_io_stress.cfg"
    RHEL_VM_TYPE = "RHEL"
    WIN_VM_TYPE = "WINDOWS"
    ...
    def execute(self):
        ...
        # create VF
        self.create_virtual_function(self.NO_OF_LINUX_VMS, nw_adapter_name)
        ...
        mac_id_list = self.assign_macs_to_vfs(self.NO_OF_LINUX_VMS, vf_mac_id_details, nw_adapter_name)
        ...
        for index, vm_name in zip(range(len(self.LIST_OF_VM_NAMES)), self.LIST_OF_VM_NAMES):
            ...
            self.attach_pci_nw_device_to_vm(vfs_data_list[index], vm_type)
            ...
            vf_adapter_name = self.get_vf_adapter_name_in_vm(vm_type, mac_id_list[index])
            self._test_content_logger.end_step_logger(12, True)
            # assign the IP to the VM
            self._test_content_logger.start_step_logger(13)
            self.assign_ip_to_vf_in_vm(vm_os_obj, ip, subnet, vf_adapter_name, vm_type)
        ...
        for index, vm_name in zip(range(len(self.LIST_OF_VM_NAMES)), self.LIST_OF_VM_NAMES):
            ...
            self.detach_pci_nw_device_from_vm(vfs_data_list[index], vm_type)
