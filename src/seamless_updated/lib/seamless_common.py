...
class SeamlessBaseTest(BaseTestCase):
    ...
    def __init__(self, test_log, arguments, cfg_opts, bios_config_file_path=None):
        ...
        self.start_workload_command = ""
        self.stop_workload_command = ""
        ...
        self.bios_util = BiosUtil(cfg_opts, bios_config_file=self.bios_config_file_path, bios_obj=self.bios, log=self._log, common_content_lib=self._common_content_lib)
    ...
    def block_until_complete(self, pre_version, update_type=""):
        ...
        if update_type == "SPS":
            ...
            self._log.info("SPS Activation Time: "+ str(self.time_sps_ok-self.time_sps_reset))
            if ((self.time_sps_ok-self.time_sps_reset) > timedelta(seconds=10)):
    ...
    def send_capsule(self, path, timeout_seconds, start_workloads=False, update_type="", capsule_path2=""):
        ...
        if self.run_orchestrator:
            try:
                ...
                self._log.error("bmc reset operation failed, please reset the bmc manually before proceeding to next test case")

        return result

    def send_capsule_negative(self, path, timeout_seconds, start_workloads=False, update_type="", capsule_path2="", capsule_type=""):
        """
        Send staging capsule for Seamless update
        :param path: Path location of capsule.
        :param timeout_seconds: Amount of time to wait (in seconds) before canceling out of BMC update process.
            ...
            if not done_staging:
                ...
                if capsule_type == 'negative':
                    if self.get_current_version() == initial_version:
                        self._log.info(
                            "\t\tThe version {} is expected version {}".format(self.get_current_version(), initial_version))
                        self.time_staging = datetime.now() - time_start
    ...
    def send_capsule_parallel(self, path, timeout_seconds, start_workloads=False, update_type="", capsule_path2=""):
        ...
        try:
            ...
            if not (self.sut_mode == "uefi" or self.sut_mode == "S5"):
                initial_version = self.get_current_version()
                if initial_version == self.expected_ver:
                    self._log.info("\tThe initial version {} is already the expected version: Proceeding with the update".format(
                            initial_version))
                elif self.expected_ver in initial_version:
                    self._log.info("\tThe expected version {} is already in the initial version: Proceeding with the update".format(
                            self.expected_ver))
            ...
            self._log.info("======LATEST CAPSULE UPDATE REQUEST REJECTED, MOVING FORWARD WITH THE EXISTING CAPSULE UPDATE=======\n")
    ...
    def send_capsule_inband(self, path, start_workloads=False, update_type="", capsule_type=""):
        ...
        try:
            ...
            if capsule_type == 'negative':
                ...
                if post_version == initial_version:
                    ...
                    return result
            if not post_version == self.expected_ver:
                self._log.error("\tThe version {} is not the expected version {}".format(post_version,self.expected_ver))
                result = False
            elif post_version == 'NONE':
                self.summary_log.error("Failed to detect version: reboot system or flash bios image")
                self._log.error("\tFailed to detect version: reboot system or flash bios image")
        ...
        return result and not errors
    def send_capsule_inband_parallel(self, path, start_workloads=False, update_type="", capsule_type="", capsule_path2=""):
        """
        Send staging capsule for Seamless update using In-Band methods
        :param path: Path location of capsule.
        :param start_workloads: Spin up workloads during test.
    ...
    def stage_inband_capsule_update_parallel(self, path, path2):
        ...
        if self._os_type == "Linux":
            ...
            t1 = threading.Thread(target=self.run_ssh_command, kwargs=dict(command="sudo cat /tmp/" + filename + " > /dev/efi_capsule_loader", log_output=True, timeout_seconds=60))
            t2 = threading.Thread(target=self.run_ssh_command, kwargs=dict(command="sudo cat /tmp/" + filename2 + " > /dev/efi_capsule_loader", log_output=True,
                                  timeout_seconds=60))
            t1.start()
            time.sleep(1)
            t2.start()
    ...
    def start_VM(self):
        ...
        Restore VMs
        """
        self._log.info("Restore VMs after warm reset")
    ...
    def begin_workloads(self):
        ...
        self.run_powershell_command(command=self.start_workload_command, get_output=True)

    def begin_workloads_lin(self):
        """
