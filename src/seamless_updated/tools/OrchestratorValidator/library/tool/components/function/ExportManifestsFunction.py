...
from ...utils import save_node_to_file
...
class ExportManifestsFunction(IOutputManifestsFunction):
    ...
    def _parse_children(self, xml_node, **kwargs):
        ...
        load_data_node = self._parse_node(xml_node, self.Tags.LoadDataNode)
    ...
    def _build(self, buffer):
        ...
        if self.manifests:
            ...
            ColorPrint.success(f'{len(self.manifests)} manifests exported to {os.path.abspath(self.output.value)}')
        else:
            ColorPrint.warning('No manifest has been found.')
        self._move_buffer_to_end(buffer)

    @staticmethod
    def _save_manifest(manifest):
        with open(manifest.binary_path, 'wb') as f:
    ...
    def _copy_manifest_function_node(self, tag, setting_to_enable):
        ...
        function_node.attrib[IComponent.enabledTag] = f'/{LibConfig.settingsTag}/{setting_to_enable}.value != 0'
    ...
    def _create_layout_node(self):
        ...
        SubElement(layout_node, self.load_data_component.value.name,
                      {ValueWrapper.Tags.CalculateTag: self.load_data_component.value.value_formula})
        ...
        function_import_node.find(self.Tags.Output).attrib[ValueWrapper.Tags.CalculateTag] =\
            f'/{LibConfig.settingsTag}/binary_input.path'
        self._remove_nodes(function_import_node, [self.Tags.LoadDataNode])
        ...
        self._remove_nodes(function_validate_node, [self.Tags.Output, self.Tags.LoadDataNode])
    ...
    def _add_common_nodes(self, manifests_function_node):
        SubElement(manifests_function_node,
                   self.Tags.ManifestsList,
                   attrib={ValueWrapper.Tags.CalculateTag: f'/{LibConfig.settingsTag}/{self.ManifestsSettingName}'})
    ...
    def _save_input_xml(self):
        ...
        save_node_to_file(self.import_xml_root, xml_path)
