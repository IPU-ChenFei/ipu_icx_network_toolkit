...
class SEAM_BMC_0013_prm_update(SeamlessBaseTest):
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        self.prm_multiple_handlers_smi_event = arguments.prm_multiple_handlers_smi_event
        self.invoke_multiple_prm_handlers = arguments.invoke_multiple_prm_handlers
    ...
    def add_arguments(cls, parser):
        ...
        parser.add_argument('--prm_end_to_end_flow_for_address_translation',
                            ...
                            default="")
        parser.add_argument('--prm_multiple_handlers_smi_event',
                            action='store_true',
                            help="Add argument for multiple prm handler invocation along with smi event",
                            default="")
        parser.add_argument('--invoke_multiple_prm_handlers',
                            action='store_true',
                            help="Add argument for multiple prm handler invocation",
                            default="")
    ...
    def execute(self):
        ...
        if self.prm_valid_capsule_same_ver:
        ...
        elif self.prm_multiple_handlers_smi_event:
            """
            TC 67386.4 Command:
            python3 src\\seamless\\tests\\bmc\\functional\\SEAM_BMC_0013_prm_update.py 
            --prm_multiple_handlers_smi_event,
            ...
            python3 src\\seamless\\tests\\bmc\\functional\\SEAM_BMC_0013_prm_update.py
             "--prm_multiple_handlers_smi_event",
            "--loop" <number of loops>
            "--min_delay" <delay as per TC>,
            "--max_delay" <delay as per TC>
            """
            self.flow_type = "invoke"
            self.handler = "dummy"
            result = self.invoke_prm_handler_os_wrapper()
        ...
        elif self.invoke_multiple_prm_handlers:
            """
            TC 67385.3 Command:
            python3 src\\seamless\\tests\\bmc\\functional\\SEAM_BMC_0013_prm_update.py 
            "--invoke_multiple_prm_handlers",
            "--loop", "100"
            """
            self.flow_type = "invoke"
            self.handler = "dummy"
            ...
            if self.dummy_handler_version_1_x(1):
                ...
                return self.post_prm_cleanup()
            self._log.info("Parallely running dummy and addrtrans invoke handler")
            invoke_dummy_thread = ThreadWithReturn(target = self.prm_handler_invocation_loop,args =("invoke","addrtrans"))
            invoke_addrtrans_thread = ThreadWithReturn(target = self.prm_handler_invocation_loop,args =("invoke","dummy"))
            self._log.info("Starting dummy invoke thread")
            invoke_dummy_thread.start()
            self._log.info("Starting addrstrans invoke thread")
            invoke_addrtrans_thread.start()
            ...
            result_addrtrans_thread = invoke_addrtrans_thread.join()
            if result_dummy_thread and result_addrtrans_thread:
                self._log.info("PRM invoke for addrtrans and dummy handler successful")
                self.post_prm_cleanup()
                return result
            else:
                self._log.info("PRM invoke for addrtrans and dummy handler Failed")
                self.post_prm_cleanup()
