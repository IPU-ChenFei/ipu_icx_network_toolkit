...
def test_steps(tcd):
    ...
    # Tool Version [0.2.11]
    #ID/LINK: https://hsdes.intel.com/appstore/article/#/15011075066
    #TITLE: DSA_SIOV_mdev_user_test_all_devices_multi_VM_L
    #DOMAIN: accelerator
    ...
    if tcd.prepare("setup system for test"):
        ...
        # Note:For intel next kernel 5.15 or newer , idxd.legacy_cdev_load1 modprobe.blacklist=idxd_uacce is no longer needed
        ...
        # Note: For intel next kernel 5.15 or newer , idxd.legacy_cdev_load1 modprobe.blacklist=idxd_uacce is no longer needed
        # 5.cp or git clone
        # accel-random-config-and-test to /root/DSA for Host
    ...
    if tcd.step("Linux Kernel parameters"):
        ...
        ### Notes ###
        # For intel next kernel 5.15 or newer , idxd.legacy_cdev_load1 modprobe.blacklist=idxd_uacce is no longer needed
        ##################
    ...
    if tcd.step("Configure mdev work-queues and start virtual machine"):
        ...
        # ./Setup_Randomize_DSA_Conf.sh -maD -F1
        
        # 2. Start VM using launch script
        # ./Setup_Mdev_Multi_VM_DSA_IAX.sh -c1 -m1 -n<#of MDEVS>
        
        # Note: replace  " <# of MDEVS> " with the total number of WQs/MDEVs configured in step 4. This will launch the provided number of VMs with one MDEV per VM.
        
        sutos.execute_cmd(sut, f'cd $API_SCRIPT && python3 $API_SCRIPT/acce_random_test.py -c \'./Setup_Randomize_DSA_Conf.sh -maD -F1\' -u "False"', timeout=20*60)
        ...
        sutos.execute_cmd(sut, f'cd $ACCE_RANDOM_CONFIG_TEST_PATH_L && ./Setup_Mdev_Multi_VM_DSA_IAX_modify_for_auto.sh -n64 -c1 -m1', timeout=20*60)
        tcd.sleep(30)
        ##################
        tcd.log("### Expected result ###")
    ...
    ## 6
    if tcd.step("Open new terminals to open all VMs"):
        # Open new terminals to open all VMs
    ...
    if tcd.step("Login VM and run dmatest ."):
        ...
        sutos.execute_cmd(sut, f'cd $API_SCRIPT && python3 $API_SCRIPT/vm_execute.py -n 64 -c "python3 $API_SCRIPT/dsa_test.py -s \'Guest_Mdev_Randomize_DSA_Conf.sh\' -m \'all\'"', timeout=30*60)
    ...
    if tcd.step("Shutdown virtual machines on Host"):
        ...
        sutos.execute_cmd(sut, f'cd $API_SCRIPT && python3 $API_SCRIPT/vm_execute.py -n 64 -c "shutdown now"', no_check=True, timeout=10*60)
        ##################
        tcd.log("### Expected result ###")
