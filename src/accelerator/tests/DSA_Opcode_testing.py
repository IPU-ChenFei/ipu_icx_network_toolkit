...
def test_steps(sut, my_os):
    ...
    if ker_ver <= acce.CENTOS_INTEL_NEXT_KERNEL:
        # Step 2 - run dsa_memmove.sh in idxd file
        Case.step('run dsa_memmove.sh in idxd file')
        _, out, err = sut.execute_shell_cmd('./dsa_memmove.sh', timeout=10*60, cwd=f'{DSA_PATH_L}*/test/')
    else:
        # Step 3 - run dsa_memmove.sh in accel_ramdom file
        Case.step('run dsa_memmove.sh in accel_ramdom file')
        _, out, err = sut.execute_shell_cmd('./dsa_memmove.sh', timeout=10 * 60, cwd=f'{SPR_ACCE_RANDOM_CONFIG_PATH_L}*/dsa_test/')
