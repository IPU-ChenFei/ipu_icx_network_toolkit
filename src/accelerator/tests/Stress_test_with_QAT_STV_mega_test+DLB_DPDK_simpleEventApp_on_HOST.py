...
def test_steps(sut, my_os):
    ...
    acce.init_bashrc()
    sut.execute_shell_cmd('yum -y install https://dl.fedoraproject.org/pub/epel/epel-release-latest-8.noarch.rpm --nogpgcheck', timeout=10*60)
    ...
    # Step 4 - Run dpdk test
    Case.step('Run dpdk test')
    acce.run_dpdk()
    dpdk_path = f'{DPDK_DRIVER_PATH_L}*/builddir/app'
    dpdk_command = f'./dpdk-test-eventdev -c 0xf --vdev=dlb2_event -- ' \
                   f'--test=order_queue --plcores=1 --wlcore=2,3 --nb_flows=64'
    _, out, _ = sut.execute_shell_cmd('{}'.format(dpdk_command), cwd=dpdk_path)
    acce.check_keyword(["Success"], out, 'Run dpdk stress fail')
    sut.execute_shell_cmd('ln -s /usr/bin/python3 /usr/bin/python', timeout=60)


    # Step 5 - check mce log
