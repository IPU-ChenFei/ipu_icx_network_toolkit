...
def test_steps(sut, my_os):
    ...
    Case.prepare('Complete TC 15010636537 - ESXi_DSA_Virtualization_SIOV_assign_vDev_Guest (intel.com)')
    logger.info('')
    logger.info('---- Test Boot to Default os ----')
    acce.dsa_driver_install_esxi()
    #

    # # # Prepare step 1 - Check iadx device in VMware OS
    ...
    # # Prepare step 4
    # # Prepare step 5 - Verify DSA device available in guest VM
    ...
    esxi.wq_configuration_dmatest('dsa')


    # Step 1 - Login guest VM
    Case.step('Login guest VM')
    # Step 2 - Verify DSA device available in guest VM
    Case.step('Verify DSA device available in guest VM')
    _,out,_ = esxi.execute_vm_cmd(vm_name0, f'lspci | grep -i {acce.dsa_id}')
    acce.check_keyword(['Intel Corporation Device'], out, f'device {acce.dsa_id} not found')
    _,out,_ = esxi.execute_vm_cmd(vm_name0, f'lspci -k | grep idxd')
    acce.check_keyword(['idxd'], out, 'idxd not found')
    ...
    # Step 4 - Verify DSA device available in guest VM
    Case.step('Verify DSA device available in guest VM')
    _,out,_ = esxi.execute_vm_cmd(vm_name0, f'lspci | grep -i {acce.dsa_id}')
    acce.check_keyword(['Intel Corporation Device'], out, f'device {acce.dsa_id} not found')
    _,out,_ = esxi.execute_vm_cmd(vm_name0, f'lspci -k | grep idxd')
    ...
    # # Prepare step 6 - Configure WQ and run dmatest
    esxi.wq_configuration_dmatest(vm_name0,'dsa')
