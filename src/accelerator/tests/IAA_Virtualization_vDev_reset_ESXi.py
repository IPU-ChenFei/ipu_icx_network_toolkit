...
CASE_DESC = [
    'Verify IAA device is available in guest VM after a reset.  '
]
...
def test_steps(sut, my_os):
    ...
    #
    # # Prepare Step 1 - Make sure the VM is unregistered from ESXi while modifying the *.vmx file
    esxi.undefine_vm(vm_name0)

    # Prepare Step 2 - Configure device passthrough
    esxi.attach_mdev_to_guest(vm_name0,'iaa')
    esxi.passthrough_memory_conf(memsize, vm_name0)

    # Prepare Step 3 - Re-register and Power-on guest VM
    esxi.define_vm(vm_name0)
    esxi.start_vm(vm_name0)
    #
    # Prepare step 4 - Login guest VM
    # Prepare step 5 - Verify IAX device available in guest VM
    ...
    # Prepare step 6 - Configure WQ and run dmatest
    esxi.kernel_header_internal_devel(vm_name0)
    esxi.wq_configuration_dmatest(vm_name0, ip_name='iax')

    # Step 1 - Login guest VM
    Case.step('Login guest VM')

    # Step 2 - Verify IAA device available in guest VM
    Case.step('Verify IAA device available in guest VM')
    _,out,_ = esxi.execute_vm_cmd(vm_name0, f'lspci | grep -i {acce.iax_id}')
    acce.check_keyword(['Intel Corporation Device'], out, f'device {acce.iax_id} not found')
    _,out,_ = esxi.execute_vm_cmd(vm_name0, f'lspci -k | grep idxd')
    acce.check_keyword(['idxd'], out, f'device idxd not found')

    # step 3 - Reboot Guest VM
    Case.step('Reboot Guest VM')
    esxi.reset_vm(vm_name0)
    #
    ...
    esxi.wq_configuration_dmatest(vm_name0, ip_name='iax',config=True)
    esxi.undefine_vm(vm_name0)
