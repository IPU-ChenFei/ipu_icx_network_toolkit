...
def test_steps(tcd):
    ...
    # Tool Version [0.2.13]
    #ID/LINK: https://hsdes.intel.com/appstore/article/#/15010866784
    #TITLE: DSA_usermode_test_with_iommu_sm_on_DPDK_L
    #DOMAIN: accelerator
    ...
    if tcd.prepare("setup system for test"):
        ...
        sutos.execute_cmd(sut, f'cd $SUT_TOOLS && unzip -o $DPDK_DRIVER_BKC_ZIP_NAME')
        ...
        # 1.set feature in kernel "intel_iommu=on,sm_on iommu=on   "
        #grubby --args="intel_iommu=on,sm_on iommu=on   " --update-kernel=/boot/vmlinuz-*.intel_next.*.x86_64+serv
        ...
        sutos.execute_cmd(sut, f'yum install -y meson', timeout=10*60)
        sutos.execute_cmd(sut, f'yum -y install python3-pyelftools', timeout=10*60)
    ...
    if tcd.step("Build DPDK:"):
        ...
        sutos.execute_cmd(sut, f'cd $Accelerator_REMOTE_TOOL_PATH/dpdk && meson build', timeout=10*60)
    ...
    if tcd.step("ninja -C build"):
        ...
        sutos.execute_cmd(sut, f'cd $Accelerator_REMOTE_TOOL_PATH/dpdk && ninja -C build', timeout=10*60)
        ##################
        tcd.log("### Expected result ###")
