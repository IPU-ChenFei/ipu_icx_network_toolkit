...
def test_steps(sut, my_os):
    ...
    # Step 4
    Case.step('Install QAT driver and check qat service status')
    ...
    # Step 4 - Check QAT device status and unbind QAT VF device
    Case.step('Check QAT device status and unbind QAT VF device')
    qat_dev_list = []
    vf_num = 16
    for pf in range(pf_num):
        qat_list = acce.get_qat_dev_id_list(pf, vf_num)
        qat_dev_list.append(qat_list)
        for i in range(1, vf_num+1):
            acce.unbind_device(f'qat_{pf}_{i}')
    print(qat_dev_list)

    dev_id_list = []
    for dev_id in qat_dev_list:
        for dev in dev_id:
            dev_id_list.append(dev)
    print(dev_id_list)

    # Step 5 - Create vm and attach device list to vm, open vm
    Case.step('Create vm and attach device list to vm, open vm')
    qemu.create_rich_vm(pf_num, file_name)
    qemu.attach_acce_dev_to_vm_grouply(dev_id_list, 16)
    qemu.start_rich_vm()

    # Step 6 - Install kernel package and clear abort log
    Case.step('Install kernel package and clear abort log')
    ...
    # Step 7 - Check QAT device,Install QAT driver and run qat sanmple code stress
    Case.step('Check QAT device,Install QAT driver and run qat sanmple code stress')
    acce.check_device_in_rich_vm(qemu, 'qat', vf_num)
