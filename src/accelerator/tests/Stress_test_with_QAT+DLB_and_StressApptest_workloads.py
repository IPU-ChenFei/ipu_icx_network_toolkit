...
def test_steps(sut, my_os):
    ...
    # Step 3 - Check QAT Status
    Case.step('Check QAT Status')
    acce.check_qat_service_status()

    # Step 4 - run and check sample code
    ...
    # Step 6 - Run QAT cpa_sample_code with parameters for 12 hrs
    ...
    # Step 8 - Run ldb_traffic example on the PF
    ...
    # Step 9 - Run StressApp for 12 hrs
    Case.step('Run StressApp for 12 hrs')
    acce.run_stressapptest_stress_async()

    # Step 10 - Run DLB workload for 12 hrs
    ...
    # Step 12 - Save StressApptest log
    Case.step('Save StressApptest log')
    sut.download_to_local(remotepath='/root/stressapptest_async_log', localpath=LOG_PATH)
    _, out, err = sut.execute_shell_cmd('cat /root/stressapptest_async_log')
    acce.check_keyword(["Status: PASS - please verify no corrected errors"], out, 'Run stressapptest stress fail')

    # Step 13 -  Check mce log
