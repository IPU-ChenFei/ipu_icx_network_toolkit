...
def test_steps(sut, my_os):
    ...
    # Step 6 - Unbind QAT per device 16 VF
    Case.step('Unbind QAT per device 16 VF')
    for vf in range(1, vf_num+1):
        acce.unbind_device(f'qat_0_{vf}')

    # Step 8 - get device list
    Case.step('get device list')
    dev_list = []
    for vf in range(1, vf_num + 1):
        qat_dev_id = acce.get_dev_id('qat', 0, vf)
        print(qat_dev_id)
        dlb_dev_id = acce.get_dlb_dev_id_list(0, 16)
        print(dlb_dev_id)
    ...
    exec_res = qemu.execute_rich_vm_cmd_parallel('./ldb_traffic -n 102400000', timeout=600, cwd=f'{DLB_DRIVER_PATH_L}libdlb/examples')
    for vm in exec_res:
        out = exec_res[vm][1]
        acce.check_keyword(['Received 102400000 events'], out, 'execute dlb stress fail')
