...
def test_steps(tcd):
    ...
    # Tool Version [0.2.11]
    #ID/LINK: https://hsdes.intel.com/appstore/article/#/15011082679
    #TITLE: QAT_VFLR_Functional_level_reset_of_virtual_function_devices
    #DOMAIN: accelerator
    ...
    if tcd.prepare("setup system for test"):
        ...
        # 2.Install Kernel Devel  rpm files.
    ...
    if tcd.step("Run cpa_sample_code."):
        ...
        sutos.execute_cmd(sut, f'cd $API_SCRIPT && python3 $API_SCRIPT/run_qat_sample_code.py -q \'signOfLife=1\'', timeout=20*60)
        ##################
        tcd.log("### Expected result ###")
        tcd.log("The sample code is run completed")
        tcd.log("with no error. the last line is \"Sample code completed successfully\"")
        tcd.log("#./cpa_sample_code signOfLife=1")
        tcd.log("....")
        tcd.log("....")
    ...
    if tcd.step("Perform Function level reset on QAT virtual function and rerun the cpa_sample_code again"):
        ...
        # for i in {8..135}; do ./adf_ctl qat_dev$i restart ; done
        
        #./cpa_sample_code signOfLife=1
        
        sutos.execute_cmd(sut, f'cd $API_SCRIPT && python3 $API_SCRIPT/qat_reset_12h.py -m \'virtual\'', timeout=13*60*60)
        ##################
        tcd.log("### Expected result ###")
        tcd.log("# for i in {8..135}; do ./adf_ctl qat_dev$i restart ; done")
        tcd.log("Restarting device qat_dev8")
        tcd.log("Processing /etc/4xxxvf_dev4.conf")
        tcd.log("Restarting device qat_dev9")
        tcd.log("Processing /etc/4xxxvf_dev5.conf")
        tcd.log("Restarting device qat_dev10")
        tcd.log("...")
        tcd.log("...")
        tcd.log("# #./cpa_sample_code signOfLife=1")
        tcd.log("....")
