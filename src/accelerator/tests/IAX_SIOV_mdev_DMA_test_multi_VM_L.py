...
def test_steps(tcd):
    ...
    # Tool Version [0.2.13]
    #ID/LINK: https://hsdes.intel.com/appstore/article/#/15010635542
    #TITLE: IAX_SIOV_mdev_DMA_test_multi_VM_L
    #DOMAIN: accelerator
    ...
    if tcd.prepare("setup system for test"):
        ...
        # EDKII Menu > Socket Configuraiton > IIO Configuration >IOAT   configuration >socket <n> IOAT configure > DSA = enabled (Notes: This is only support on SPR, not support GNR)
    ...
    if tcd.step("Linux Kernel parameters"):
        ...
        tcd.log("If kernel newer than 5.15 ,\"idxd.legacy_cdev_load=1 modprobe.blacklist=idxd_uacce\" is not need to add.")
        ##################
    ...
    if tcd.step("Login VMs and prepare test env."):
        ...
        sutos.execute_cmd(sut, f'cd $API_SCRIPT && python3 $API_SCRIPT/vm_copy.py -s "$INTERNAL_MODULE_NAME" -d "$SUT_TOOLS" -n 10', timeout=10*60)
        sutos.execute_cmd(sut, f'cd $API_SCRIPT && python3 $API_SCRIPT/vm_execute.py -c "rpm -ivhf $INTERNAL_MODULE_NAME" -n 10', timeout=20*60)
        ##################
        tcd.log("### Expected result ###")
        tcd.log("Kernel parameters in host:")
        tcd.log("intel_iommu=on,sm_on no5lvl")
        ##################
    ...
    if tcd.step("Run dmatest on VMs ."):
        ...
        # ./Guest_Mdev_Randomize_IAX_Conf.sh -k
        ...
        sutos.execute_cmd(sut, f'cd $API_SCRIPT && python3 $API_SCRIPT/vm_execute.py -c "python3 $API_SCRIPT/lnx_exec_with_check.py -c \'cd $ACCE_RANDOM_CONFIG_TEST_PATH_L && ./Guest_Mdev_Randomize_IAX_Conf.sh -k\' -m \'keyword\' -l \'enabled 1 wq(s) out of 1\'" -n 10', timeout=20*60)
        ##################
        tcd.log("### Expected result ###")
        tcd.log("dmatest runs on each virtual machine without errors.")
        tcd.log("#./Guest_Mdev_Randomize_IAX_Conf.sh -k")
        tcd.log("Configuring 1 IAX Devices")
        tcd.log("iax0; numa_node: -1; max_wq_size: 1; op_cap: 0xd 0x7f331c 0x0 0x0; gen_cap: 0x71f10001f0115; token_limit: 0; max_tokens: 0; max_batch: 1; max_transfer_size: 2147483648; ims_size: 0; cdev_major: 237; iax_clients: 0;")
        tcd.log("wq: wq0.0; Group: 0; Size: 1; Threshold: 1; Priority: 9; Type: kernel; Mode: shared; Block On Fault: 0; Name: dmaengine")
        tcd.log("Enabling work-queues:")
        tcd.log("iax0/wq0.0")
        tcd.log("[  550.415952] dmaengine wq0.0: wq wq0.0 enabled")
        ##################
        
        
    ## 8
    if tcd.step("./Guest_Mdev_Randomize_IAX_Conf.sh -i 1000 -j 10"):
        # ./Guest_Mdev_Randomize_IAX_Conf.sh -i 1000 -j 10
        
        sutos.execute_cmd(sut, f'cd $API_SCRIPT && python3 $API_SCRIPT/vm_execute.py -c "python3 $API_SCRIPT/dmatest_check.py -c \'Guest_Mdev_Randomize_IAX_Conf.sh -i 1000 -j 10\' -a IAX" -n 10', timeout=20*60)
        ##################
        tcd.log("### Expected result ###")
        tcd.log("Logs show dmatest completed successfully on each thread.")
        tcd.log("#./Guest_Mdev_Randomize_IAX_Conf.sh -i 1000 -j 10")
        tcd.log("...")
        tcd.log("...")
        tcd.log("Number of WQs: 1")
        tcd.log("Threads per WQ: 10")
        tcd.log("Total Threads: 10")
        tcd.log("[  570.236660] dmatest: Added 10 threads using dma0chan0")
        tcd.log("[  570.237258] dmatest: Started 10 threads using dma0chan0")
        tcd.log("Waiting for dmatest to complete[  570.869274] dmatest: dma0chan0-direc: summary 1000 tests, 0 failures 1746.55 iops 13991 KB/s (0)")
        tcd.log("[  570.873121] dmatest: dma0chan0-direc: summary 1000 tests, 0 failures 1733.37 iops 13690 KB/s (0)")
        tcd.log("...")
        tcd.log("...")
        tcd.log("Mar 04 08:11:07 embargo kernel: dmatest: dma0chan0-direc: summary 1000 tests, 0 failures 1678.59 iops 13845 KB/s (0)")
        tcd.log("Mar 04 08:11:07 embargo kernel: dmatest: dma0chan0-direc: summary 1000 tests, 0 failures 1670.31 iops 13344 KB/s (0)")
        tcd.log("Total Threads: 10")
