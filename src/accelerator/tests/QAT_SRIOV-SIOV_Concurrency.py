...
def test_steps(sut, my_os):
    ...
    sut.execute_shell_cmd(f'sed -i "s/NumberAdis = 0/NumberAdis = 16/g" /etc/4xxx_dev0.conf', timeout=60)
    ...
    def choose_qat_device(num):
        ...
        qat_dev_id = acce.get_dev_id('qat', 1, num)
        print(qat_dev_id)
        acce.unbind_device(f'qat_1_{num}')
        return qat_dev_id
    ...
    def execute_cmd(qemu_cmd):
        ker_ver = acce.kernel_version()
        if ker_ver <= acce.CENTOS_INTEL_NEXT_KERNEL:
            exec_cmd = f'{qemu_cmd} -name {vm_name} -machine q35 -enable-kvm -global kvm-apic.vapic=false -m 10240 -cpu host -drive format=raw,file=/home/{vm_name}.qcow2 -bios /home/OVMF.fd  -device vfio-pci,sysfsdev={dev_path} -device vfio-pci,host={qat_dev_id} -smp 16 -serial mon:stdio -fsdev local,security_model=none,id=fsdev0,path=/home -device intel-iommu,caching-mode=on,dma-drain=on,x-scalable-mode=""modern"",device-iotlb=on,aw-bits=48 -nographic -nic user,hostfwd=tcp::2222-:22'
        else:
            exec_cmd = f'{qemu_cmd} -name {vm_name} -machine q35 -enable-kvm -global kvm-apic.vapic=false -m 10240 -cpu host -drive format=raw,file=/home/{vm_name}.qcow2 -bios /home/OVMF.fd  -device vfio-pci,sysfsdev={dev_path} -device vfio-pci,host={qat_dev_id} -smp 16 -serial mon:stdio -nographic -nic user,hostfwd=tcp::2222-:22'
        return exec_cmd
    ...
    # Step 10 - Install QAT driver and run qat sanmple code stress
    ...
    # Step 11 - Install QAT driver and run qat sample code stress
    ...
    rcode, out, err = qemu.execute_vm_cmd(vm_name, f"lspci -vnd:{acce.qat_vf_id}")
    acce.check_keyword(['8086', f'{acce.qat_vf_id}'], out, 'QAT SW installed fail')
    rcode, out, err = qemu.execute_vm_cmd(vm_name, 'adf_ctl status')
    acce.check_keyword(['state: up'], out, 'qat_dev0 down')
    acce.run_qat_sample_code_vm(qemu, vm_name, 'runTests=1 signOfLife=1')
