...
def test_steps(sut, my_os):
    ...
    # Step 5 - Unbind each dlb 16 vf
    Case.step('Unbind each dlb 16 vf')
    ...
    for pf in range(pf_num):
        for vf in range(1, vf_num+1):
            dlb_dev_id_list = acce.get_dlb_dev_id_list(pf, 16)
            dev_id_list.append(dlb_dev_id_list[vf-1])
    print(dev_id_list)
    ...
    acce.check_device_in_rich_vm(qemu, 'dlb', 16)
    ...
    exec_res = qemu.execute_rich_vm_cmd_parallel('./ldb_traffic -n 1024', timeout=60 * 10, cwd=f'{DLB_DRIVER_PATH_L}libdlb/examples')
    for vm in exec_res:
        out = exec_res[vm][1]
