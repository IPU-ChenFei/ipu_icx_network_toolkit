...
def test_steps(sut, my_os):
    ...
    get_qat_dev_id = []
    get_qat_dev_id.append(get_qat_dev_id_00)
    get_qat_dev_id.append(get_qat_dev_id_10)
    print(get_qat_dev_id)
    ...
    sym_uuid1 = get_device_uuid(out)
    print(sym_uuid1)
    _, out, err = sut.execute_shell_cmd(f'./build/vqat_ctl create {get_qat_dev_id_10} sym', timeout=60, cwd=QAT_DRIVER_PATH_L)
    acce.check_keyword(['VQAT-sym created successfully'], out, 'VQAT-sym create fail')
    sym_uuid2 = get_device_uuid(out)
    print(sym_uuid2)
    _, out, err = sut.execute_shell_cmd('./build/vqat_ctl show', timeout=60, cwd=QAT_DRIVER_PATH_L)
    sym_num1 = 0
    sym_num2 = 0
    ...
    for line in line_list:
        ...
        if str_list[0] == f'{get_qat_dev_id_00}':
            sym_name1 = str_list[1].strip().split(':')
            sym_num1 = int(sym_name1[1].strip())
        if str_list[0] == f'{get_qat_dev_id_10}':
            sym_name2 = str_list[1].strip().split(':')
            sym_num2 = int(sym_name2[1].strip())
    if sym_num1 != 7 and sym_num2 != 7:
    ...
    qemu.create_rich_vm(2, file_name, add_by_host=False, is_sriov=False)
    qemu.attach_acce_dev_to_vm_grouply(get_qat_dev_id, 1)
    qemu.start_rich_vm()
    ...
    exec_res = qemu.execute_rich_vm_cmd_parallel("lspci -v -d 8086:0da5")
    for vm in exec_res:
        out = exec_res[vm][1]
        acce.check_keyword(['Device 0000'], out, 'QAT SW installed fail')
    acce.check_device_in_rich_vm(qemu, 'qat', 0, mdev=True)
    acce.qat_install_rich_vm(qemu, './configure --enable-icp-sriov=guest', enable_siov=True)

    # Step 10 - Install QAT driver and run qat sample code stress
    Case.step('Install QAT driver and run qat sample code stress')
    exec_res = qemu.execute_rich_vm_cmd_parallel('./build/adf_ctl qat_dev0 up', cwd=f'{QAT_DRIVER_PATH_L}')
    for vm in exec_res:
        out = exec_res[vm][1]
        acce.check_keyword(['Starting device qat_dev0'], out, 'qat_dev0 down')
    exec_res = qemu.execute_rich_vm_cmd_parallel('adf_ctl status')
    for vm in exec_res:
        out = exec_res[vm][1]
        acce.check_keyword(['state: up'], out, 'qat_dev0 down')
    for i in range(6):
        acce.run_qat_sample_code_rich_vm(qemu, f'signOfLife=1 runTests={2**i}')
