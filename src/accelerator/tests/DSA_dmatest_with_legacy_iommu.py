...
def test_steps(sut, my_os):
    ...
    if ker_ver <= acce.CENTOS_INTEL_NEXT_KERNEL:
        acce.modify_kernel_grub("intel_iommu=on,sm_off idxd.legacy_cdev_load=1 modprobe.blacklist=idxd_uacce", True)
    else:
    ...
    if ker_ver <= acce.CENTOS_INTEL_NEXT_KERNEL:
        ...
        acce.modify_kernel_grub("intel_iommu=on,sm_off idxd.legacy_cdev_load=1 modprobe.blacklist=idxd_uacce", False)
    else:
