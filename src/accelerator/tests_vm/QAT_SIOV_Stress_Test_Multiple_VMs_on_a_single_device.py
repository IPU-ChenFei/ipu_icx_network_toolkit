...
def test_steps(tcd):
    ...
    if tcd.prepare("setup system for test"):
        ...
        sutos.execute_cmd(sut, f'cd $API_SCRIPT && python3 $API_SCRIPT/vm_img_copy.py -d \'qat\' -s \'32\' -g \'2\' -p \'1\' -f $CENTOS_IMG_NAME', timeout=30*60)
    ...
    if tcd.step("Verify sIOV enabled on device"):
        ...
        sutos.execute_cmd(sut, f'cd $API_SCRIPT && python3 $API_SCRIPT/resource_config_login.py -d \'qat\' -s \'32\' -g \'2\' -p \'1\' -m \'sym\'', timeout=10*60)
