...
def test_steps(tcd):
    ...
    # Tool Version [0.2.11]
    #ID/LINK: https://hsdes.intel.com/appstore/article/#/15011258185
    #TITLE: DLB_SIOV_Stress_Test_Multiple_VMs_per_device
    #DOMAIN: accelerator
    ...
    if tcd.prepare("setup system for test"):
        ...
        sutos.execute_cmd(sut, f'cd $API_SCRIPT && python3 $API_SCRIPT/vm_img_copy.py -d \'dlb\' -s \'16\' -g \'2\' -p \'1\' -f $CENTOS_IMG_NAME', timeout=60*60)
    ...
    if tcd.step("Start VM using QEMU:"):
        # Start VM using QEMU:
        # Install qemu for 5.15 kernel first
        # dnf update --allowerasing --nobest
        # dnf install qemu-kvm -y --allowerasing
        
        # /usr/libexec/qemu-kvm -name guestVM1 -machine q35 -enable-kvm -global kvm-apic.vapic=false -m 4096 -cpu host -drive format=raw,file=/home/spr-bkc-pc-centos-stream-8-coreserver-6.13.0.img -bios /home/OVMF.fd -smp 8 -nic user,hostfwd=tcp::2202-:22 -nographic
        
        sutos.execute_cmd(sut, f'dnf update --allowerasing --nobest -y', timeout=60*60)
        sutos.execute_cmd(sut, f'dnf install qemu-kvm -y --allowerasing', timeout=20*60)
        sutos.execute_cmd(sut, f'pip3 install setuptools_rust paramiko scp', timeout=10*60)
        sutos.execute_cmd(sut, f'cd $API_SCRIPT && python3 $API_SCRIPT/resource_config_login.py -d \'dlb\' -s \'16\' -g \'2\' -p \'1\'', timeout=60*60)
    ...
    if tcd.step("Set password and Copy the DLB and DPDK package and kernel rpms inside the VM"):
        ...
        ##################
        tcd.log("### Expected result ###")
        tcd.log("Driver install successfully")
        ### Notes ###
        # If you are use intel next kernel , please rmmod dlb2 driver first .
        # #rmmod dlb2
        ##################
    ...
    ## 6
    if tcd.step("Launch Multiple VMs. The max MDEVs that can be created on single DLB device is 16."):
        # Launch Multiple VMs. The max MDEVs that can be created on single DLB device is 16.
        # on a 2S XCC system we have 8 DLB devices available, 8 devices * 16 MDEVs per device = 128 MDEVs
        # Try to crate maximum possible number of VMs to utilize all the MDEVs that can be created on all the DLB devices available.
        
        # E.g. Create 128 VMs with 1 DLB MDEVs each (128 VMs * 1 MDEVs per VM = 128 MDEVs)
        # or
        # Create 64 VMs with 2 DLB MDEVs each (64 VMs * 2 MDEVs per VM = 128 MDEVs)
        ...
        # ./Accelerators_SIOV_Multi_VM.sh -n 16 -a -H -m 1 -c 8 -i gva
        ##################
        tcd.log("### Expected result ###")
        ...
        tcd.log("# ./Accelerators_SIOV_Multi_VM.sh -n 16 -a -H -m 1 -c 8 -i gva")
        tcd.log("...")
        tcd.log("...")
        ...
        # Command to launch 16 VMs per each device and 1 MDEVs per VM(on 2S : 8 devices * 16 VMs per device = 128VMs)  with DLB MDEVs
        # # ./Accelerators_SIOV_Multi_VM.sh -n 16 -a -H -m 1 -c 8 -i gva
