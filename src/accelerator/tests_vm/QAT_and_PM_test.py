...
CASE_DESC = [
    ...
    'warm reset while run cpa_sample_code',
    'cold reset while run cpa_sample_code',
    'graceful shutdown while run cpa_sample_code',
    'forceful shutdown while run cpa_sample_code',
    'g3 cycling while run cpa_sample_code'
]
...
def test_steps(sut, my_os):
    ...
    # Step 2 - warm reset while run cpa_sample_code and check if error occurred
    Case.step('warm reset while run cpa_sample_code and check if error occurred')
    for i in range(1):
        if i < 1:
    ...
    # Step 3 - cold reset while run cpa_sample_code and check if error occurred
    Case.step('cold reset while run cpa_sample_code and check if error occurred')
    for i in range(1):
        if i < 1:
            acce.check_qat_service_status()
            acce.run_qat_sample_code('')
    ...
    # Step 4 - graceful shutdown while run cpa_sample_code and check if error occurred
    Case.step('graceful shutdown while run cpa_sample_code and check if error occurred')
    for i in range(1):
        if i < 1:
            acce.check_qat_service_status()
            acce.run_qat_sample_code('')
            sut.execute_shell_cmd_async('./cpa_sample_code', timeout=10*60, cwd=f'{QAT_DRIVER_PATH_L}/build/')
            time.sleep(20)
            my_os.s5_cycle_step(sut, gracefully=True)
            time.sleep(60)
            sut.execute_shell_cmd('rm -rf /root/abrt.log', timeout=60)
            sut.execute_shell_cmd('abrt-cli list | tee /root/abrt.log', timeout=60)
            sut.download_to_local(remotepath='/root/abrt.log', localpath=os.path.join(LOG_PATH, 'Logs'))
    ...
    # Step 5 - forceful shutdown while run cpa_sample_code and check if error occurred
    Case.step('forceful shutdown while run cpa_sample_code and check if error occurred')
    for i in range(1):
        if i < 1:
            acce.check_qat_service_status()
            acce.run_qat_sample_code('')
            sut.execute_shell_cmd_async('./cpa_sample_code', timeout=10*60, cwd=f'{QAT_DRIVER_PATH_L}/build/')
            time.sleep(20)
            my_os.s5_cycle_step(sut, gracefully=False)
            time.sleep(60)
            sut.execute_shell_cmd('rm -rf /root/abrt.log', timeout=60)
            sut.execute_shell_cmd('abrt-cli list | tee /root/abrt.log', timeout=60)
            sut.download_to_local(remotepath='/root/abrt.log', localpath=os.path.join(LOG_PATH, 'Logs'))
    ...
    # Step 6 - g3 cycling while run cpa_sample_code and check if error occurred
    Case.step('g3 cycling while run cpa_sample_code and check if error occurred')
    for i in range(1):
        if i < 1:
            acce.check_qat_service_status()
            acce.run_qat_sample_code('')
...
def test_main():
    ...
    # --help to see all allowed parameters    user_parameters = ParameterParser.parse_embeded_parameters()
