...
def test_steps(tcd):
    ...
    # Tool Version [0.2.11]
    #ID/LINK: https://hsdes.intel.com/appstore/article/#/15011258202
    #TITLE: QAT_SIOV_Stress_Test_Multiple_VMs_per_device
    #DOMAIN: accelerator
    ...
    if tcd.prepare("setup system for test"):
        ...
        sutos.execute_cmd(sut, f'cd $API_SCRIPT && python3 $API_SCRIPT/vm_img_copy.py -d \'qat\' -s \'64\' -g \'8\' -p \'1\' -f $CENTOS_IMG_NAME', timeout=10*60)
    ...
    if tcd.step("Note: If you want to enable sIOV, rather than SR-IOV enabled by default. You need modify PF configure files under /etc/. Detail steps are list below."):
        ...
        # vim /etc/4xxx_dev2.conf
        # vim /etc/4xxx_dev3.conf
        # vim /etc/4xxx_dev4.conf
        # vim /etc/4xxx_dev5.conf
        # vim /etc/4xxx_dev6.conf
        # vim /etc/4xxx_dev7.conf
    ...
    if tcd.step("Verify sIOV enabled on device"):
        ...
        sutos.execute_cmd(sut, f'cd $API_SCRIPT && python3 $API_SCRIPT/resource_config_login.py -d \'qat\' -s \'64\' -g \'8\'  -p \'1\' -m \'sym\'', timeout=10*60)
        ...
        tcd.log("VQAT-sym created successfully, device name = e7adb0f7-7ab9-4b72-aa77-3038aae5780b")
        ...
        tcd.log("1syme7adb0f7-7ab9-4b72-aa77-3038aae5780bactive")
        tcd.log("--------------------------------------------------------------")
        tcd.log("BDF: 0000:70:00.0")
        tcd.log("Available sym : 64")
        tcd.log("Available asym : 0")
        tcd.log("Available dc : 0")
        tcd.log("Active VQATs:")
        tcd.log("...")
    ...
    if tcd.step("Set password and copy the QAT package inside the VM"):
        ...
        # > bye
        # mv qat20.l.0.9.0-00023_1.zip /root/QAT
    ...
    if tcd.step("Install QAT driver on Guest"):
        ...
        sutos.execute_cmd(sut, f'cd $API_SCRIPT && python3 $API_SCRIPT/vm_execute.py -c "cd $API_SCRIPT && python3 $API_SCRIPT/qat_install.py -w \'guest\'"', timeout=60*60)
    ...
    if tcd.step("Repeat the Step 5 and 6 to launch Multiple VMs. The max MDEVs that can be created on single QAT device is 64."):
        ...
        # on a 2S XCC system we have 8 QAT devices available, 8 devices * 64 MDEVs per device =  512 MDEVs
        
        # Try to crate maximum possible number of VMs to utilize all the MDEVs that can be created on all the QAT devices available.
        
        # E.g. Create 128 VMs with 4 QAT MDEVs each (128 VMs * 4 MDEVs per VM = 512 MDEVs)
        # or
        # Create 64 VMs with 8 QAT MDEVs each (64 VMs * 8 MDEVs per VM = 512 MDEVs)
        ...
        # ./Accelerators_SIOV_Multi_VM.sh -n 16 -a -Q sym -m 4 -c 8 -i gva
        ...
        tcd.log("# ./Accelerators_SIOV_Multi_VM.sh -n 1 -a -Q sym -m 1 -c 8 -i gva")
        ...
        # Command to launch 16 VMs per each device and 4 MDEVs per VM(on 2S : 8 devices * 16 VMs per device = 128VMs)  with QAT MDEVs
        # # ./Accelerators_SIOV_Multi_VM.sh -n 16 -a -Q sym -m 4 -c 8 -i gva
    ...
    ## 9
    if tcd.step("Open new terminals and run QAT workload on all VMs Concurrently which uses SIOV Virtual device interface"):
        # Open new terminals and run QAT workload on all VMs Concurrently which uses SIOV Virtual device interface
