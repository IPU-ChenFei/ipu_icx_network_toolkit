...
class RdtMonitorLlcGroupStress(content_base_test_case.ContentBaseTestCase):
    """
    HPQC ID : H79571-PI_RDT_A_MonitorLLC_MemBW_CoreGroup_stress_L, H55277-PI_RDT_A_MonitorLLC_MemBW_CoreGroup_stress

    This test case aims to install RDT if it not installed and Check the RDT monitoring functions for all cores in a
    core group with pqos command when stress is running on these cores.
    """

    TEST_CASE_ID = ["H79571","PI_RDT_A_MonitorLLC_MemBW_CoreGroup_stress_L",
                    "H55277", "PI_RDT_A_MonitorLLC_MemBW_CoreGroup_stress"]
    CORE = "12-23"
    RDT_MONITOR_CMD = "pqos -m 'all:[{}]' -t 10".format(CORE)
    TASKSET_CMD = "taskset -c {} stress -m 100 -c 100".format(CORE)
    step_data_dict = {
        ...
        3: {'step_details': 'Turn on RDT monitoring for CPU core {}'.format(CORE),
            'expected_results': 'RDT monitoring is turned on successfully and collected data is collected'},
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create an instance of RdtMonitorLlcGroupStress

        :param cfg_opts: Configuration Object of provider
        :param test_log: Log object
        :param arguments: None
        """
        super(RdtMonitorLlcGroupStress, self).__init__(test_log, arguments, cfg_opts)
    ...
    def prepare(self):  # type: () -> None
        ...
        super(RdtMonitorLlcGroupStress, self).prepare()
        self._install_collateral.install_stress_tool_to_sut()

    def execute_rdt_monitor_llc_membw_core_group_stress(self):
        """
        This method executes the below:
        1. Verify if RDT is installed, If not it will install
        2. Restore default monitoring: pqos -R
        3. Turn on RDT monitoring for all events on selected group of cores: pqos -m 'all: [12-23] '
        4. Start memory and cpu load process: taskset -c 12-23 stress -m 100 -c 100
        5. Turn on RDT monitoring for all events on selected group of cores: pqos -m 'all: [12-23] '
            and compare the event values are increased
        6. Turn the stress off
        7. Turn on RDT monitoring for all events on selected group of cores: pqos -m 'all: [12-23] '
        ...
        # Turn on RDT monitoring for all events on selected group of cores: pqos -m 'all: [12-23] '
        ...
        # Start memory and cpu load process: taskset -c 12-23 stress -m 100 -c 100
        self.os.execute_async(self.TASKSET_CMD)
        self._test_content_logger.end_step_logger(4, return_val=True)

        self._test_content_logger.start_step_logger(5)
        # Turn on RDT monitoring for all events on selected group of cores and compare the event values are increased
        ...
        # Turn on RDT monitoring for all events on selected group of cores and compare the event values are decreased
    ...
    def execute(self):
        ...
        for index in range(self._rdt.ITERATION):
            try:
                ...
                self.execute_rdt_monitor_llc_membw_core_group_stress()
    ...
    def cleanup(self, return_status):
        ...
        if self._rdt.is_stress_running():
            self._rdt.stop_stress_tool()
        super(RdtMonitorLlcGroupStress, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if RdtMonitorLlcGroupStress.main() else Framework.TEST_RESULT_FAIL)
