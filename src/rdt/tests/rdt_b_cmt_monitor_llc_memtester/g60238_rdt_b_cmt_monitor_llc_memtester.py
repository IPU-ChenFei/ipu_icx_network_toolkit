...
class RdtCmtMonitorLlcMemtester(content_base_test_case.ContentBaseTestCase):
    """
    Glasgow ID : G60238.1 - PI_RDT_B_CMTMonitor_LLC_memtester

    This test case aims to install RDT if it not installed and also installs memtester tool and
    verify that CMT Monitor LLC occupancy for cores/tasks via pqos command when memtester is running
    """
    TEST_CASE_ID = ["G60238.1-PI_RDT_B_CMTMonitor_LLC_memtester"]
    CORE = 4
    MEMTESTER_CMD = "taskset -c {} memtester 100M".format(CORE)
    CMT_MONITOR_CMD = "pqos -m llc:0-23 -t 30"
    CMT_CAP_COMMAND = "pqos -d"
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create an instance of RdtCmtMonitorLlcMemtester

        :param cfg_opts: Configuration Object of provider
        :param test_log: Log object
        :param arguments: None
        """
        super(RdtCmtMonitorLlcMemtester, self).__init__(test_log, arguments, cfg_opts)
    ...
    def prepare(self):  # type: () -> None
        ...
        super(RdtCmtMonitorLlcMemtester, self).prepare()
    ...
    def execute(self):
        ...
        5. Turn on RDT monitoring for CMT events on all core: pqos -m llc:0-23 -t 30 and
            check if values are high for core 4
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if RdtCmtMonitorLlcMemtester.main() else Framework.TEST_RESULT_FAIL)
