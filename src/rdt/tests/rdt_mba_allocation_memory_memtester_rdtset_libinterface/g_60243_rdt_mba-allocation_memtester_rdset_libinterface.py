...
class RdtMbaAllocationMemoryMemTesterRdtsetLibInterface(content_base_test_case.ContentBaseTestCase):
    """
    Glasgow ID : G60243.0 - PI_RDT_B_MBAAllocation_Memory_memtester_rdtset_LibInterface
    This test case verifies MBA Set COS definition command and verify MBA via rdtset OS
    interface when memtester is running
    """
    TEST_CASE_ID = ["G60243.0 - PI_RDT_B_MBAAllocation_Memory_memtester_rdtset_LibInterface"]
    RDT_SET_CMD = 'rdtset -I -t "mba={};cpu=5-6" -c 5-6 memtester 10M'
    MBA_RATE_VALUE = "50"
    MBA_INVALID_RATE_VALUE = "200"
    STEP_DATA_DICT = \
        {
            1: {'step_details': 'Restore default Interface allocation',
                'expected_results': 'Restore RDT to default monitoring is successful'},
            2: {'step_details': 'Run command with provided MBA rate ',
                'expected_results': 'Observed 50% availability for the Required COS in pqos output'},
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create an instance of RdtMbaAllocationMemoryMemTesterRdtsetLibInterface

        :param cfg_opts: Configuration Object of provider
        :param test_log: Log object
        :param arguments: None
        """
        super(RdtMbaAllocationMemoryMemTesterRdtsetLibInterface, self).__init__(test_log, arguments, cfg_opts)
    ...
    def prepare(self):
        ...
        """
        super(RdtMbaAllocationMemoryMemTesterRdtsetLibInterface, self).prepare()
    ...
    def execute(self):
        ...
        1. Restore default Interface allocation
        ...
        self._rdt.restore_default_rdt_monitor(self._rdt.INTERFACE_CMD)
        self._test_content_logger.end_step_logger(1, return_val=True)

        self._test_content_logger.start_step_logger(2)
        self.os.execute_async(self.RDT_SET_CMD.format(self.MBA_RATE_VALUE))
        command_result = self._common_content_lib.execute_sut_cmd(self._rdt.CHECK_L3CA_MBA_CMD,
                                                                  self._rdt.CHECK_L3CA_MBA_CMD,
                                                                  self._command_timeout)
        search = re.search(self._rdt.PQOS_SUCCESSFULL_INFO.format(self.MBA_RATE_VALUE), command_result).group()
        if not search:
            raise content_exceptions.TestFail("Failed to Observe Pqos Output")
        self._log.info("Successfully Observed the Pqos Output {}".format(search))
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if RdtMbaAllocationMemoryMemTesterRdtsetLibInterface.main() else Framework.TEST_RESULT_FAIL)
