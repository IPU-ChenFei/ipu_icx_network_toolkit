...
class case_execute:
    def case10(self, method, core1, cos1, cycle=1):
        sys_info().show_sys_info()
        TESTMBA2().reset()
        TESTMBA2().refresh()
        stress_tools().kill_tool()
        resetmsrCommand = "wrmsr 0xC84 2"
        ...
        if method == "random":
            for i in range(0, cycle):
                ...
                for k in range(100, 0, -10):
                        mbm = TESTMBA2().case10_MinMax_SameCOS(command1, command2, randcore1, randcos1, randcore2, randcos2, k)
                        mbmmax1 = float(mbm[0])
                        mbmmin1 = float(mbm[1])
                        mbmmax2 = float(mbm[2])
                        mbmmin2 = float(mbm[3])
                ...
                print("\n")
                print("{:>12}\t{:>12}\t{:>12}\t{:>12}\t{:>12}\t{:>12}\t{:>12}\t{:>12}\t{:>12}\t{:>12}\t{:>12}\t{:>12}\t".format("Traffic","Core1","COS1","MBA1","MBMMax1","Percentage1<105","Status1","Core2","COS2","MBMMax2","Percentage2<105","Status2"))
...
logname = r"{}/log/Case10_result_".format(cpath)+gettime+".log"
if __name__ == '__main__':
    sys.stdout = Logger(logname, sys.stdout)
    case_execute().case10(method, core1, cos1, cycle)
