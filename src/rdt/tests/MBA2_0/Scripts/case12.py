...
class case_execute:
    def case12(self, method, core, cos, cycle=1):
        ...
        if method == "iteration":
            ...
            for i in range(0, cycle):
                ...
                traffic_info = TrafficType().remote_traffic(randcore)
                command = traffic_info[1]
                traffic = traffic_info[0]
                randcos = random.randint(1,cos-1)
                for k in range(100, 0, -10):
                    mbm = TESTMBA2().case12_MBR(command, randcore, randcos, k)
                    mbl = float(mbm[0])
                    mbr = float(mbm[1])
                    mblresult.append(mbl)
                    mbrresult.append(mbr)
                ...
                for key, value, tf,ma,cores, coss, statuss in zip(mblresult,mbrresult,trafficlist,mbalist,corelist,coslist,statuslist):
                    print ("{:>12}\t {:>12}\t{:>12}\t {:>12}\t{:>12}\t{:>12}\t{:>12}\t".format(cores,coss,tf,ma,key,value,statuss))
        elif method == "custom":
                TESTMBA2().reset()
                mbm = TESTMBA2().case12_MBR(core, cos)
                mbl = float(mbm[0])
                mbr = float(mbm[1])
                if mbl <= 100:
                    status = "Pass"
...
logname = r"{}/log/Case012_result_".format(cpath)+gettime+".log"
if __name__ == '__main__':
    sys.stdout = Logger(logname, sys.stdout)
    case_execute().case12(method, core, cos, cycle)
