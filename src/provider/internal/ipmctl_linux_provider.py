...
class IpmctlProviderLinux(IpmctlProvider):
    ROOT = "/root"
    ...
    NDCTL_CMD_TO_CREATE_NAMESPACE_BLOCK_MODE = "ndctl create-namespace --mode sector --region={}"
    ...
    IPMCTL_CMD_FOR_CLEAR_MEDIA_ERROR = r" set -dimm {} Clear=1 Poison='{}'"
    ...
    NDCTL_CMD_TO_CREATE_NAMESPACE_WITH_DAX_MODE = "ndctl create-namespace --mode=fsdax --region={}"

    def __init__(self, log, os_obj, uefi_util_obj=None, cfg_opts=None):
        super(IpmctlProviderLinux, self).__init__(log, os_obj, uefi_util_obj, cfg_opts)
        self._log = log
        self._os = os_obj
        self.ipmctl_cmd_name = "ipmctl"
        self.ipmctl_path = "/root"
    ...
    def dcpmm_new_pmem_disk(self, region_data_list, mode=None):
        ...
        :return: the created Device Namespaces data
        """
        create_namespace_list = []
        create_namespace = None
        for index in range(0, len(region_data_list)):
            if mode == CRFileSystems.BLOCK:
                create_namespace = self.common_content_lib_obj.execute_sut_cmd(
                    self.NDCTL_CMD_TO_CREATE_NAMESPACE_BLOCK_MODE.format(region_data_list[index]),
                    "Creating Namespace of {}".format(region_data_list[index]), self.command_timeout)
            elif mode == CRFileSystems.DAX:
                create_namespace = self.common_content_lib_obj.execute_sut_cmd(
                    self.NDCTL_CMD_TO_CREATE_NAMESPACE_WITH_DAX_MODE.format(region_data_list[index]),
                    "Creating Namespace of {}".format(region_data_list[index]), self.command_timeout)
            self._log.info("Successfully created the Namespaces\n{}".format(create_namespace))
            create_namespace_list.append(create_namespace)
        return create_namespace_list
    ...
    def dcpmm_get_disk_namespace(self):
        ...
        dcpmm_disk_namespace = self.common_content_lib_obj.execute_sut_cmd(
            self.NDCTL_CMD_GET_NAMESPACE, "DCPMM disk namespace", self.command_timeout)

        if dcpmm_disk_namespace != '':
            dcpmm_disk_namespace = ast.literal_eval(dcpmm_disk_namespace)
    ...
    def dcpmm_check_disk_namespace_initilize(self, namespace_info):
        ...
        if namespace_info != '':
            initialize_dimm_result = self._os.execute(self.NDCTL_CMD_REMOVE_NAMESPACE, self.command_timeout)
            self._log.debug("command {} : output is : {}".format(self.NDCTL_CMD_REMOVE_NAMESPACE,
                                                                 initialize_dimm_result.stdout))
            self._log.debug("command {} : std error is : {}".format(self.NDCTL_CMD_REMOVE_NAMESPACE,
                                                                 initialize_dimm_result.stderr))
            if initialize_dimm_result.cmd_failed():
                ...
                initialize_dimm = self.common_content_lib_obj.execute_sut_cmd(
                    self.NDCTL_CMD_REMOVE_NAMESPACE, "DCPMM disk namespace Initialize", self.command_timeout)
    ...
    def get_pmem_device(self):
        ...
        raise NotImplementedError("The function '{}' is supported only from "
                                  "UEFI shell..".format("get_pmem_device"))
    ...
    def get_memory_dimm_information(self):
        ...
        try:
            ...
            ipmctl_show_dimm = self.common_content_lib_obj.execute_sut_cmd(
                self.ipmctl_cmd_name + self.IPMCTL_CMD_SHOW_DIMM, "DCPMM info", self.command_timeout, self.ipmctl_path)

            show_dimm_command_output = ipmctl_show_dimm.split(",")[0]
            self._log.debug("SHOW DIMM data : \n {} ".format(show_dimm_command_output))
            ...
            self._log.info("Health DIMM  list : \n{}".format(self.dimm_healthy_list))
            self.dimm_healthy_and_manageable_list = memory_info_obj.get_dimm_info_healthy_manageable()
            self._log.info("healthy and Manageable DIMM list : \n{}".format(self.dimm_healthy_and_manageable_list))
    ...
    def get_dcpmm_disk_list(self):
        ...
        """
        pmem_block_info = self.common_content_lib_obj.execute_sut_cmd(
            self.SHOW_BLOCK_INFO_CMD, "Display pmem block devices present in the OS", self.command_timeout)
        self._log.info(
    ...
    def dcpmm_get_pmem_unused_region(self):
        ...
        """
        region_data_list = []
        get_region_data = self.common_content_lib_obj.execute_sut_cmd(self.NDCTL_CMD_TO_LIST_ALL_REGION,
    ...
    def dcpmm_configuration(self, cmd=None, cmd_str=None):
        ...
        dcpmm_disk_create_mixed_mode = self.common_content_lib_obj.execute_sut_cmd(
            cmd, "DCPMM mode allocation", self.command_timeout, self.ipmctl_path)
        self._log.info("DCPMM disk has been created with {}..\n{}".format(cmd_str, dcpmm_disk_create_mixed_mode))

        # The memory allocation goal has been done.
        dcpmm_disk_goal = self.common_content_lib_obj.execute_sut_cmd(
            self.ipmctl_cmd_name + self.IPMCTL_CMD_SHOW_GOAL, "DCPMM goal", self.command_timeout, self.ipmctl_path)
    ...
    def dcpmm_get_pmem_device_fl_info(self):
        ...
        """
        raise NotImplementedError("The function '{}' is supported only from "
                                  "UEFI shell..".format("dcpmm_get_pmem_device_fl_info"))
    ...
    def get_system_capability(self):
        ...
        ipmctl_system_cap = self.common_content_lib_obj.execute_sut_cmd(
            "ipmctl show -system -capabilities", "Get system capability", self.command_timeout)
    ...
    def delete_pcd_data(self):
        ...
        :return: None
        """
        self._log.info("Executing the command : {}".format(self.ipmctl_cmd_name + self.DELETE_PCD_COMMAND))
        cmd_res = self.common_content_lib_obj.execute_sut_cmd(
            self.ipmctl_cmd_name + self.DELETE_PCD_COMMAND, "To delete the pcd data", self.command_timeout, self.ROOT)
        self._log.debug(cmd_res)
    ...
    def umount_existing_devices(self):
        ...
        self._log.info("Existing mount points : {}".format(existing_mount_points))
        if existing_mount_points:
            for index in existing_mount_points:
                find_mount_result = self._os.execute("findmnt | grep {}".format(index), self.command_timeout)
                if find_mount_result.stdout:
                    self.common_content_lib_obj.execute_sut_cmd(
                        "umount {}".format(index), "unmounting device", self.command_timeout)
    ...
    def get_existing_mount_points_fstab(self):
        ...
        """
        existing_drive_list = []
        cmd_opt = self.common_content_lib_obj.execute_sut_cmd("cat /etc/fstab", "Get fstab data", self.command_timeout)
    ...
    def remove_fstab_data(self):
        ...
        self.common_content_lib_obj.execute_sut_cmd(
            ...
            "Deleting mount data from fstab", self.command_timeout, cmd_path=self.ROOT)
        self.common_content_lib_obj.execute_sut_cmd("rm -f /etc/fstab.bak", "deleting backup file",
                                                    self.command_timeout)
    ...
    def show_dimm_media_error(self):
        ...
        dimm_media_result = self._os.execute(cmd, self.command_timeout, self.ipmctl_path)
    ...
    def dimm_diagnostic_config_check(self):
        ...
        config_val = self.common_content_lib_obj.execute_sut_cmd(
            ...
            self.command_timeout, cmd_path=self.ipmctl_path)
    ...
    def dcpmm_get_app_direct_mode_settings(self):
        ...
        """
        raise NotImplementedError("The function '{}' is supported only from "
                                  "UEFI shell..".format("dcpmm_get_app_direct_mode_settings"))
    ...
    def clear_inject_media_error(self, dimm, addr):
        """
        This method is to clear the injected media error
        ...
        """
        try:
            result = self.common_content_lib_obj.execute_sut_cmd(
                self.ipmctl_cmd_name + self.IPMCTL_CMD_FOR_CLEAR_MEDIA_ERROR.format(dimm, str(addr)),
                "DCPMM clear injected poison", self.command_timeout)
            self._log.debug("clear inject media error command result: {}".format(result))
        except Exception as ex:
            log_error = "Unable to clear injected poison due to Exception {}".format(ex)
            self._log.error(log_error)
            raise RuntimeError(log_error)
    ...
    def show_dimm_pcd(self):
        ...
        :return: pcd_info
        """
        self._log.info("Executing the command : {}".format(self.ipmctl_cmd_name + self.IPMCTL_CMD_FOR_SHOW_PCD))
        pcd_info = self.common_content_lib_obj.execute_sut_cmd(
            ...
            self.command_timeout, self.ipmctl_path)
        if "segmentation" in pcd_info:
            raise RuntimeError("Segmentation fault in PCD Data")
    ...
    def verify_lm_provisioning_configuration(self, dcpmm_disk_goal, mode):
        ...
        verify_provisioning_status = self._memory_common_lib.verify_lm_provisioning_configuration_linux(
            dcpmm_disk_goal, system_memory_data, mode)
        return verify_provisioning_status
    ...
    def verify_pmem_device_presence_cap(self, present_namespace_list, appdirect_percent=None):
        ...
        """
        pmem_device_list = self._os.execute("ls -l /dev/pmem*", self.command_timeout)
        ...
        result = [index for index in range(0, len(present_namespace_list))
                  if present_namespace_list[index]['blockdev'] in pmem_device_list]
        if not result:
            self._log.info("Failed to fetch and verify the pmem devices..")
