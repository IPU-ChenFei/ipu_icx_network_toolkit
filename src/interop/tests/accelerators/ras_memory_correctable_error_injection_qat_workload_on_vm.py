...
class RasMemorycorrectableErrorInjectionQatWorkloadOnVm(HqmBaseTest, QatBaseTest):
    ...
    TEST_CASE_ID = ["16013249599-Ras_Memory_correctable_Error_Injection_Qat_Workload_On_Vm"]
    ...
    step_data_dict = {1: {'step_details': 'Make necessary settings in Bios option.',
                      ...
                      2: {'step_details': 'Install QAT in VM',
                          'expected_results': 'QAT to be installed successfully in VM'},
                      3: {'step_details': 'Run QAT Workload in VM',
                          'expected_results': 'QAT Workload ran successfully'},
                      4: {'step_details': 'Run fisher in VM with QAT workload to inject errors.',
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        super(RasMemorycorrectableErrorInjectionQatWorkloadOnVm, self).__init__(test_log, arguments, cfg_opts, self.acc_bios_knobs)
    ...
    def prepare(self):
        ...
        super(RasMemorycorrectableErrorInjectionQatWorkloadOnVm, self).prepare()
    ...
    def execute(self):
        ...
        qat_device = self.device_string.format(qat_uuid)

        self.devices_list.append(qat_device)
        ...
        #Install QAT tools on VM
        if (self.qat_enable_status.lower() == 'true'):
            self._common_lib_acc.install_qat_on_vm(self.port_num, configure_spr_cmd='./configure --enable-icp-sriov=guest')

        self._test_content_logger.end_step_logger(2, True)

        self._log.info("Run QAT Workload in VM")
        self._test_content_logger.start_step_logger(3)
        self._common_lib_acc.execute_cpa_sample(self.port_num, './cpa_sample_code runTests=1 signOfLife=1', self._run_time)
        self._test_content_logger.end_step_logger(3, True)

        self._log.info("Run fisher in VM with QAT workload to inject errors")
        self._test_content_logger.start_step_logger(4)
        self._common_lib_acc.run_fisher_tool_on_vm(self.port_num, type_of_error="correctable", workload="QAT")
    ...
    def cleanup(self, return_status):
        ...
        super(RasMemorycorrectableErrorInjectionQatWorkloadOnVm, self).cleanup(return_status)

if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if RasMemorycorrectableErrorInjectionQatWorkloadOnVm.main() else Framework.TEST_RESULT_FAIL)
