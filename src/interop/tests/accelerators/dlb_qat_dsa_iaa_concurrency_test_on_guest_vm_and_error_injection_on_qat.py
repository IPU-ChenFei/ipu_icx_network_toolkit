...
class AccelInstallOnVMAndErrorInjectionOnQat(HqmBaseTest, QatBaseTest):
    NUMBER_OF_VMS = 1
    VM = [VMs.CENTOS] * 1
    ...
    TEST_CASE_ID = ["16014644452-DLB+QAT+DSA+IAX Concurrency test on Guest VM and Error Injection on QAT"]
    ...
    STEP_DATA_DICT = {1: {'step_details': 'BIOS Settings as per TC and update grub configs',
                      ...
                      7: {
                          'step_details': 'Run 4 Accelerators workloads Concurrently for an hour on Guest VM and inject QAT error on Guest VM',
                          'expected_results': 'Ran 4 Accelerators workloads Concurrently for an hour successfully on Guest VM and injected QAT error on Guest VM'}
                      }
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        super(AccelInstallOnVMAndErrorInjectionOnQat, self).__init__(test_log, arguments, cfg_opts,
    ...
    def execute(self):
        ...
        qat_error_injection = threading.Thread(target=self._error_injection.inject_ce_errors, args=("QAT",))  # Modify
        qat_th_err_log_handler = self._thread_logger.thread_logger(qat_error_injection)  # Modify


        #Thread started based on enable status
        if (self.qat_enable_status.lower() == 'true'):
            qat_workload_thread.start()
            qat_error_injection.start()
        ...
        qat_error_injection.join() # Modify
        ...
        self._thread_logger.stop_thread_logging(qat_th_err_log_handler)
    ...
    def cleanup(self, return_status):  # type: (bool) -> None
        super(AccelInstallOnVMAndErrorInjectionOnQat, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(
        Framework.TEST_RESULT_PASS if AccelInstallOnVMAndErrorInjectionOnQat.main() else Framework.TEST_RESULT_FAIL)
