...
class AccelInstallOnVMAndErrorInjectionOnIax(HqmBaseTest, QatBaseTest):  # Modify
    NUMBER_OF_VMS = 1
    VM = [VMs.CENTOS] * 1
    ...
    TEST_CASE_ID = ["16014644465-DLB+QAT+DSA+IAX Concurrency test on Guest VM and Error Injection on IAX"]  # Modify
    ...
    STEP_DATA_DICT = {1: {'step_details': 'BIOS Settings as per TC and update grub configs',
                      ...
                      7: {
                          'step_details': 'Run 4 Accelerators workloads Concurrently for an hour on Guest VM and inject IAX error on Guest VM',
                          'expected_results': 'Ran 4 Accelerators workloads Concurrently for an hour successfully on Guest VM and injected IAX error on Guest VM'}
                      }
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        super(AccelInstallOnVMAndErrorInjectionOnIax, self).__init__(test_log, arguments, cfg_opts,
    ...
    def execute(self):
        ...
        iax_error_injection = threading.Thread(target=self._error_injection.inject_ce_errors, args=("IAX",))
        iax_th_err_log_handler = self._thread_logger.thread_logger(iax_error_injection)
        ...
        self._thread_logger.stop_thread_logging(iax_th_err_log_handler)
    ...
    def cleanup(self, return_status):  # type: (bool) -> None
        super(AccelInstallOnVMAndErrorInjectionOnIax, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(
        Framework.TEST_RESULT_PASS if AccelInstallOnVMAndErrorInjectionOnIax.main() else Framework.TEST_RESULT_FAIL)
