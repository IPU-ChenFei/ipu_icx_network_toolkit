...
class RasMemoryCorrectableErrorInjectionWithDSAWorkloadVM(ContentBaseTestCase):
    """
    Phoenix ID : 16014312646 - RAS_Memory_Correctable_Error_Injection_with_DSA_Workload_VM
    The purpose of this test case is to validate the DSA fisher execution on guest VM and run workload.
    """
    NUMBER_OF_VMS = 1
    VM = [VMs.CENTOS] * 1
    TEST_CASE_ID = ["16014312646", "RAS_Memory_Correctable_Error_Injection_with_DSA_Workload_VM"]
    ...
    STEP_DATA_DICT = {1: {'step_details': 'BIOS Settings as per TC and update grub configs',
                      ...
                      2: {'step_details': 'Unbind the DSA device and attach and create VM..',
                          'expected_results': 'VM to be created successful with DSA device attached'},
                      3: {'step_details': 'DSA to run on VM',
                          'expected_results': 'DSA to be successfully run in VM'},
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        super(RasMemoryCorrectableErrorInjectionWithDSAWorkloadVM, self).__init__(test_log, arguments, cfg_opts,
    ...
    def execute(self):
        """
        This Method validates the DSA fisher for guest VM and run workload.
        ...
        uuid = self._common_lib_acc.get_uuid("DSA")
        ...
        self._common_lib_acc.run_mdev_workload_on_vm("DSA", "-uc", "-o 0x3", port_number=self.port_number)
        self._common_lib_acc.run_mdev_workload_on_vm("DSA", "-kc", "-i 100 -j 2", port_number=self.port_number)
        self._test_content_logger.end_step_logger(3, True)

        self._test_content_logger.start_step_logger(4)
        self._common_lib_acc.run_fisher_tool_on_vm(self.port_number, type_of_error="correctable", workload="DSA")
    ...
    def cleanup(self, return_status):  # type: (bool) -> None
        ...
        super(RasMemoryCorrectableErrorInjectionWithDSAWorkloadVM, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(
        Framework.TEST_RESULT_PASS if RasMemoryCorrectableErrorInjectionWithDSAWorkloadVM.main() else
