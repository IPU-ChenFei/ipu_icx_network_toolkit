...
class ErrorInjectionOnIax(ContentBaseTestCase):
    BIOS_CONFIG_FILE = "../accelerator_config.cfg"

    INTEL_IOMMU_ON_STR = "intel_iommu=on,sm_on iommu=on"

    TEST_CASE_ID = ["16014644465-DLB+QAT+DSA+IAX Concurrency test on HOST and Error Injection on IAX"]
    ...
    step_data_dict = {1: {'step_details': 'BIOS Settings',
                      ...
                      6: {'step_details': 'Run 4 Accelerators workloads Concurrently for an hour and inject iax error',
                          'expected_results': 'Ran 4 Accelerators workloads Concurrently for an hour successfully and injected iax error'}
                      }

    def __init__(self, test_log, arguments, cfg_opts):
        self.acc_bios_knobs = os.path.join(os.path.dirname(os.path.abspath(__file__)), self.BIOS_CONFIG_FILE)
        super(ErrorInjectionOnIax, self).__init__(test_log, arguments, cfg_opts, self.acc_bios_knobs)
    ...
    def prepare(self):
        ...
        super(ErrorInjectionOnIax, self).prepare()
    ...
    def execute(self):
        ...
        # self._error_injection.inject_ce_errors("IAX")
...
if __name__ == "__main__":
    sys.exit(
        Framework.TEST_RESULT_PASS if ErrorInjectionOnIax.main() else Framework.TEST_RESULT_FAIL)
