...
class ErrorInjectionOnQat(ContentBaseTestCase):
    BIOS_CONFIG_FILE = "../accelerator_config.cfg"

    INTEL_IOMMU_ON_STR = "intel_iommu=on,sm_on iommu=on"

    TEST_CASE_ID = ["16014644452-DLB+QAT+DSA+IAX Concurrency test on HOST and Error Injection on QAT"]
    ...
    step_data_dict = {1: {'step_details': 'BIOS Settings',
                      ...
                      6: {'step_details': 'Run 4 Accelerators workloads Concurrently for an hour and inject QAT error',
                          'expected_results': 'Ran 4 Accelerators workloads Concurrently for an hour successfully and injected QAT error'}
                      }

    def __init__(self, test_log, arguments, cfg_opts):
        self.acc_bios_knobs = os.path.join(os.path.dirname(os.path.abspath(__file__)), self.BIOS_CONFIG_FILE)
        super(ErrorInjectionOnQat, self).__init__(test_log, arguments, cfg_opts, self.acc_bios_knobs)
    ...
    def prepare(self):
        ...
        super(ErrorInjectionOnQat, self).prepare()
    ...
    def execute(self):
        ...
        qat_error_injection = threading.Thread(target=self._error_injection.inject_ce_errors, args=("QAT",))
        ...
        qat_error_injection.join()

        self._test_content_logger.end_step_logger(6, return_val=True)


        # self._error_injection.inject_ce_errors("QAT")
...
if __name__ == "__main__":
    sys.exit(
        Framework.TEST_RESULT_PASS if ErrorInjectionOnQat.main() else Framework.TEST_RESULT_FAIL)
