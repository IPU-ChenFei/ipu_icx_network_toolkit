...
class IaxPowerManagementVerificationsAcpiPstatesCstatesL(HqmBaseTest, QatBaseTest):
    
    ACC_CONFIG_FILE = "../accelerator_config.cfg"

    INTEL_IOMMU_ON_STR = "intel_iommu=on,sm_on iommu=on"

    TEST_CASE_ID = ["16014287187-IAX_Power_Management_Verifications_of_ACPI_P_States_C_States_CentOS"]
    ...
    step_data_dict = {1: {'step_details': 'Enable VT-d & C State Control Support in Bios option. Boot to OS',
                      ...
                      7: {'step_details': 'Download and build accel-config tool. Download DSA/IAX random config scripts, Configure all IAX devices with user type work-queues and run IAX test',
                          'expected_results': 'accel-config tool built successful. IAX Test executed succesfully'},
                      ...
                      11: {'step_details': 'C state with IAX workload Interrupt',
                          'expected_results': './Setup_Randomize_IAX_Conf.sh stopped proccessing successfully.'},
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        super(IaxPowerManagementVerificationsAcpiPstatesCstatesL, self).__init__(test_log, arguments, cfg_opts,
                                                                              self.acc_bios_knobs)
    ...
    def prepare(self):
        ...
        super(IaxPowerManagementVerificationsAcpiPstatesCstatesL, self).prepare()
    ...
    def execute(self):
        ...
        self._accelerator_lib.run_workload_with_interrupt_on_host(workload='IAX')
...
if __name__ == "__main__":
    sys.exit(
        Framework.TEST_RESULT_PASS if IaxPowerManagementVerificationsAcpiPstatesCstatesL.main() else Framework.TEST_RESULT_FAIL)