...
class UncorrectableFatalErrorOnQatDevices(ContentBaseTestCase):
    BIOS_CONFIG_FILE = "../accelerator_config.cfg"

    INTEL_IOMMU_ON_STR = "intel_iommu=on,sm_on iommu=on"

    TEST_CASE_ID = ["16013412960-RAS PCIe QAT Uncorrectable Fatal Error Injection for all QAT devices"]

    step_data_dict = {
        1: {'step_details': 'QAT driver Installation',
            'expected_results': 'DLB driver successful '},

        2: {'step_details': 'Inject uncorrectable error into all QAT devices.',
            'expected_results': 'Injected uncorrectable error into all QAT devices.'}
    }

    def __init__(self, test_log, arguments, cfg_opts):
        self.acc_bios_knobs = os.path.join(os.path.dirname(os.path.abspath(__file__)), self.BIOS_CONFIG_FILE)
        super(UncorrectableFatalErrorOnQatDevices, self).__init__(test_log, arguments, cfg_opts, self.acc_bios_knobs)
    ...
    def execute(self):
        ...
        self._error_injection.inject_uce_errors("QAT")
...
if __name__ == "__main__":
    sys.exit(
        Framework.TEST_RESULT_PASS if UncorrectableFatalErrorOnQatDevices.main() else Framework.TEST_RESULT_FAIL)
