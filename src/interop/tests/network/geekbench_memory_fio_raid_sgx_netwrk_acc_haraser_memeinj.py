...
class NetworkRaidFioGeekMemoryAccHarasserMemErr(ContentBaseTestCase):
    """
    Phoenix ID : 16016997643 - RAID using VMD VROC + Storage IO (FIO-64K block size rw) + Network Traffic (iperf)+ Memory(StressAPP), CPU stress(GeekBench) + SGX+PM harassers+Accelerators Workloads+Memory error injection
    This class to verify stress and stability after running geekbench, Stressapp, iperf, PM harassers, accelerators, sgx over raid FIO
    """
    TEST_CASE_ID = ["16016997643 - RAID using VMD VROC + Storage IO (FIO-64K block size rw) + Network Traffic (iperf)+ Memory(StressAPP), CPU stress(GeekBench) + SGX+PM harassers+Accelerators Workloads+Memory error injection"]
    STEP_DATA_DICT = {
                      ...
                      6: {'step_details': 'Create Raid',
                          'expected_results': 'Raid created successfully'},
                      7: {'step_details': 'Run all workloads',
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        # """
        # Creates a new object for NetworkRaidFioGeekMemoryAccHarasserMemErr
        #
        # :param test_log: Used for debug and info messages
        # :param arguments:
        # :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        # """
        super(NetworkRaidFioGeekMemoryAccHarasserMemErr, self).__init__(test_log, arguments, cfg_opts)
        ...
        super(NetworkRaidFioGeekMemoryAccHarasserMemErr, self).__init__(test_log, arguments, cfg_opts, self.mem_einj_acc_biosknobs)
        ...
        self.FIO_CMDS = ["fio --name=read --rw=read --numjobs=2 --bs=64k --filename=/dev/{}"
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(NetworkRaidFioGeekMemoryAccHarasserMemErr, self).prepare()
    ...
    def execute(self):
        """
        This method is used to run the Memory and CPU stress, run network traffic,memory stress fio on raid,
        accelerator workloads, pm harassers and memory error injection

        :return: True or False
        :raise: if non zero errors raise content_exceptions.TestFail
        """''

        cpu_stress_thread = threading.Thread(target= self._support_methods.execute_geekbench)
        ...
        mem_err_thread = threading.Timer(4.0,self._support_methods.execute_fisher_command,[self.FIO_CMDS[0]])
        ...
        cpu_usage_thread = threading.Timer(60.0,self._support_methods.cpu_monitoring)
    ...
    def cleanup(self, return_status):
        ...
        super(NetworkRaidFioGeekMemoryAccHarasserMemErr, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if NetworkRaidFioGeekMemoryAccHarasserMemErr.main()
