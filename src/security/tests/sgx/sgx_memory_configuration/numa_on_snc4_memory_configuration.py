...
class SgxNumaOnSnc4MemoryConfiguration(content_base_test_case.ContentBaseTestCase):
    """
    Glasgow Id : G59134.2-NUMA ON / SNC4 Memory Configuration

    Verify the SGX memory range compatibility with NUMA ON and SNC4 configuration.
    Verifying system set-up of PRMRR regions with SGX enabled in memory configuration mode NUMA ON and SNC4 enabled
    """
    SNC4_CONFIG_FILE = "numa_on_snc4.cfg"
    TEST_CASE_ID = ["G59134.2","NUMA ON / SNC4 Memory Configuration"]
    STEP_DATA_DICT = {
        1: {'step_details': 'Clear cmos',
            'expected_results': 'CMOS cleared'},
        2: {'step_details': 'Enable TME, SGX, NUMA, SNC4 and disable UMA-Based Clustering ',
            'expected_results': 'Verified TME, SGX, NUMA, SNC4 enabled and UMA-Based Clustering disabled'},
        3: {'step_details': 'Checking the PRMRR Bases without the mask values',
            'expected_results': 'The all values are non-zero results are different'},
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        """
        self.snc4_bios_config_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), self.SNC4_CONFIG_FILE)
        self.bios_config_file = SGXConstant(test_log).sgx_bios_knobs(self.snc4_bios_config_file)
        super(SgxNumaOnSnc4MemoryConfiguration, self).__init__(test_log, arguments, cfg_opts, self.bios_config_file)
    ...
    def prepare(self):
        """preparing the setup:
        clear cmos
        enabling SGX, SNC4 and Numa and disabling UMA-Based Clustering option
        """
        ...
        super(SgxNumaOnSnc4MemoryConfiguration, self).prepare()

    def execute(self):
        """This method executes the below:
        1.verifies SGX and SNC4 is enabled
        2.check the prmrr base value for snc4
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if SgxNumaOnSnc4MemoryConfiguration.main() else Framework.TEST_RESULT_FAIL)
