...
class SGXRegistrationDcPowerCycle(SGXCyclingCommon):
    """
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new SGXRegistrationDcPowerCycle object

        :param test_log: Used for debug and info messages
        :param arguments: None
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(SGXRegistrationDcPowerCycle, self).__init__(test_log, arguments, cfg_opts)
        self.total_sgx_dc_cycle_number, self.dc_cycle_recovery_mode = \
            self._common_content_configuration.get_sgx_num_of_cycles(self._DC_CYCLE_REG)
    ...
    def execute(self):
        ...
        """
        dc_cycle_str = "DC Cycle Registration"
        self.sgx_provider.check_sgx_tem_base_test()
        self.trigger_sgx_cycle(self.total_sgx_dc_cycle_number, self.sgx_cycle_type[self._DC_CYCLE_REG],
                               dc_cycle_str, self.dc_cycle_recovery_mode, True)
        return True


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if SGXRegistrationDcPowerCycle.main() else Framework.TEST_RESULT_FAIL)
