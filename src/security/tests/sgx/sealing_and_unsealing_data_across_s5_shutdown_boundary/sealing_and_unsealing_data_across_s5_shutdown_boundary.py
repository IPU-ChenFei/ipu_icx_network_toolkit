...
class SealingAndUnsealingDataAcrossS5ShutdownBoundary(content_base_test_case.ContentBaseTestCase):
    """
    Phoenix ID : P18014072186 - Platform Power State Compatibility - Sealing  and Unsealing data across S5
                                shutdown boundary
    Glasgow ID : G63281.1-Sealing and Unsealing data across S5 shutdown boundary
    Verifies data sealing/unsealing across S5 shutdown
    """
    BIOS_CONFIG_FILE = "../sgx_enable_through_bios.cfg"
    TEST_CASE_ID = ["G63281.1", "Sealing and Unsealing data across S5 shutdown boundary"]
    SOCKET0 = 0
    SOCKET1 = 1
    STEP_DATA_DICT = {
        ...
        3: {'step_details': 'Perform Shutdown test for socket 0',
            'expected_results': 'Shutdown test was successfully performed'},
        4: {'step_details': 'Set correct permission by running command sudo usermod -a -Groot aesmd',
            'expected_results': 'Permission command executed successfully'},
        5: {'step_details': 'Perform Shutdown test for socket 1',
            'expected_results': 'Shutdown test was successfully performed'},
        6: {'step_details': 'Set correct permission by running command sudo usermod -a -Groot aesmd',
            'expected_results': 'Permission command executed successfully'},
        7: {'step_details': 'Verify S5 shutdown sealing/unsealing',
            'expected_results': 'S5 shutdown sealing/unsealing verified successfully'}
        }

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create an instance of SealingAndUnsealingDataAcrossS5ShutdownBoundary
        ...
        super(SealingAndUnsealingDataAcrossS5ShutdownBoundary, self).__init__(test_log, arguments, cfg_opts,
    ...
    def prepare(self):
        ...
        super(SealingAndUnsealingDataAcrossS5ShutdownBoundary, self).prepare()
        self._log.info("SGX set successfully")
        self._test_content_logger.end_step_logger(1, return_val=True)

    def execute(self):
        """Run Sealing And Unsealing Data Across S5 Shutdown Boundary"""
        ...
        self.sgx.perform_s5_shutdown_boundary(self.phy, self.SOCKET0)
        ...
        self.sgx.perform_s5_shutdown_boundary(self.phy, self.SOCKET1)
        ...
        self.sgx.verify_seal_unseal_shutdown_boundary()
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if SealingAndUnsealingDataAcrossS5ShutdownBoundary.main() else
