...
class AlgorithmAgilitySha384(TxtBaseTest):
    """
    Phoneix ID : P18014069570
    Verify SHA384 algorithm on TPM 2.0, provision TPM 2.0, and verify can boot trusted.
    1: Enable TXT knobs,  boot to tboot + Linux.
    2: Reboot SUT to UEFI shell and provision TPM 2.0 with SHA384.
    3: Reboot system and verify SUT can boot trusted.
    """

    _BIOS_CONFIG_FILE = "txt_base/security_txt_bios_knobs_enable.cfg"
    _TEST_CASE_ID = ["18014069598", "AlgorithmAgilitySha384"]
    ...
    step_data_dict = {1: {'step_details': 'Executing lsmem/lscpu/lspci command, enabling TXT BIOS Knobs'
                      ...
                      3: {'step_details': 'Execute TPM provisioning command with SHA384',
                          'expected_results': 'Provisioning completed successfully'}}

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create an instance of AlgorithmAgilitySha384
        ...
        super(AlgorithmAgilitySha384, self).__init__(test_log, arguments, cfg_opts)
    ...
    def execute(self):
        ...
        try:
            ...
            self.verify_sha384_and_provision_tpm(usb_drive_list)
    ...
    def cleanup(self, return_status):  # type: (bool) -> None
        ...
        if str(self.current_boot_order) != str(self.previous_boot_oder):
            self.set_boot_order(self.itp_xml_cli_util, self.previous_boot_oder)
        super(AlgorithmAgilitySha384, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if AlgorithmAgilitySha384.main() else Framework.TEST_RESULT_FAIL)
