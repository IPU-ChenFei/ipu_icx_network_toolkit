...
class LCP208BADMLE(TxtBaseTest):
    """
    Glasgow ID : 58937

    This test case tests LCP208 with a provisioned PO index with 1 TPM2 list of: RSA-3072-SHA256, SHA1 MLE LEGACY
    ELEMENT(BAD), SHA1 MLE ELEMENT.
    ...
    LCP208 = os.getcwd() + "\\lcp208.pol"
    IDEF_FILE = os.getcwd() + "\\ExamplePO_Sha256.iDef"
    IDEF_FILE_BACKUP = os.getcwd() + "\\ExamplePO_Sha256.iDef.bak"

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create an instance of LCP208BADMLE

        :param cfg_opts: Configuration Object of provider
        :param test_log: Log object
        :param arguments: None
        """
        super(LCP208BADMLE, self).__init__(test_log, arguments, cfg_opts, self.BIOS_CONFIG_FILE)
    ...
    def execute(self):
        ...
        self.create_lcp2_list(out="lcp208.list", elements=[mle_legacy_element_name, mle_element_name], sig_alg='rsa',
                              list_ver=self.platform_family)
        self.sign_lcp2_list(out="lcp208.list", rev='2', private_key=self.PRIVATE_KEY, public_key=self.PUBLIC_KEY,
                            sig_alg='rsa', hash_alg=HashAlgorithms.SHA256)
        self.create_policy(cbnt_product_version=self.platform_family, pol="lcp208.pol", data="lcp208.data",
                           lcp2_lists=["lcp208.list"], masks=[HashAlgorithms.SHA1, HashAlgorithms.SHA256],
                           policy_type='list', ctrl='2', rev='2', alg=HashAlgorithms.SHA256, min_ver='0',
                           sign="rsa-3072-sha256")
        self._log.debug("Copying lcp208.data into /boot/")
        self._os.execute("cp lcp208.data /boot/", self._command_timeout)
        self._os.copy_file_from_sut_to_local("/root/lcp208.pol", self.LCP208)
        ...
        self.modify_linux_grub_file(self.LAST_IMAGE_IN_GRUB, False,
                                    new_phase="module2 /lcp208.data")
        self.modify_linux_grub_file(self.LAST_IMAGE_IN_GRUB, False,
                                    new_phase="echo \"loading lcp.data for lcp208\"")

        self.generate_file_for_tpm2_provisioning(self.LCP208,
                                                 self.IDEF_FILE_BACKUP,
                                                 self.IDEF_FILE)
        self._os.execute("mkdir lcp208", self._command_timeout)
        self._os.copy_local_file_to_sut("ExamplePO_Sha256.iDef", "/root/lcp208/ExamplePO_Sha256.iDef")
        self._os.copy_local_file_to_sut("ExamplePO_Sha256.iDef", "/root/lcp208/lcp208.iDef")

        self._copy_usb.copy_file_from_sut_to_usb(self._common_content_lib, self._common_content_configuration,
                                                 "/root/lcp208", "/" + Tpm2Drive.TPM2_FOLDER +
                                                 Tpm2Drive.TPM2_PROVISIONING_FOLDER)
    ...
    def cleanup(self, return_status):  # type: (bool) -> None
        super(LCP208BADMLE, self).cleanup(return_status)
        self._log.info("Copying over any policy for cleanup")
        self._os.copy_local_file_to_sut(self.IDEF_FILE_BACKUP, "/root/lcp208/ExamplePO_Sha256.iDef")
        self._copy_usb.copy_file_from_sut_to_usb(self._common_content_lib, self._common_content_configuration,
                                                 "/root/lcp208", "/" + Tpm2Drive.TPM2_FOLDER +
                                                 Tpm2Drive.TPM2_PROVISIONING_FOLDER)
        self.modify_linux_grub_file(targeted_phrase="module2 /lcp208.data", deletion=True)
        self.modify_linux_grub_file(targeted_phrase="echo \"loading lcp.data for lcp208\"", deletion=True)
        if not self._uefi_util_obj.enter_uefi_shell():
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if LCP208BADMLE.main() else Framework.TEST_RESULT_FAIL)
