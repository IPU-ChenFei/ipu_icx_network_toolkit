...
"""
    ...
    Verify available max keys are split between MKTME and TDX in BIOS.
"""
import sys
...
class TdxKeySplit(LinuxTdxBaseTest):
    """
            ...
            :Test steps:
                ...
                :7: Repeat for each option in key split menu in BIOS.

            :Expected results: The number of TDX keys + the number of TME-MT keys = the number of max keys.
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        :return: None
        """
        super(TdxKeySplit, self).__init__(test_log, arguments, cfg_opts)
        self.max_keys = self.tdx_consts.MAX_KEYS
    ...
    def execute(self):
        for split in self.tdx_consts.KEY_IDX.keys():
            if split != 0:  # skip assigning zero keys, covered in another test case
                ...
                if tmemt_keys + tdx_keys != self.max_keys:
                    ...
                    self._log.error("Keys do not add up.  TDX keys: {} TME keys: {} Max keys: {}".format(
                        tdx_keys, tmemt_keys, self.max_keys))
                    return False
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if TdxKeySplit.main() else Framework.TEST_RESULT_FAIL)
