...
HVC_CONSOLE="-chardev stdio,id=mux,mux=on,logfile=$LOGFILE \
...
process_args() {
    ...
    if [[ $GUEST_IMG == *.qcow2 ]]; then
      ...
      QEMU_CMD+=" -drive file=$(readlink -f "${GUEST_IMG}"),if=virtio,format=raw "
    fi
    ...
    if [[ ${USE_VSOCK} == true ]]; then
        QEMU_CMD+=" -device vhost-vsock-pci,guest-cid=$CID_LABEL "
    fi
