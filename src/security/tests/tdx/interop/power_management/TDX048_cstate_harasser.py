...
##########################################################################
"""
    :C-state Harasser + TDX Base Test:

    Launch a given number of TD guests and run c-state harasser pm_utis test suite on the SUT for prescribed amount
    of time."""
...
class TdxCstateHarasser(TdxPmUtilsBaseTest):
    """
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new object for TdxCstateHarasser

        :param test_log: Used for debug and info messages
        :param arguments:
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(TdxCstateHarasser, self).__init__(test_log, arguments, cfg_opts)
        self.pm_util_timeout = self._common_content_configuration.security_pmutils_running_time()
        self.pm_util_cmd = self.tdx_consts.PmUtilsCmds.PMUTILS_CSTATE_HARASSER.format(self.pm_util_timeout)  # command

    def execute(self):
        return super(TdxCstateHarasser, self).execute()


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if TdxCstateHarasser.main() else Framework.TEST_RESULT_FAIL)
