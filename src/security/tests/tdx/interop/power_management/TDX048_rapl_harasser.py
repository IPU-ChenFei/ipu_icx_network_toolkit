...
##########################################################################
"""
    :RAPL Harasser with TDX:

    Launch a given number of TD guests and run RAPL harasser pm_utis test suite on the SUT for prescribed amount of
    time."""
...
class TdxRaplHarasser(TdxPmUtilsBaseTest):
    """
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        """
        super(TdxRaplHarasser, self).__init__(test_log, arguments, cfg_opts)
        self.pm_util_timeout = self._common_content_configuration.security_pmutils_running_time()
        self.pm_util_cmd = self.tdx_consts.PmUtilsCmds.PMUTILS_RAPL_HARASSER.format(self.pm_util_timeout)  # command

    def execute(self):
        return super(TdxRaplHarasser, self).execute()


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if TdxRaplHarasser.main() else Framework.TEST_RESULT_FAIL)
