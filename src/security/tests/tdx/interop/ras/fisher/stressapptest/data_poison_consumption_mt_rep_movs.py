...
##########################################################################
"""
    :TDX RAS Interop Data Poison Consumption MT - REP MOVS:
...
class TdxDataPoisonConsumptionMTREPMOVS(TdxRasFisherBaseTest):
    """
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        :return: None
        """
        super(TdxDataPoisonConsumptionMTREPMOVS, self).__init__(test_log, arguments, cfg_opts)
    ...
    def execute(self):
        self.fisher_command = "fisher --workload=\"stressapptest -M {} -F -s 7200\" " \
                              "--injection-type=memory-uncorrectable-non-fatal --match=DRd_rep.movs " \
                              "--runtime={}h".format(int(self.free_mem), int(self.run_time))
        return super(TdxDataPoisonConsumptionMTREPMOVS, self).execute()


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if TdxDataPoisonConsumptionMTREPMOVS.main() else Framework.TEST_RESULT_FAIL)
