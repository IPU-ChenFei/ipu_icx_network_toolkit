...
##########################################################################
"""
    :TDX RAS Interop Poison Cache Flush:
...
class TdxPoisonCacheFlush(TdxRasFisherBaseTest):
    """
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        :return: None
        """
        super(TdxPoisonCacheFlush, self).__init__(test_log, arguments, cfg_opts)
        self.workload_name = "stress-ng"
        self.run_time = 10  # 10 hours time out
        self.fisher_command = "fisher --workload=\"stress-ng --cache \\$(shuf -i1-\\$(nproc) -n1) --cache-fence " \
                              "--cache-flush --cache-prefetch --cache-level=3 --verify --timeout 5m\" " \
                              "--injection-type=memory-uncorrectable-non-fatal --match=PreF --inspect-rate=200us " \
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if TdxPoisonCacheFlush.main() else Framework.TEST_RESULT_FAIL)
