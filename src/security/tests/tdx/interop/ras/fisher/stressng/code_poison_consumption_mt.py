...
##########################################################################
"""
    :TDX RAS Interop Code Poison Consumption MT:
...
class TdxCodePoisonConsumptionMT(TdxRasFisherBaseTest):
    """
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        :return: None
        """
        super(TdxCodePoisonConsumptionMT, self).__init__(test_log, arguments, cfg_opts)
        _RAS_FISHER_BIOS_KNOB = "collateral/fisher_knobs.cfg"
        self.workload_name = "stress-ng"
        self.run_time = 1  # 1 hour time out
        self.fisher_command = "fisher --workload=\"stress-ng --cpu 0 --cpu-method all --verify --timeout 5m\" " \
                              "--injection-type=memory-uncorrectable-non-fatal --match=CRd " \
                              f"--workload-exit-code=0,2 --runtime={self.run_time}h"
        self.pass_ratio = 0.8
        self.bios_knobs = ["fisher_poison_knobs.cfg"]


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if TdxCodePoisonConsumptionMT.main() else Framework.TEST_RESULT_FAIL)
