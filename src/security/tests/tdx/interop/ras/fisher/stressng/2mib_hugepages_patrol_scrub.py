...
##########################################################################
"""
    :TDX RAS Interop Data Poison 2 MiB Huge Pages:
...
class TdxTwoMiBHugePagesPatrolScrub(TdxRasFisherBaseTest):
    """
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        :return: None
        """
        super(TdxTwoMiBHugePagesPatrolScrub, self).__init__(test_log, arguments, cfg_opts)
        self.workload_name = "stress-ng"
        self.run_time = 10  # 10 hours time out
        self.fisher_command = "fisher --workload=\"stress-ng --stream 0 --stream-madvise hugepage --verify " \
                              "--timeout 5m\" --injection-type=memory-uncorrectable-non-fatal --match=DRd " \
                              "--workload-exit-code=0,2 " \
                              "--os-verification-rules=os-verification-trigger " \
                              f"--strict-inspect --runtime={self.run_time}h"
        self.pass_ratio = 0.8
        self.bios_knobs = ["fisher_poison_knobs.cfg"]


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if TdxTwoMiBHugePagesPatrolScrub.main() else Framework.TEST_RESULT_FAIL)
