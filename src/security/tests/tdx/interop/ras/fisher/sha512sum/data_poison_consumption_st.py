...
##########################################################################
"""
    :TDX RAS Interop Data Poison Consumption ST:
...
class TdxDataPoisonConsumptionST(TdxRasFisherBaseTest):
    """
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        :return: None
        """
        super(TdxDataPoisonConsumptionST, self).__init__(test_log, arguments, cfg_opts)
        self.workload_name = self.tdx_consts.WorkloadToolNames.SHA512SUM
        self.run_time = 1  # 1 hour run time - should finish before this
        self.pass_ratio = 0.8
        self.bios_knobs = ["fisher_poison_knobs.cfg"]
        self.fisher_command = "fisher --workload=\"sha512sum /dev/urandom\" " \
                              "--injection-type=memory-uncorrectable-non-fatal --injection-delay-max=0ms " \
                              "--cycles=2000"


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if TdxDataPoisonConsumptionST.main() else Framework.TEST_RESULT_FAIL)
