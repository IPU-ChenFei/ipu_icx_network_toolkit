...
class TdxNumaOnSncOffWindows(TDLaunchWindows):
    """
            ...
            :Test steps:
                ...
                :3: Verify the status of the following BIOS knobs:  Numa is enabled, UMA is enabled.
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        :return: None
        """
        super(TdxNumaOnSncOffWindows, self).__init__(test_log, arguments, cfg_opts)
    ...
    def execute(self):
        ...
        if not self.uma_check():
            raise content_exceptions.TestFail("UMA didn't enabled")
        snc_clusters = self.get_snc_setting()

        if snc_clusters == self._SNC_DISABLED:
            self._log.info("SNC is disabled and cluster count is 0")
        else:
            raise content_exceptions.TestError("SNC values doesn't set properly")
        ...
        return super(TdxNumaOnSncOffWindows, self).execute()


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if TdxNumaOnSncOffWindows.main() else Framework.TEST_RESULT_FAIL)
