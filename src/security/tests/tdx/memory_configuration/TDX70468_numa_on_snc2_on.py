...
class TdxNumaOnSncTwo(TDVMLaunchLinux):
    """
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        :return: None
        """
        super(TdxNumaOnSncTwo, self).__init__(test_log, arguments, cfg_opts)
    ...
    def prepare(self):
        if not self.one_dpc_config_check() and not self.two_dpc_config_check():
            raise content_exceptions.TestSetupError("Dimm configuration on SUT does not support SNC2.")
        super(TdxNumaOnSncTwo, self).prepare()
        cpu_sockets = self.get_sockets()
        if cpu_sockets == 2:
            self._NUMBER_SNC_CLUSTERS = 4
        elif cpu_sockets == 4 or cpu_sockets == 8:
    ...
    def execute(self):
        ...
        return super(TdxNumaOnSncTwo, self).execute()


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if TdxNumaOnSncTwo.main() else Framework.TEST_RESULT_FAIL)
