...
class MkTmeKeyCheckForEachSocket(MktmeBaseTest):
    """
    Phoenix ID : 18014070753 - Verify that TME key is different for each socket
    """
    TEST_CASE_ID = "18014070753"
    ...
    def __init__(self, test_log: logging.Logger, arguments: argparse.Namespace, cfg_opts: ElementTree) -> None:
        ...
        super(MkTmeKeyCheckForEachSocket, self).__init__(test_log, arguments, cfg_opts)
        self.socket_count = 0

    def prepare(self):
        # type: () -> None
        super(MkTmeKeyCheckForEachSocket, self).prepare()
    ...
    def execute(self):
        ...
        Tme_key for each socket is unique
        """
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if MkTmeKeyCheckForEachSocket.main() else Framework.TEST_RESULT_FAIL)
