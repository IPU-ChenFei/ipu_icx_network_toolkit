...
class UpiLaneReceiveFailureSingleEndAllHighLanes(UpiLaneFailoverCommon):
    """
    Glasgow_id : G58366-lane_receive_failure_single_end_all_high_lanes
    ...
    This test causes a UPI port 0 AND a UPI Port 1 transmit failure on double link endpoints on all high lanes [7:0]
    and verifies the error is corrected and signaled to the OS as such.
    """
    TEST_CASE_ID = ["G58366", "lane_receive_failure_single_end_all_high_lanes"]
    _BIOS_CONFIG_FILE = "upi_lane_failure_bios_knobs.cfg"

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new UpiLaneReceiveFailureSingleEndAllHighLanes object

        :param test_log: Used for debug and info messages
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(UpiLaneReceiveFailureSingleEndAllHighLanes, self).__init__(test_log, arguments, cfg_opts, self._BIOS_CONFIG_FILE)
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(UpiLaneReceiveFailureSingleEndAllHighLanes, self).prepare()
    ...
    def execute(self):
        ...
        causes a UPI port 0 AND a UPI Port 1 transmit failure on double link endpoints on all high lanes [7:0] and
    ...
    def cleanup(self, return_status):
        # type: (bool) -> None
        """DTAF cleanup"""
        super(UpiLaneReceiveFailureSingleEndAllHighLanes, self).cleanup(return_status)
        if self.os.is_alive():
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if UpiLaneReceiveFailureSingleEndAllHighLanes.main() else
