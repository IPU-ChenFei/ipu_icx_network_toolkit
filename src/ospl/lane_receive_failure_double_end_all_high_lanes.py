...
class UpiLaneReceiveFailureDoubleEndAllHighLanes(UpiLaneFailoverCommon):
    """
    Glasgow_id : G58369-lane_receive_failure_double_end_all_high_lanes
    ...
    This test causes a UPI port 0 and a UPI port 1 receive failure on double link endpoints on all high lanes [7:0]
    and verifies the error is corrected and signaled to the OS as such.
    """
    TEST_CASE_ID = ["G58369 ", "lane_receive_failure_double_end_all_high_lanes"]
    _BIOS_CONFIG_FILE = "upi_lane_failure_bios_knobs.cfg"

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new UpiLaneReceiveFailureDoubleEndAllHighLanes object

        :param test_log: Used for debug and info messages
        :param arguments: None
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(UpiLaneReceiveFailureDoubleEndAllHighLanes, self).__init__(test_log, arguments, cfg_opts,
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(UpiLaneReceiveFailureDoubleEndAllHighLanes, self).prepare()
    ...
    def execute(self):
        ...
        causes a UPI port 0 and a UPI port 1 receive failure on double link endpoints on all high lanes [7:0] and
        verifies the error is corrected and signaled to the OS as such.
        ...
        return self.inject_and_check_upi_link_width_change_failure(self.RECEIVE,
                                                                   self.UPI_LANE_MASK_DISABLE_ALL_HIGH_LANES,
                                                                   change_both_link_ends=True)

    def cleanup(self, return_status):
        # type: (bool) -> None
        """DTAF cleanup"""
        super(UpiLaneReceiveFailureDoubleEndAllHighLanes, self).cleanup(return_status)
        if self.os.is_alive():
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if UpiLaneReceiveFailureDoubleEndAllHighLanes.main() else
