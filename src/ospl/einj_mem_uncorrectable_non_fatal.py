...
class EinjMemUnCorrectableNonFatal(EinjCommon):
    ...
    def execute(self):
        ...
        """
        self._ras_einj_obj.einj_inject_and_check(error_type=self._ras_einj_obj.EINJ_MEM_UNCORRECTABLE_NONFATAL)

    def cleanup(self, return_status):
        # type: (bool) -> None
        """DTAF cleanup"""
        super(EinjMemUnCorrectableNonFatal, self).cleanup(return_status)
        if self.os.is_alive():
