...
# from src.configuration.config.sut_tool.sut_tools import SUT_TOOLS_WINDOWS_NETWORK
...
CASE_DESC = [
    "Install iperf3 to windows sut"
]

def test_steps(sut, sutos):

    SUT_TOOLS_WINDOWS_NETWORK = sut_tool('SUT_TOOLS_WINDOWS_NETWORK')
    tools = get_tool(sut)
    ...
    tools.get('iperf3_w').uncompress_to(method='zip')

    Case.step('Install iperf3')
    # create symbolic link to target file/directory if some version number are included inside uncompressed path
    try:
        sutos.execute_cmd(sut,
                          rf'mklink /D {SUT_TOOLS_WINDOWS_NETWORK}\iperf3, {tools.iperf3_w.ipath}\{os.path.basename(tools.iperf3_w.ipath)}',
                          timeout=30)
    except:
        sutos.execute_cmd(sut, rf'rd /s/q {SUT_TOOLS_WINDOWS_NETWORK}\iperf3')
        sutos.execute_cmd(sut,
                          rf'mklink /D {SUT_TOOLS_WINDOWS_NETWORK}\iperf3, {tools.iperf3_w.ipath}\{os.path.basename(tools.iperf3_w.ipath)}',
                          timeout=30)
    Case.step('Run iperf3 in test script')
    sutos.execute_cmd(sut, rf'{SUT_TOOLS_WINDOWS_NETWORK}\iperf3\iperf3.exe -v', timeout=30)
