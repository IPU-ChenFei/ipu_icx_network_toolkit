...
CASE_DESC = [
    "Install pxe server to linux sut, only for < Redhat > OS"
]
...
def test_steps(sut, sutos):
    ...
    tools.get('pxeserver_l').uncompress_to(method='copy')

    # install
    filename = tools.get("pxeserver_l").filename
    sut.execute_shell_cmd(f'dos2unix {filename} && sh {filename}', timeout=10 * 60, cwd=SUT_TOOLS_LINUX_NETWORK)
