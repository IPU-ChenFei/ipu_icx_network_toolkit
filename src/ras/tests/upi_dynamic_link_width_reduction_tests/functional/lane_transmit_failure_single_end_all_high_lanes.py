...
class UpiLaneTransmitFailureSingleEndAllHighLanes(UpiLaneFailoverCommon):
    """
    Glasgow_id : 58357-lane_transmit_failure_single_end_all_high_lanes
    ...
    This test causes a UPI port 0 transmit failure on a single link endpoint on all high lanes [7:0] and verifies the
    error is corrected and signaled to the OS as such.
    """
    TEST_CASE_ID = ["G58357", "lane_transmit_failure_single_end_all_high_lanes"]
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        """
        super(UpiLaneTransmitFailureSingleEndAllHighLanes, self).__init__(test_log, arguments, cfg_opts,
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(UpiLaneTransmitFailureSingleEndAllHighLanes, self).prepare()

    def execute(self):
        """
        This method is used to execute check_upi_lane_failover_enabled to verify if Upi self healing is enabled and
        causes a UPI port 0 transmit failure on a single link endpoint on all high lanes [7:0] and verifies the error
        is corrected and signaled to the OS as such.
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if UpiLaneTransmitFailureSingleEndAllHighLanes.main() else
