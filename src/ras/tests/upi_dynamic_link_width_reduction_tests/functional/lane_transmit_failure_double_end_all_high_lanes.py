...
class UpiLaneTransmitFailureDoubleEndAllHighLanes(UpiLaneFailoverCommon):
    """
    Glasgow_id : G58362-lane_transmit_failure_double_end_all_high_lanes
    ...
    This test causes a UPI port 0 AND UPI port 1 receive failure on double link endpoints on all low lanes [7:0] and
    verifies the error is corrected and signaled to the OS as such.
    """
    TEST_CASE_ID = ["G58362", "lane_transmit_failure_double_end_all_high_lanes"]
    _BIOS_CONFIG_FILE = "upi_lane_failure_bios_knobs.cfg"

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new UpiLaneTransmitFailureDoubleEndAllHighLanes object

        :param test_log: Used for debug and info messages
        :param arguments: None
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(UpiLaneTransmitFailureDoubleEndAllHighLanes, self).__init__(test_log, arguments, cfg_opts,
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(UpiLaneTransmitFailureDoubleEndAllHighLanes, self).prepare()

    def execute(self):
        """
        This method is used to execute check_upi_lane_failover_enabled to verify if Upi self healing is enabled and
        causes a UPI port 0 AND UPI port 1 receive failure on double link endpoints on all low lanes [7:0] and verifies
         the error is corrected and signaled to the OS as such.
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if UpiLaneTransmitFailureDoubleEndAllHighLanes.main() else
