...
class UpiLaneTransmitFailureSingleEndOneHighLane(UpiLaneFailoverCommon):
    """
    Glasgow_id : G58364-lane_transmit_failure_single_end_one_high_lanes
    ...
    This test causes a UPI port 1 transmit failure on a single link endpoint on all high lanes [7:0] and verifies the
     error is corrected and signaled to the OS as such.
    """
    TEST_CASE_ID = ["G58364", "lane_transmit_failure_single_end_one_high_lanes"]
    _BIOS_CONFIG_FILE = "upi_lane_failure_bios_knobs.cfg"

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new UpiLaneTransmitFailureSingleEndOneHighLane object

        :param test_log: Used for debug and info messages
        :param arguments: None
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(UpiLaneTransmitFailureSingleEndOneHighLane, self).__init__(test_log, arguments, cfg_opts,
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(UpiLaneTransmitFailureSingleEndOneHighLane, self).prepare()

    def execute(self):
        """
        This method is used to execute check_upi_lane_failover_enabled to verify if Upi self healing is enabled and
        causes a UPI port 1 transmit failure on a single link endpoint on all high lanes [7:0] and verifies the error is
        ...
        return self.inject_and_check_upi_link_width_change_failure(self.TRANSMIT,
                                                                   self.UPI_LANE_MASK_DISABLE_FIRST_HIGH_LANE)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if UpiLaneTransmitFailureSingleEndOneHighLane.main() else
