...
class UpiLaneTransmitFailureSingleEndAllLowLanes(UpiLaneFailoverCommon):
    """
    Glasgow_id : 58281
    ...
    This test causes a UPI port 0 transmit failure on a single link endpoint on all low lanes [7:0] and verifies the
    error is corrected and signaled to the OS as such.
    """
    TEST_CASE_ID = ["G58281", "lane_transmit_failure_single_end_all_low_lanes"]
    _BIOS_CONFIG_FILE = "upi_lane_failure_bios_knobs.cfg"

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new UpiLaneTransmitFailureSingleEndAllLowLanes object

        :param test_log: Used for debug and info messages
        :param arguments: None
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(UpiLaneTransmitFailureSingleEndAllLowLanes, self).__init__(test_log, arguments, cfg_opts,
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(UpiLaneTransmitFailureSingleEndAllLowLanes, self).prepare()

    def execute(self):
        """
        This method is used to execute check_upi_lane_failover_enabled to verify if Upi self healing is enabled and
        causes a UPI port 0 transmit failure on a single link endpoint on all low lanes [7:0] and verifies the error is
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if UpiLaneTransmitFailureSingleEndAllLowLanes.main() else
