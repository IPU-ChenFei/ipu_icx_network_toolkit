...
class PcieUceNonfatalG3CycleEdpcEnabled(IoPmCommon):
    """
    Glasgow_id : 70796 PM RAS - Linux - PCIe Uncorrectable nonfatal G3 Cycling - eDPC enabled
    Requirement: cscripts_ei_pcie_device socket and pxp_port must be set in content_configuration.xml
    No error reported in OS logs when injecting UCE Nonfatal prior to OS Boot with EDPC enabled
    """
    NUM_CYCLES = 10
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new PcieUceNonfatalG3CycleEdpcEnabled object

        :param test_log: Used for debug and info messages
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        self.sil_cfg = cfg_opts.find(SiliconRegProvider.DEFAULT_CONFIG_PATH)
        super(PcieUceNonfatalG3CycleEdpcEnabled, self).__init__(test_log, arguments, cfg_opts, self._BIOS_CONFIG_FILE)
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(PcieUceNonfatalG3CycleEdpcEnabled, self).prepare()

    def execute(self):
        """
        PCIe UCE error injection in early boot during cold G3 cycling.
        eDPC enabled.
        ...
        for i in range(self.NUM_CYCLES):
            try:
                ...
                if not self._os_log_obj.verify_os_log_error_messages(__file__,
                    ...
                    log_err = "Error found in OS logs! No error expected in OS logs when injecting UCE Nonfatal prior" \
                              " to OS Boot."
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if PcieUceNonfatalG3CycleEdpcEnabled.main() else Framework.TEST_RESULT_FAIL)
