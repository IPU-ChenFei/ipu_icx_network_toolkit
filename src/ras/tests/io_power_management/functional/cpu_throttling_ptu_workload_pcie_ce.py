...
class CpuThrottlingPtuWorkloadPcieCe(IoPmCommon):
    """
    Glasgow_id : 70693 CPU throttling + PTU workload + PCIe correctable error injections + OS log checks
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        super(CpuThrottlingPtuWorkloadPcieCe, self).__init__(test_log, arguments, cfg_opts, self._BIOS_CONFIG_FILE)
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(CpuThrottlingPtuWorkloadPcieCe, self).prepare()
    ...
    def execute(self):
        ...
        self.os.execute_async(self.PTU_DICT_WINDOWS["RUN_TDP"], self.PTU_DICT_WINDOWS["SUT_DIR"])
        with ProviderFactory.create(self.sil_cfg, self._log) as cscripts_obj:
            ...
            try:
                ...
                # Kill PTU before cleanup
                self.kill_ptu_win(cscripts_obj.silicon_cpu_family)

        return True


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if CpuThrottlingPtuWorkloadPcieCe.main() else Framework.TEST_RESULT_FAIL)
