...
class PcieCeAlternateSocketAccessEthernet(EinjCommon):
    """
    PM RAS Alternate/secondary socket CPU IO access +PTU + PCIe correctable error + OS log checks using ethernet
    https://hsdes.intel.com/appstore/article/#/18014548750

    """

    _BIOS_CONFIG_FILE = "../io_power_management/alt_socket_biosknobs_fw_1st.cfg"
    ...
    def __init__(self, test_log, arguments, cfg_opts,):
        ...
        """
        super(PcieCeAlternateSocketAccessEthernet, self).__init__(test_log, arguments, cfg_opts,
    ...
    def prepare(self):
        ...
        super(PcieCeAlternateSocketAccessEthernet, self).prepare()

    def execute(self):
        """
        PCIe CE injection at runtime with PTU load and network performance tool running.
        eDPC enabled.
        ...
        if not self._ras_ethernet_utils_obj.inject_pcie_correctable_errors(injection_count, 'ns1', netdev1,
                                                                           self.post_ce_inj_wait_secs):
        ...
        self._ras_numa_utils_obj.numa_compare_nodes(numa_delta_0, numa_delta_1, '0', '0', threshold_upi,
                                                    injection_count, '2')
        self._ras_ethernet_utils_obj.cleanup_ethernet(sut_nic_driver)
        return True
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if PcieCeAlternateSocketAccessEthernet.main()
