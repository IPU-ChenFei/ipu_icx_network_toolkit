...
class PcieLinkDegradeWarmReboot(IoPmCommon):
    """
    Glasgow_id : 70824 PM RAS - Linux - PCIe link degrade - Warm reboot cycle
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        super(PcieLinkDegradeWarmReboot, self).__init__(test_log, arguments, cfg_opts, self._BIOS_CONFIG_FILE)
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(PcieLinkDegradeWarmReboot, self).prepare()
    ...
    def execute(self):
        ...
        for i in range(self.NUM_CYCLES):
            try:
                ...
                # initiate warm boot
                self._log.info("Issuing OS reboot ..")
                self.warm_reboot_non_blocking(alt_restart_cmd=True)
                ...
                self._log.info("resuming platform ...")
                sdp_obj.go()
                time.sleep(self.BUS_RESET_DELAY_SEC)
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if PcieLinkDegradeWarmReboot.main() else Framework.TEST_RESULT_FAIL)
