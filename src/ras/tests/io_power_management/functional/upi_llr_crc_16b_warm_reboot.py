...
class UpiLlrCrc16bWarmReboot(IoPmCommon):
    """
    Glasgow_id : 70712 PM RAS - UPI LLR CRC 16b Warm Reboot Cycling - Linux
    """
    NUM_CYCLES = 10
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        super(UpiLlrCrc16bWarmReboot, self).__init__(test_log, arguments, cfg_opts, self._BIOS_CONFIG_FILE)
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(UpiLlrCrc16bWarmReboot, self).prepare()

    def execute(self):
        """
        llr crc error injection in early boot during warm boot cycling.
        ...
        for i in range(self.NUM_CYCLES):
            try:
                ...
                with ProviderFactory.create(self.sil_cfg, self._log) as cscripts_obj:
                    ...
                    # inject error save injection count and check that it increments on warm boot
                    initial_inj_count = self.inject_and_check_upi_error_single_injection(cscripts_obj,
                                                                                         init_err_count=initial_inj_count,
                                                                                         socket=0,
                                                                                         port=0)
                    # reset break point
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if UpiLlrCrc16bWarmReboot.main() else Framework.TEST_RESULT_FAIL)
