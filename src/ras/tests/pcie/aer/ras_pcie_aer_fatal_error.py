...
class RasPcieAerFatalError(PcieAerCommon):
    """
    Glasgow id : 59688.2
    ...
    TEST_CASE_ID = ["G59688", "PI_RAS_PCIe_AER_FATAL"]

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new RasPcieAerFatalError object

        :param test_log: Used for debug and info messages
        :param arguments: Arguments used in Baseclass
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(RasPcieAerFatalError, self).__init__(test_log, arguments, cfg_opts, self.BIOS_CONFIG_FILE)
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(RasPcieAerFatalError, self).prepare()
    ...
    def execute(self):
        ...
        """
        bdf_value = []
        self.create_aer_file(type_error=RasErrorType.FATAL, cmd_path=self._aer_inject_tool_path)
        ...
        for item_value in bdf_value:
            ...
            self._log.info(
                "PCIe device: with bdf: {} is targeting for injecting Error".format(
                    str("{:02x}".format(item_value)) + ":00.0"))
            self.inject_aer_error(bdf_value=(str("{:02x}".format(item_value)) + ":00.0"),
                                  cmd_path=self._aer_inject_tool_path)            # Adding bdf value in Signature to Check in OS.
            self.DMESG_COMP_ABORT_UNC_FATAL_ERR_SIG.append(self.UNC_FATAL_ERR_RECEIVED.format(str("{:02x}".format(item_value))+":00.0"))
            time.sleep(self.WAIT_TIME)
            ret_val = self._os_log_verification_obj.verify_os_log_error_messages(
                __file__, self._os_log_verification_obj.DUT_DMESG_FILE_NAME, self.DMESG_COMP_ABORT_UNC_FATAL_ERR_SIG)
            self.DMESG_COMP_ABORT_UNC_FATAL_ERR_SIG.pop()
            if not ret_val:
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if RasPcieAerFatalError.main() else Framework.TEST_RESULT_FAIL)
