...
class RasPcieAerMultipleCorrNonFatalError(PcieAerCommon):
    """
    Glasgow id : 59690.2

    Sequentially inject a correctable bad TLP and an uncorrectable/non-fatal completion abort error into the device
    with header  log words 0 1 2 3 and 4 5 6 7.
    """
    BIOS_CONFIG_FILE = "ras_pcie_aer_bios_knobs.cfg"
    TEST_CASE_ID = ["G59690", "PI_RAS_PCIe_AER_multiple-corr-nonfatal"]

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new RasPcieAerMultipleCorrNonFatalError object

        :param test_log: Used for debug and info messages
        :param arguments: Arguments used in Baseclass
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(RasPcieAerMultipleCorrNonFatalError, self).__init__(test_log, arguments, cfg_opts, self.BIOS_CONFIG_FILE)
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(RasPcieAerMultipleCorrNonFatalError, self).prepare()
    ...
    def execute(self):
        ...
        """
        bdf_value = []
        self.create_aer_file(type_error=RasErrorType.MULTIPLE_CORRECTABLE_NON_FATAL, cmd_path=self._aer_inject_tool_path)
        ...
        for item_value in bdf_value:
            ...
            self.DMESG_BAD_TLP_CORR_ABORT_UNC_NONFATAL_ERR_SIG.append(self.CORR_ERR_RECEIVED.format(str("{:02x}".format(item_value))+":00.0"))
            ...
            ret_val = self._os_log_verification_obj.verify_os_log_error_messages(
                ...
                self.DMESG_BAD_TLP_CORR_ABORT_UNC_NONFATAL_ERR_SIG)
            self.DMESG_BAD_TLP_CORR_ABORT_UNC_NONFATAL_ERR_SIG.pop()
            if not ret_val:
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if RasPcieAerMultipleCorrNonFatalError.main() else Framework.TEST_RESULT_FAIL)
