...
class RasPcieAerNonFatalError(PcieAerCommon):
    """
    Glasgow id : 59691.2

    Inject an uncorrectable/non-fatal training error into the device with header log words 0 1 2 3.
    """
    BIOS_CONFIG_FILE = "ras_pcie_aer_bios_knobs.cfg"
    TEST_CASE_ID = ["G59691", "PI_RAS_PCIe_AER_Non_fatal"]

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new RasPcieAerNonFatalError object

        :param test_log: Used for debug and info messages
        :param arguments: Arguments used in Baseclass
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(RasPcieAerNonFatalError, self).__init__(test_log, arguments, cfg_opts, self.BIOS_CONFIG_FILE)
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(RasPcieAerNonFatalError, self).prepare()
    ...
    def execute(self):
        ...
        """
        bdf_value = []
        self.create_aer_file(type_error=RasErrorType.NON_FATAL, cmd_path=self._aer_inject_tool_path)
        ...
        for item_value in bdf_value:
            ...
            #  Adding targeted bdf value to DMESG Signature
            self.DMESG_COMP_ABORT_UNC_NONFATAL_ERR_SIG.append(self.UNC_NONFATAL_ERR_RECEIVED.format(str("{:02x}".format(item_value))+":00.0"))
            time.sleep(self.WAIT_TIME)
            ret_val = self._os_log_verification_obj.verify_os_log_error_messages(
                __file__, self._os_log_verification_obj.DUT_DMESG_FILE_NAME, self.DMESG_COMP_ABORT_UNC_NONFATAL_ERR_SIG)
            self.DMESG_COMP_ABORT_UNC_NONFATAL_ERR_SIG.pop()
            if not ret_val:
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if RasPcieAerNonFatalError.main() else Framework.TEST_RESULT_FAIL)
