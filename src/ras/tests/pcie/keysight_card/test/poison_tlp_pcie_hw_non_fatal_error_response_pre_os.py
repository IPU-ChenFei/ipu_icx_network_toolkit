...
class PoisonTlpPcieHwErrNonFatalErrorResponsePreOs(KeysightPcieErrorInjectorCommon):
    """
    Glasgow_id : G59909

    The objective of this test is to verify platform response to an Non-Fatal HW error injection from a PCIe card on
    PCI lanes connected to the CPU under a UEFI environment.
    """
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new PoisonTlpPcieHwErrNonFatalErrorResponsPreOs object

        :param test_log: Used for debug and info messages
        :param arguments:
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(PoisonTlpPcieHwErrNonFatalErrorResponsePreOs, self).__init__(test_log, arguments, cfg_opts, self.BIOS_CONFIG_FILE)
    ...
    def execute(self):
        ...
        6. Check the Poison TLP Error status.
        ...
        try:
            ...
            injection_addr = self.get_memory_addr_to_inject(uefi_type=True)

            #  Get requester_bdf_id
            reqid = self.get_requester_bdf_id()

            #  halt the Machine.
            sdp.halt_and_check()

            #  Inject Poison TLP Error.
            key_sight_provider.poisoned_tlp(addy=injection_addr, data=self.POISON_TLP_DATA, reqid=reqid)
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if PoisonTlpPcieHwErrNonFatalErrorResponsePreOs.main() else Framework.TEST_RESULT_FAIL)
