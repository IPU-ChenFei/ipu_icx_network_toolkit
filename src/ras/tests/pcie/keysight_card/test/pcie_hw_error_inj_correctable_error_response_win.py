...
class LcrcCorrectibleErrorResponseWindows(KeysightPcieErrorInjectorCommon):
    """
    Glasgow_id : 59912

    The objective of this test is to verify platform response to a Correctible HW error injection from a
    PCIe card in an Windows OS environment
    """
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new LcrcCorrectibleErrorResponseWindows object

        :param test_log: Used for debug and info messages
        :param arguments:
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(LcrcCorrectibleErrorResponseWindows, self).__init__(test_log, arguments, cfg_opts, self.BIOS_CONFIG_FILE)
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(LcrcCorrectibleErrorResponseWindows, self).prepare()
    ...
    def execute(self):
        ...
        #  Verify in OS
        os_event_log_found = self._os_log_ver_obj.verify_os_log_error_messages(__file__,
                                                                               self._os_log_ver_obj.DUT_WINDOWS_WHEA_LOG,
                                                                               self.LCRC_COR_ERR_WINDOWS)

        if not os_event_log_found:
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if LcrcCorrectibleErrorResponseWindows.main()
