...
class PoisonTlpPcieHwErrNonFatalErrorResponseLinux(KeysightPcieErrorInjectorCommon):
    """
    Glasgow_id : 59810

    The objective of this test is to verify platform response to an Non-Fatal HW error injection from a PCIe card
    connected to the CPU lanes under Linux OS.
    """
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new PoisonTlpPcieHwErrNonFatalErrorResponseLinux object

        :param test_log: Used for debug and info messages
        :param arguments:
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(PoisonTlpPcieHwErrNonFatalErrorResponseLinux, self).__init__(test_log, arguments, cfg_opts,
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(PoisonTlpPcieHwErrNonFatalErrorResponseLinux, self).prepare()
    ...
    def execute(self):
        ...
        addy = self.get_memory_addr_to_inject()

        #  Get reqid
        reqid = self.get_requester_id()

        #  halt the Machine.
        sdp.halt_and_check()

        #  Inject Poison TLP Error.
        key_sight_provider.poisoned_tlp(addy=addy, data=self.POISON_TLP_DATA, reqid=reqid)
        ...
        os_error_log_found = self._os_log_ver_obj.verify_os_log_error_messages(
            __file__, self._os_log_ver_obj.DUT_MESSAGES_FILE_NAME, self.POISONED_TLP_UCF_SIG_LINUX)
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if PoisonTlpPcieHwErrNonFatalErrorResponseLinux.main() else
