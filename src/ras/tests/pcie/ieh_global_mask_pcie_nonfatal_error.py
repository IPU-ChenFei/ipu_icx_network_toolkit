...
class IehGlobalMaskPcieNonFatalError(PcieCommon):
    """
    Glasgow_id : 58517
    This test injects a PCIE Non Fatal Error and verifies the IEH global error status register if error is masked.
    """
    _BIOS_CONFIG_FILE = "pcie_bios_knobs.cfg"

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new IehGlobalMaskPcieNonFatalError object

        :param test_log: Used for debug and info messages
        :param arguments:
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(IehGlobalMaskPcieNonFatalError, self).__init__(test_log, arguments, cfg_opts, self._BIOS_CONFIG_FILE)
    ...
    def prepare(self):
        ...
        if self._cscripts.silicon_cpu_family in self._common_content_lib.SILICON_10NM_CPU:
            self._pcie_telemetry.collect_io_telemetry_to_csv("IehGlobalMaskPcieNonFatalError", 0)
    ...
    def execute(self):
        ...
        2. enable_ieh_mask - Enabling the Mask by Changing the Register Access and
         Reverting the Access after Enabling.
        3. injecting_pcie_non_fatal_error_and_mask - Inject Pcie Non Fatal Error and Check if the Error is Masked

        :return: True
        """
        self.verify_no_current_ieh_errors()
        self.enable_ieh_mask(error_type="non_fatal")
        return self.injecting_pcie_non_fatal_error_and_mask()

    def cleanup(self, return_status):
        # type: (bool) -> None
        """DTAF cleanup"""
        self._pcie_telemetry.collect_io_telemetry_to_csv("IehGlobalMaskPcieNonFatalError", 1)
        self._common_content_lib.perform_graceful_ac_off_on(self.ac_power)
        self.os.wait_for_os(self.reboot_timeout)
        super(IehGlobalMaskPcieNonFatalError, self).cleanup(return_status)

if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if IehGlobalMaskPcieNonFatalError.main() else Framework.TEST_RESULT_FAIL)
