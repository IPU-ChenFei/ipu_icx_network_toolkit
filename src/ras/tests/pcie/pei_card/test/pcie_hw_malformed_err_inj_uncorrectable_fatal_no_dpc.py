...
class PcieHwMalformedErrInjUncorrectableFatalNoDpc(PcieHWLeakyBucketBaseTest):
    """
    Phoenix_id : 22014204669
    ...
    def __init__(self, test_log, arguments, cfg_opts, bios_config_file=None):
        """
        Creates a new PcieHwMalformedErrInjUncorrectableFatalNoDpc object
        ...
        """
        try:
            super(PcieHwMalformedErrInjUncorrectableFatalNoDpc, self).__init__(test_log, arguments, cfg_opts)
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(PcieHwMalformedErrInjUncorrectableFatalNoDpc, self).prepare()
    ...
    def execute(self):
        ...
        for inject_iter in range(2):
            ...
            if not self.os.is_alive():
                ...
                while os_hung_wait_time > 0:
                    time.sleep(self.WAIT_TIME_SEC)
                    if self.os.is_alive():
                        break
                    os_hung_wait_time -= self.WAIT_TIME_SEC
                if not self.os.is_alive():
                    self.sdp_obj.itp.resettarget()
                    self.os.wait_for_os(self.reboot_timeout)

            error_type = err_type + "_no_dpc"
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if PcieHwMalformedErrInjUncorrectableFatalNoDpc.main() else Framework.TEST_RESULT_FAIL)
