...
class VerifyAdddcEnabledInSameDeviceAdjacentBanks(MemMirroringBaseTest):
    """
        Glasgow_id : G62305-ADC(SR)Same_Device_Adjacent_Banks
        This test injects correctable multibit errors till threshold is met and checks whether ADDDC was
        activated and the steps are repeated for same device adjacent banks.
    """
    TEST_CASE_ID = ["G62305", "ADC(SR)Same_Device_Adjacent_Banks"]
    ...
    REGEX_EI_LOG_FILE_MAPPING_OUT_BANK = [r"(0\snew\scorrectable\sHA\serror\sdetected)",
                                          r"(\|\s*0\s*3\s*0\s*0\s*00\s\|)"]
    REGEX_EI_LOG_FILE_MAPPING_OUT_ADJ_BANK = [r"(1\snew\scorrectable\sHA\serror\sdetected)",
                                          r"(\|\s*1\s*2\s*0\s*0\s*00\s\|)"]

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new VerifyAdddcEnabledInSameDeviceAdjacentBanks object
        ...
        super(VerifyAdddcEnabledInSameDeviceAdjacentBanks, self).__init__(test_log, arguments, cfg_opts,
                                                                          bios_config_file)
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(VerifyAdddcEnabledInSameDeviceAdjacentBanks, self).prepare()
    ...
    def execute(self):
        ...
        This method executes below:
            ...
            9.Injects 3 correctable errors to the target rank, adjacent bank from above and verifies After
            injection, correctable error count will increases and overflow flag will be set on the target rank.
            10.Makes the system go. Verifies SMI is seen in the debug/serial output.
            adddc_sparing should be enabled and the spare operation starts and verifies Upon initiating
            the spare copy, the correctable error counter and overflow bit will be cleared by
            HW. PLUS1_RANK0.faildevice fields will indicate the failed DRAM device number
            11.Verifies the bank is mapped out by injecting an error to the same adjacent bank.
            12.Halts, Sets smmexitbreak=0 and resumes the system.
        ...
        try:
            ...
            for i in range(0, 3):
                ...
                self._ei.injectMemError(socket=self.socket_value, channel=self.channel_value,
                                        dimm=self.dimm_slot, rank=self.rank_value, sub_rank=self.sub_rank_value,
                                        bank_group=self.bank_group_value, bank=int(self.bank_value) - 1, errType="ce")
                self._sdp.stop_log()
            ...
            with open(self.EI_LOG_FILE, "r") as ei_log:
                ...
                self._log.info("Extracting the Error info from : {}".format(log_file))
                log_file_check_status = self._common_content_lib.extract_regex_matches_from_file(
                    log_file, self.REGEX_EI_LOG_FILE_MAPPING_OUT_ADJ_BANK)
            if log_file_check_status:
    ...
    def cleanup(self, return_status):
        super(VerifyAdddcEnabledInSameDeviceAdjacentBanks, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(
        Framework.TEST_RESULT_PASS if VerifyAdddcEnabledInSameDeviceAdjacentBanks.main() else Framework.TEST_RESULT_FAIL)
