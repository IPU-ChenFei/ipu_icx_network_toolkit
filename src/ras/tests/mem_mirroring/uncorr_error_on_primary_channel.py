...
class UnCorrErrOnPrimaryChannel(MemMirroringBaseTest):
    """
    Glasgow_id : 59898
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        """
        super(UnCorrErrOnPrimaryChannel, self).__init__(test_log, arguments, cfg_opts, self._BIOS_CONFIG_FILE)
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(UnCorrErrOnPrimaryChannel, self).prepare()
    ...
    def execute(self):
        ...
        6. Inject Un Correctable error on Primary channel.
        ...
        try:
            ...
            if ret_val:
                self._log.info("Un-corrected mirror channel error is detected on Primary Channel")
            else:
                log_err = "Un-Corrected mirror channel error is not detected on Primary Channel"
                self._log.error(log_err)
                raise Exception(log_err)
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if UnCorrErrOnPrimaryChannel.main() else Framework.TEST_RESULT_FAIL)
