...
class CorrErrOnPrimaryChannel(MemMirroringBaseTest):
    """
    Glasgow_id : 59897

    Tests the feature to keep a duplicate (secondary or mirrored) copy of the contents of memory as a redundant
    backup for use if the primary memory fails.
    """
    _BIOS_CONFIG_FILE = "memory_mirroring_bios_knobs.cfg"
    _CE_KLAXON_MEM_SIG = ["valid = 0x1", "cor_err_cnt = 0x1"]
    _CE_KLAXON_M2MEM_SIG = ["mirrorcorrerr [62] = 0x1", "errortype [20:11] = 0xa"]
    _ADDR_SIG = "ADDR "

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new CorrErrOnPrimaryChannel object

        :param test_log: Used for debug and info messages
        :param arguments: Arguments used in Baseclass
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(CorrErrOnPrimaryChannel, self).__init__(test_log, arguments, cfg_opts, self._BIOS_CONFIG_FILE)
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(CorrErrOnPrimaryChannel, self).prepare()
    ...
    def execute(self):
        ...
        try:
            ...
            self._log.info("Inject the correctable error on Primary Channel")
            self._ras_common_obj.inject_memory_error(self._CORE_ADDR, error_type="ce", Inject2ndChannel=False)
            ret_val = self.check_mirroring_error_was_detected(
                self._ras_common_obj._CE_ERROR_ON_PRI_CHANNEL[self._product], True)
            if ret_val:
            ...
            ret_val = self._ras_common_obj.log_klaxon_memory_errors(klaxon_m2mem_file_signature_list=
                                                                    self._CE_KLAXON_M2MEM_SIG,
                                                                    klaxon_mem_error_signature_list=
                                                                    self._CE_KLAXON_MEM_SIG, error_validate_flag=True)
            if not self._os.is_alive():
                self._sdp.pulse_pwr_good()
                self._os.wait_for_os(self._reboot_timeout_in_sec)
            self._ras_common_obj._CE_ERROR_LOG_SIG.append(self._ADDR_SIG + str(hex(self._CORE_ADDR)).replace('0x', ''))
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if CorrErrOnPrimaryChannel.main() else Framework.TEST_RESULT_FAIL)
