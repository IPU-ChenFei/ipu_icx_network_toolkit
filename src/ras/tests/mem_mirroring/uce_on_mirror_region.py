...
class UnCorrErrOnMirrorRegion(MemMirroringBaseTest):
    """
    Glasgow_id : 59661

    This test covers partial mirror for UCE error..
    """
    _BIOS_CONFIG_FILE = "memory_mirroring_bios_knobs.cfg"
    _UC_KLAXON_MEM_SIG = ["0x00000001 : mirrorcorrerr",
                          "0x0000030c : errortype"]
    _UC_KLAXON_M2MEM_SIG = ["corrcount [52:38] = 0x1", "errortype [20:11] = 0xc"]
    _ADDR_SIG = "ADDR "

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new UnCorrErrOnMirrorRegion object

        :param test_log: Used for debug and info messages
        :param arguments: Arguments used in Baseclass
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(UnCorrErrOnMirrorRegion, self).__init__(test_log, arguments, cfg_opts, self._BIOS_CONFIG_FILE)
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(UnCorrErrOnMirrorRegion, self).prepare()
    ...
    def execute(self):
        ...
        6. Inject Un Correctable error on Mirror Region.
        ...
        try:
            ...
            self._log.info("Inject the Uncorrectable Error on Mirror Region")
            ...
            if ret_val:
                self._log.info("Un-corrected error is detected on Mirror Region")
            else:
                log_err = "Un-Corrected error is not detected on Mirror Region"
                self._log.error(log_err)
                raise Exception(log_err)

            ret_val = self._ras_common_obj.log_klaxon_memory_errors(klaxon_m2mem_file_signature_list=
                                            self._UC_KLAXON_M2MEM_SIG, klaxon_mem_error_signature_list=
                                            self._UC_KLAXON_MEM_SIG, error_validate_flag=True)
            self._ras_common_obj._UC_ERROR_ON_PRI_CH_LOG_SIG.append(self._ADDR_SIG +
                                                                    str(hex(self._CORE_ADDR)).replace('0x', ''))
            if not self._os.is_alive():
                self._sdp.pulse_pwr_good()
                self._os.wait_for_os(self._reboot_timeout_in_sec)
            self._log.info("Verify the Os Log....")
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if UnCorrErrOnMirrorRegion.main() else Framework.TEST_RESULT_FAIL)
