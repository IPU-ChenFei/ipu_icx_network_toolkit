...
class MirrorFailover(MemMirroringBaseTest):
    """
    Glasgow_id : 59899
    ...
    """
    _BIOS_CONFIG_FILE = "partial_mirroring_bios_knobs.cfg"
    _SMM_MODE_ENTRY = 1
    _SMM_MODE_EXIT = 0

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new MirrorFailover object

        :param test_log: Used for debug and info messages
        :param arguments: Arguments used in Baseclass
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(MirrorFailover, self).__init__(test_log,
                                             arguments, cfg_opts,
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(MirrorFailover, self).prepare()
    ...
    def execute(self):
        ...
        6. Inject UnCorrectable error on Primary Channel and Again inject un correctable error with failover.
        ...
        """
        try:
            if self.memory_mirroring_utils_obj.get_mirror_status_registers():
                self._log.info("Partial Mirroring is enabled")
            else:
                log_err = "Partial Mirroring is not enabled"
            ...
            self.smmentrybreak_mode_control(self._SMM_MODE_ENTRY)
            self._ras_common_obj.inject_memory_error(err_addr=self._CORE_ADDR, error_type="uce")
            ...
            if ret_val:
                self._log.info("Un-corrected error is detected on Primary Channel")
            else:
                log_err = "Error is not detected"
                self._log.error(log_err)
                raise Exception(log_err)

            self._ras_common_obj.inject_memory_error(err_addr=self._CORE_ADDR, error_type="uce", one_Injection=False)

            ret_val = self.check_mirroring_error_was_detected(self._ras_common_obj._UC_ERR_ON_PRI_FAILOVER[self._product],
                                                 True)
            if ret_val:
                self._log.info("Uncorrected Error on Primary Channel and Mirror Failover detected")
            else:
                log_err = "Error is not detected"
                self._log.error(log_err)
                raise Exception(log_err)
            ...
            self.smmentrybreak_mode_control(self._SMM_MODE_EXIT)
            if not self._os.is_alive():
                self._sdp.pulse_pwr_good()
                self._os.wait_for_os(self._reboot_timeout_in_sec)
            self._os_log_obj.verify_os_log_error_messages(__file__, self._os_log_obj.DUT_MESSAGES_FILE_NAME,
                                                          self._ras_common_obj._MIRROR_FAILOVER_SIGN)
            return ret_val
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if MirrorFailover.main() else
