...
class OutputCommandParser:
    ...
    """
    def __init__(self, task_name: str, input_parser: InputFilesParser):
        print("===== Running output command parser ===== ")
        task = Task(task=task_name.lower(), config=input_parser)
        self.provision_command = task.provision_command
        self.product_versions = task.product_versions
        print("Provision command for task {task}: \n{command}".format(task=task_name.upper(),
