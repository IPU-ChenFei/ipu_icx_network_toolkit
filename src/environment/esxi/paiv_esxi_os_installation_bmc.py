...
version 1.0 23-August-2021 :- Created ESXI BMC VMM Os Installation Script

version 1.1 13-may-2022 :- Created ESXI Auto-installer ready with xmlcli/ssh/powercli
"""
import re
import os
import six
...
class Os_Installation(BaseTestCase):
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        self.os_params = self._os_pre_req_lib.get_sut_inventory_data("esxi")
    ...
    def add_arguments(cls, parser):
        ...
        parser.add_argument('--ATF_ISO_PATH', action="store", dest="atf_iso_path", default="https://ubit-artifactory-ba.intel.com/artifactory/dcg-dea-srvplat-local/Automation_Tools/esxi8AIV1.iso",
    ...
    def download_tool_to_host(self, dest=None):
        ...
        try:
            apath = r"https://ubit-artifactory-ba.intel.com/artifactory/dcg-dea-srvplat-local/Automation_Tools/CI_Tools/KS_CUST.zip"
    ...
    def usb_copy_tools(self):
        ...
        if (self.download_tool_to_host(drive_letter) == True):
            with zipfile.ZipFile(drive_letter + "KS_CUST.zip", 'r') as zip_ref:
            ...
            for i in range(0, 80):
                try:
                    ...
                    if (self._os.is_alive() == True):
                        self._log.info("INSTALLED And Entered Into ESXI OS Successfully")
                        self._phy.disconnect_usb()
                        return True
                    elif (i == 50):
                        self._log.info("ESXI Os Installation Had Issues Waited For 50 Mins")
                        return False
                    else:
                        self._log.debug("ESXI OS Installation Is IN-Progress")
                except:
                    continue
