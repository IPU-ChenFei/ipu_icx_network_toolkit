...
class BmcFlashingBaninoOnline(BaseTestCase):
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        self._phy_red = ProviderFactory.create(redfish_flash_cfg, test_log)
        ...
        bmc_console_cfg = ConfigurationHelper.filter_provider_config(sut=sut, provider_name=r"console",
                                                                      attrib=dict(id="BMC"))
        ...
        self._direct_atf_bmc_path = self._direct_atf_bmc_path.replace("artifactory-ba.intel.com/artifactory/dcg-dea-srvplat-local",
                                              "artifactory.intel.com/artifactory/dcg-dea-srvplat-repos")
        self._log.info("new BMC package path = {0}".format(self._direct_atf_bmc_path))
    ...
    def add_arguments(cls, parser):
        ...
        if (str(platform.architecture()).find("WindowsPE") != -1):
            ...
            if (path.exists("C:\BMC_Image") == True):
                subprocess.check_output("rmdir /q /s C:\BMC_Image", shell=True)
            time.sleep(3)
            subprocess.check_output("mkdir C:\BMC_Image", shell=True)
            print(" --> C:\BMC_Images Folder cleaned up <--")
        ...
        parser.add_argument('--ATF_PASSWORD', action="store", dest="atf_password", default=SysUser.PWD,
                            help="Inorder To Download From Other Artifactory Location give correct")
    ...
    def enter_into_bios(self):
        ...
        if self.setupmenu.wait_for_entry_menu(1500):
            for i in range(0, 3):
    ...
    def get_mount_points(self):
        ...
        if str(platform.architecture()).find("WindowsPE") != -1:
            ...
            if not list_usb_drives:
                self._log.error("Connect the Pendrive to Banino or Check the Banino Connection Pendrive Not Detected")
                sys.exit(False)
            else:
    ...
    def copy_collateral_to_usb_drive(self, collateral_name, usb_mp):
        ...
        host_collateral_path = os.path.join("C:\Automation\Tools\SPR", archive_relative_path,
                                            archive_file_name)
    ...
    ################## New Functions Ends
    def download_tool_to_host(self,dest=None):
        """
        ...
        if (dest == None):
            ...
            if not os.path.exists(dest):
                self._log.error("User Given Path in Host Doesn't Exists Do Check Destination path")
                return False
        start = time.time()
        atf_username = SysUser.USER
        atf_password = SysUser.PWD
        try:
            apath=r"https://ubit-artifactory-ba.intel.com/artifactory/dcg-dea-srvplat-local/Automation_Tools/SPR/Uefi/cmdtool.zip"
            subprocess.check_output(
                "curl --silent -u " + str(atf_username) + ":" + str(atf_password) + " --ssl-no-revoke -X GET " + str(
                    apath) + " --output " + str(path) + "\\" + str(tool_name), shell=True)
            end = time.time()
            zip_extract = (abs(start - end))
            zip_extract = ("{:05.2f}".format(zip_extract))
            self._log.info("Time Taken for This Tool >> {0} << TO Download {1} Seconds".format(tool_name, zip_extract))
            self._log.info("\nTool Downloaded Successfull to this Location {0}".format(path))
            return True
        except Exception as ex:
            self._log.error("Failed to Download cmdtool to Pendrive {0}".format(ex))
            return False

    def usb_copy_tools(self):
        # Swithing USB TO HOST Copying tool Getting the Pendrive Drive letter and Downloading tool from ATF Extract in same Drive
        self.switch_usb_to_host()
        ret = self.get_mount_points()
        drive_letter = ' '.join(map(str, ret)) + r"\\"
        if(self.download_tool_to_host(drive_letter) == True):
            with zipfile.ZipFile(drive_letter + "cmdtool.zip", 'r') as zip_ref:
                zip_ref.extractall(drive_letter)
            self._log.info("CMDEFI Tool Extraction done to Pendrive")
            time.sleep(2)
            self.switch_usb_to_target()
            return True
        else:
            self._log.error("Tools didn't Copy To Pendrive For BMC configuration")
            return False
    ...
    def execute(self):
        ...
        #step-1:
        self._log.info("---------------->  STEP-1 <-----------------")
        if(self.usb_copy_tools() != True):
            self._log.error("ISSUE in Downloading Tools from Artifactory and copying to the USB")
            return False
        if not self.switch_usb_to_host():
            self._log.error("Issue in switching USB to host")
            return False
        ret = self.get_mount_points()
        drive_letter = ' '.join(map(str, ret)) + r"\\"
        drive_path = drive_letter + "EnableUsers.nsh"
        ret = self._phy_red.get_bmc_mac()
        self._log.info("MAC = {0}".format(ret))
        self._log.info("System Testing Passed For Getting BMC MAC")

        with open(drive_path, "a+") as fh:
            set_mac = "\r\ncmdtool.efi 20 c0 a1 0 " + ret.replace(":", " ")
            self._log.info("adding {0} to EnableUsers.nsh file".format(set_mac))
            fh.write(set_mac)
            fh.flush()

        time.sleep(2)
        self._log.info("File has been updated at {}..".format(drive_path))

        if not self.switch_usb_to_target():
            self._log.error("Issue in switching USB to SUT")
            return False

        #step-2:
        self._log.info("---------------->  STEP-2 <-----------------")
        # Flashing BMC Image
        ...
        if(self.enter_into_bios() != True):
            self._log.error("Failed to Enter Into Bios")
            return False
        self.bios_path_navigation('Boot Manager Menu,UEFI Internal Shell')
        self._uefi.wait_for_uefi(1000)
        self._log.info("Platform is in UEFI...")
        time.sleep(10)
        ...
        time.sleep(5)
        self.execute_uefi_cmd("EnableUsers.nsh")
        # wait to complete nsh execution
        time.sleep(150)


        #STEP-3
        self._log.info("---------------->  STEP-3 <-----------------")
        ...
        ret = self._bmc_console.wait_for_login(365)
        ret1 = self._bmc_console.login()
        self._bmc_console.in_console()

        try:
            self._bmc_console.execute('sed -i \'s/#NTP=/NTP=10.91.127.1/g\' /etc/systemd/timesyncd.conf', timeout=10)
        except:
            pass
        time.sleep(4)

        self._log.info("---------------->  STEP-4 <-----------------")

        #STEP-4
        import xml.etree.ElementTree as ET
        tree = ET.parse('C:\Automation\system_configuration.xml')
        root = tree.getroot()
        for child in root.iter('rasp'):
            rasp = child.text
        if rasp.lower() in ["true", "yes", "on", "enable"]:
            rasp = True
        else:
            rasp = False

        if rasp:
            self._log.info("System is configured for RASP system . Fetching BMC IP from system_configuration.xml")
            for child in root.iter('redfish'):
                bmc_ip=(child.find('ip').text)
                break
            self._log.info("bmc_ip = {0}".format(bmc_ip))
        else:
            self._log.info("Banino is Configured For NON-RASP. Fetching BMC IP from BMC console")
            no_of_attempts = 15
            total_attempts = 0
            bmc_ip = None
            while total_attempts < no_of_attempts:
                ret = self._bmc_console.execute(r"ifconfig eth0 | grep 'inet addr:'", timeout=10, end_pattern='Bcast')
                bmc_ip = re.search(r'((2[0-5]|1[0-9]|[0-9])?[0-9]\.){3}((2[0-5]|1[0-9]|[0-9])?[0-9])', ret, re.I) \
                    .group()
                bmcip_header = ["IPA<", ">IPA"]
                bmc_ip_with_header = bmc_ip.join(bmcip_header)
                if len(bmc_ip_with_header) in range(15, 24):
                    self._log.info("BMC IP :: {}".format(bmc_ip))
                    break
                total_attempts += 1
            else:
                self._log.error("Failed to get the BMC IP after {} times of attempts.\nCommand output : {}".
                                format(total_attempts, bmc_ip))
                raise RuntimeError("Error in getting the BMC IP")

        self.switch_usb_to_host()
        ret = self.get_mount_points()
        drive_letter = ' '.join(map(str, ret)) + r"\\"
        ret=subprocess.check_output(
            "curl --user debuguser:0penBmc1 -d @{0}\install_ca.json -k -X POST --header Content-type:application/json " \
            "https://{1}/redfish/v1/Managers/bmc/Truststore/Certificates --silent --proxy \"\" ".format(drive_letter, str(bmc_ip).strip()),
            shell=True)
        if "error" in str(ret):
            self._log.error("Failed To install artifactory certificate. \n{}".format(ret))
            return False
        else:
            self._log.info("Successfully installed the certificate. \n{}".format(ret))
        ret = subprocess.check_output("ping  "+str(bmc_ip),shell=True)
        if("Lost = 0 (0% loss)" in str(ret)):
            self._log.info("Pinging Succesful TO BMC {0} \n\n {1}".format(bmc_ip,ret))
            return True
        else:
            self._log.error("Failure to Ping {0}".format(ret))
            return False
