...
class Os_Installation(BaseTestCase):
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        self.os_params = self._os_pre_req_lib.get_sut_inventory_data("windows")
        ...
        self.dict_ssds = {1: ['E'], 2: ['E', 'F'], 3: ['E', 'F', 'G'], 4: ['E', 'F', 'G', 'H']}
        self.entry_total = 0
        self.previous_wim_img_path = None
        ...
        # software package
        self._initialize_sft_pkg = arguments.initialize_sft_pkg  # True or Flase to initialize software package
        self._direct_sft_pkg_download = arguments.direct_sft_download  # whether to download os image given by user #True or False
                    ...
                    try:
                        ...
                        if (str(platform.architecture()).find("WindowsPE") != -1):
                            if not os.path.exists(cwd):
                                self._log.info("Wim_Imager Tool is Being Downloaded From Artifactory")
                                opt = subprocess.Popen("curl --silent -u " + str(self._atf_username) + ":" + str(
                                    self._atf_password) + " --ssl-no-revoke -X GET https://ubit-artifactory.intel.com/artifactory/dcg-dea-srvplat-repos/Automation_Tools/CI_Tools/wimimager.zip --output wimimager.zip",
                                                       cwd=r"C:\os_package", stdin=PIPE, stdout=PIPE, stderr=STDOUT,
                                                       shell=True)
                                opt = opt.stdout.read()
                ...
                if (Extract == True):
                    ...
                    try:
                        if (self._ostype == "Linux"):
                            ...
                            if (self._host == "windows_host"):
                                start = time.time()
                                ret=subprocess.check_output("echo Y|copy " + str(Dnd_img_path) + " S:", shell=True)
                                time.sleep(50)
                                end = time.time()
                                wim_copy = (abs(start - end))
                                wim_copy = ("{:05.2f}".format(wim_copy))
                                #  Fix if more than one HDD connected, assigned drive letter accordingly.
                                no_of_ssds = self._os_pre_req_lib.get_ssd_names_config("windows")
                                self.entry_total = no_of_ssds
                                ...
                                with open("H:\WimagerUsbCommands.xml", "r+") as f:
                                    ...
                                    try:
                                        text = re.sub("sample.wim",
                                                      r"{}:\\".format(self.dict_ssds[no_of_ssds][-1]) + str(self._base_os_name),
                                                      text)
                                        self.previous_wim_img_path = r"{}:\\".format(self.dict_ssds[no_of_ssds][-1]) + str(self._base_os_name)
                                        self.dict_ssds[no_of_ssds].pop(-1)
                                    except Exception as ex:
    ...
    def automation_post_installation_enabler(self):
        if (os.path.isfile("S:\\" + str(self._rhel_cfg_file_name)) == True):
            ...
            opt = subprocess.Popen("curl --ssl-no-revoke -X GET " + str(self._curl_server) + "/files/" + str(
                self._rhel_cfg_file_name) + " --output " + str(self._rhel_cfg_file_name), stdin=PIPE, stdout=PIPE,
                                   stderr=STDOUT, shell=True)
            opt = opt.stdout.read()
            if (str(opt).find(r"100") != -1):
                self._log.info("Rhel Configuration File Downloaded")
            else:
                self._log.error("Failed To Download Rhel {0} From Curl {1} Server".format(self._rhel_cfg_file_name,
                                                                                          self._curl_server))
                return False
            cwd = os.getcwd()
            if (self._host == "windows_host"):
                subprocess.check_output(
                    "echo y | xcopy " + str(cwd) + "\\" + str(self._rhel_cfg_file_name) + " S:\\" + " /e", shell=True)
            else:
                subprocess.check_output("sudo mv " + str(cwd) + "//" + str(self._rhel_cfg_file_name) + " S://",
                                        shell=True)
            '''
            ...
            with open("S:\\" + str(self._rhel_cfg_file_name), 'w') as file1:
                file1.write(filedata)
            file1.close()
            '''
        else:
            self._log.error("This Is Not Rhel8.1 OS")
    ...
    def counter_check(self):
        ...
        self._log.info("Entered into counter initiative..")
        ...
        no_of_ssds = self._os_pre_req_lib.get_ssd_names_config("windows")
        self.entry_total = no_of_ssds
        self.current_entry_count = self.current_entry_count + 1
        if self.entry_total >= self.current_entry_count:
            if no_of_ssds == 0 or no_of_ssds is None:
                self._log.error("Failed to get the SSD information, reasons could be are listed "
                                "below, \n 1. Seems there are no SSDs connected to the SUT. \n "
                                "2. If Manual - SUT Inventory config file is not updated correctly"
            ...
            with open("H:\WimagerUsbCommands.xml", "r+") as f:
                text = f.read()
                try:
                    text = re.sub(self.previous_wim_img_path,
                                  r"{}:\\".format(self.dict_ssds[no_of_ssds][-1]) + str(self._base_os_name),
                                  text)
                    current_path = r"{}:\\".format(self.dict_ssds[no_of_ssds][-1]) + str(self._base_os_name)
                    self.dict_ssds[no_of_ssds].pop(-1)
                except Exception as ex:
                    self._log.error("Either there are no hard disk found or more than 4 hard disks"
                                    "are connected, please connect only 4 or less and try again..")
        ...
        self._log.info("Entering Into Bios SETUP Page To Select USB For OS-Installation")
        ...
        sut_static_ip = self._os_pre_req_lib.get_sut_static_ip()
        for i in range(0, 200):
            try:
                time.sleep(60)
                cmd_line = "ping {}".format(sut_static_ip)
                process_obj = subprocess.Popen(cmd_line, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                               shell=True)
                stdout, stderr = process_obj.communicate()
                self._log.debug("Return code of the ping command is {}".format(process_obj.returncode))
                self._log.debug("Standard output of the ping command is: \n{}".format(stdout))
                if stderr is not None:
                    self._log.debug("Standard error of the ping command is: \n{}".format(stderr))
                if self._os.is_alive():
                    ...
                    self._log.info("Still Os Installation Had Issues, Starting Counter initiative again if available "
                                   "else the installation will exit now...")
                    return False
                else:
                    self._log.debug("OS Installation Is IN-Progress")
            except:
                continue
    def format_usb_drive_fat32(self, size, format_usb=None, wim=None):
        """
        :- size Pendrive Size accepted size values are 8,16,32,64
           wim for windwos installation it requires a special format of the pendrive into 2 partion, pass yes or True if os that needs to be copied into pendrive is .wim
        return True
                ...
                if ((str(wim)).lower() in ["true", "yes", "on"]):
                    ...
                    try:
                        if six.PY2:
                            ...
                            time.sleep(2)
                            p.stdin.flush()
                            res6 = p.stdin.write(b"create partition primary SIZE=" + str(mb) + " \n")
                            time.sleep(3)
                            p.stdin.flush()
                            res6 = p.stdin.write(b"FORMAT FS=fat32 Label=WIMAGERUSB QUICK \n")
                            time.sleep(6)
                            ...
                            p.stdin.flush()
                            res6 = p.stdin.write(
                                bytes("create partition primary SIZE=" + str(mb) + " \n", encoding='utf8'))
                            time.sleep(3)
                            p.stdin.flush()
                            res6 = p.stdin.write(bytes("FORMAT FS=fat32 Label=WIMAGERUSB QUICK \n", encoding='utf8'))
                            time.sleep(6)
    ...
    def execute(self):
        ...
        if (self.prepare() != False):
            ...
            if (self._os_install == True):
                ...
                # if(self._os.wait_for_os(30000)!=True):
                #     self._log.info("Entered Into OS Successfully")
                #     return True
                # else:
                #     self._log.error("Booting Process In Progress")
                sut_static_ip = self._os_pre_req_lib.get_sut_static_ip()
                for i in range(0, 200):
                    try:
                        time.sleep(60)
                        ...
                        if self._os.is_alive():
                            if process_obj.returncode == 0:
                                ...
                                timer = (abs(start - end))
                                final_time = ("{:05.2f}".format(timer))
                                self._log.info(
                                    "=========> Platform Windows OS Installation Took {0} Seconds <<=======".format(
                                        final_time))
                                return True
                        elif (i == 40):
                            ...
                            counter_result = None
                            self._log.info("Os Installation Had Issues, Starting Counter initiative...")

                            for find_letter in range(0, self.entry_total-1):
                                counter_result = self.counter_check()
    ...
    def cleanup(self, return_status):  # type: (bool) -> None
        # Format pen drive to FAT32
        self.format_usb_drive_fat32(self._usb_size, wim=True)
        super(Os_Installation, self).cleanup(return_status)
