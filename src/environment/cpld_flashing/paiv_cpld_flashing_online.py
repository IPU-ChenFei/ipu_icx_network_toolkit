...
version 2.0 26-oct-2021 :- Added workaround for Flashing issue for stability
...
class Os_Installation(BaseTestCase):
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        cpld_flash_cfg = ConfigurationHelper.filter_provider_config(sut=sut, provider_name=r"flash",
                                                                       attrib=dict(id="3"))
        cpld_flash_cfg = cpld_flash_cfg[0]
        self._flash = ProviderFactory.create(cpld_flash_cfg, test_log)
        ...
        self._os_install = False  # for flashing cpld image alone
        ...
        self._enable_cpld_flashing = True
        self._cpldpath = arguments.CPLDPATH
        self._cpldfile = arguments.CPLDFILE
        ...
        # self._cpld_dnd_extract = arguments.img_dnd_extract    #True or Flase by default True
        self._direct_cpld_pkg_download = True  # whether to download cpld from ATF given by user #True or False
        self._direct_atf_cpld_main_path = arguments.atf_cpld_main_path  # full atf main cpld_pkg path along with file name extension usually .zip
        self._direct_atf_cpld_main_path = self._direct_atf_cpld_main_path.replace("artifactory-ba.intel.com/artifactory/dcg-dea-srvplat-local",
                                              "artifactory.intel.com/artifactory/dcg-dea-srvplat-repos")
        self._log.info("new CPLD main package path = {0}".format(self._direct_atf_cpld_main_path))
        self._direct_atf_cpld_sec_path = arguments.atf_cpld_sec_path  # full atf secondary cpld_pkg path along with file name extension usually .zip
        self._direct_atf_cpld_sec_path = self._direct_atf_cpld_sec_path.replace("artifactory-ba.intel.com/artifactory/dcg-dea-srvplat-local",
                                              "artifactory.intel.com/artifactory/dcg-dea-srvplat-repos")
        self._log.info("new CPLD Secondary package path = {0}".format(self._direct_atf_cpld_sec_path))

        # target location where it needs to be copied
        self._local_cpld_host_mode = False
        self._local_cpld_host_path = arguments.cpld_img_dwnd_host_path # In this passed location cpld package resides pre-downloaded
    ...
    def prepare(self):
        # validation on CPLD package mode
        if ((str(self._local_cpld_host_mode)).lower() in ["true", "yes", "on", "enable"]):
            self._local_cpld_host_mode = True
            if (self._local_cpld_host_mode and self._direct_cpld_pkg_download):
                self._log.error(
                    "Either Already Available Package or Newly Download From Artifactory Either one option can be Made True")
                return False
            if not self._local_cpld_host_path:
                self._log.error("Give In The lOCATION OF Pre-Downloaded cpld ZIP LOCATION")
                return False

        if ((str(self._direct_cpld_pkg_download)).lower() in ["true", "yes", "on", "enable"]):
            self._direct_cpld_pkg_download = True
            if not self._direct_atf_cpld_main_path or not self._direct_atf_cpld_sec_path:
    ...
    def add_arguments(cls, parser):
        ...
        cpldpath = os.environ.get("CPLDPATH")
        cpldfile = os.environ.get("CPLDFILE")
        ...
        if (str(platform.architecture()).find("WindowsPE") != -1):
            host = "windows_host"
            cpld_img_hostpath = "C:\CPLD_Image\\"
            sft_and_baseos = "C:\OS\\"
            # host sanity check for folders
            if (path.exists("C:\CPLD_Image") != True):
                subprocess.check_output("mkdir C:\CPLD_Image", shell=True)
            else:
                subprocess.check_output("rmdir /s /q C:\CPLD_Image", shell=True)
                time.sleep(2)
                subprocess.check_output("mkdir C:\CPLD_Image", shell=True)
        elif (str(platform.architecture()).find(r"ELF") != -1):
            host = "rpi_linux_host"
            cpld_img_hostpath = "/home/pi/Public/firmware/chipwrite/"
        ...
        ##CPLD PACKAGE
        # --------------------- Whether To Download and Use ALready Existing CPLD Package
        parser.add_argument('--ENABLEFLASHING', action="store", dest="ENABLE_CPLD_FLASHING", default=enableflashing,
                            help="Incase CPLD .bin Needs To Be Flashed On Platform Give True By Default False it WIll Be False")
        parser.add_argument('--CPLDPATH', action="store", dest="CPLDPATH", default=cpldpath)  # owr
        parser.add_argument('--CPLDFILE', action="store", dest="CPLDFILE", default=cpldfile)  # owr
        # --------------------- FOR DIRECT Download of CPLD PACKAGE
        parser.add_argument('--DIRECT_CPLD_PACKAGE_DOWNLOAD_MODE', action="store", dest="direct_cpld_download",
                            default=False,
                            help="InOrder TO Directly Download Ifwi-Package From ATF Make It True,by Default False")
        parser.add_argument('--ATF_PATH_CPLD_MAIN_PKG', action="store", dest="atf_cpld_main_path", default=None,
                            help="InOrder TO Directly Download Ifwi-Package From ATF Make It True,by Default False")
        parser.add_argument('--ATF_PATH_CPLD_SECONDARY_PKG', action="store", dest="atf_cpld_sec_path", default=None,
                            help="InOrder TO Directly Download Ifwi-Package From ATF Make It True,by Default False")
        # --------------------- FOR Custom RE-Use of CPLD PACKAGE In Local Machine
        parser.add_argument('--LOCAL_CPLD_PACKAGE_MODE', action="store", dest="pre_downloaded_cpld_pkg_mode",
                            default=None,
                            help="Inorde TO Make Use Of Already Existing Predownloaded CPLD Package From Local Hostmachine, Make It True, By Default it will be False")  # true will fetch the image location along sft img_name passed by user
        parser.add_argument('--LOCAL_CPLD_IMG_PATH', action="store", dest="cpld_img_dwnd_host_path",
                            default= cpld_img_hostpath,
                            help="Incase already Exisitng CPLD Image on Local Drive Needs To Be Used Give Full cpld path Location")  # offline flashing
        # parser.add_argument('--DND_EXTRACT', action = "store", dest = "img_dnd_extract",default = True,help="Extract CPLD Image TO USB,By Default It will be True")
        ...
        # --CPLDFILE Name of the CPLD Image with .bin Extension
        # For Direct Parameter passing no need of Artifactory usage  --CPLD_IMG_PATH xxxxx --CPLDFILE xxxxx --Dnd_Extract False
        ...
        if (self._initialize_sft_pkg == True):
            ...
            if (self._direct_sft_pkg_download == True):
                try:
                    ...
                    subprocess.check_output(
                        "curl -u " + str(self._atf_username) + ":" + str(self._atf_password) + " --ssl-no-revoke -X GET " + str(
    ...
    def platform_main_cpld_flashing(self):
        if ((str(self._enable_cpld_flashing)).lower() in ["true", "yes", "on"]):
            # Downloading and Extracting CPLD-Image Alone
            if (self._direct_cpld_pkg_download == True):  # cpld DOWNLAOD from ATF
                try:
                    self._log.info("Method-1 Direct Download Of CPLD Package From Artifactory {0}".format(
                        self._direct_cpld_pkg_download))
                    self._log.info("This Is the location of CPLD Package Artifactory Location {0}".format(
                        self._direct_atf_cpld_main_path))
                    head, tail = ntpath.split(self._direct_atf_cpld_main_path)
                    self._cpld_pkg_name = tail
                    '''
                    #Checking and Removing Older version of same name if any
                    try:
                        if(self._host == "windows_host"):
                            subprocess.check_output("del /f "+str(self._local_cpld_host_path)+str(self._cpld_pkg_name),shell=True)
                        elif(self._host == "rpi_linux_host"):
                            subprocess.check_output("sudo rm -rf "+str(self._local_cpld_host_path)+str(self._cpld_pkg_name),shell=True)
                    '''
                    if (self._host == "windows_host"):
                        ret = subprocess.check_output("cd " + str(self._local_cpld_host_path) + "&& curl -u " + str(
                            self._atf_username) + ":" + str(self._atf_password) + " --ssl-no-revoke -X GET " + str(
                            self._direct_atf_cpld_main_path) + " --output " + str(self._cpld_pkg_name), shell=True)
                    else:
                        ret = subprocess.check_output("cd " + str(self._local_cpld_host_path) + "; curl -u " + str(
                            self._atf_username) + ":" + str(self._atf_password) + " --ssl-no-revoke -X GET " + str(
                            self._direct_atf_cpld_main_path) + " --output " + str(self._cpld_pkg_name), shell=True)
                    self._log.info("Direct Download Of CPLD Package From Artifactory Successfull")
                except Exception as ex:
                    self._log.error(
                        "{0} Artifactory Path Is Not Given Properly and Please Check ATF Usernme {1} and Password {2} {3}".format(
                            self._direct_cpld_pkg_download, self._atf_username, self._atf_password, ex))
                    return False
            elif (self._local_cpld_host_mode == True):  # for making usee of already #pre downlaoded package
                self._log.info(
                    "Method-2 Pre-Downloaded CPLD Package in Local Machine {0}".format(self._local_cpld_host_path))
                head, tail = ntpath.split(self._local_cpld_host_path)
                self._cpld_pkg_name = tail
                if (".bin" in self._cpld_pkg_name):
                    if (path.exists(self._local_cpld_host_path) == True):
                        self._log.info("User Has Given Extracted CPLD-zip and Given Direct .bin Image {0}".format(
                            self._cpld_pkg_name))
                        self._cpldfile = self._cpld_pkg_name
                    else:
                        self._log.error(
                            "User-Given Location {0} CPLD File Deosn't Exists, Provide Correct Location Of CPLD Image".format(
                                self._local_cpld_host_path))
                        return False
                elif (".zip" in self._cpld_pkg_name):
                    self._log.info("User Has Given CPLD Image Zip File Directly {0}".format(self._cpld_pkg_name))
                    if (path.exists(self._local_cpld_host_path) == True):
                        with zipfile.ZipFile(str(self._local_cpld_host_path), 'r') as zip_ref:
                            zip_ref.extractall(head)
                        if (self._host == "windows_host"):
                            latest_cpld_file = str(head) + "\*.bin"
                        else:
                            latest_cpld_file = str(head) + "/*.bin"
                        latest_cpld_img = glob.glob(latest_cpld_file)
                        self._cpldfile = max(latest_cpld_img, key=os.path.getctime)
                        head, file_name = ntpath.split(self._cpldfile)
                        self._cpldfile = file_name
                        self._local_cpld_host_path = head
                        self._log.info("CPLD Image Method-2 {0} ".format(self._cpldfile))
                elif (".7z" in self._cpld_pkg_name):
                    self._log.info("User Has Given CPLD Image 7z File Directly {0}".format(self._cpld_pkg_name))
                    self._log.info("CPLD Image Method-3 {0} ".format(self._cpldfile))
                else:
                    self._log.error(
                        "User Has Given only the file location {0} No specific file was Given, Please Given Full File path also with the File Name".format(
                            self._local_cpld_host_path))
                    return False
                self._log.info("Pre-Downloaded CPLD From Local Computer {0}".format(self._cpld_pkg_name))
            else:
                # Download Begins For CPLD IMG
                self._log.info("Method -3 OWR Triggerd and Downloaded Software Package FROM Artifactory")
                Atf_img_path = str(self._basepath) + str(self._cpldpath) + str(
                    self._cpldfile)  # if future CPLD path Change is Required
                self._log.info("This Is the location of CPLD Artifactory Location {0}".format(Atf_img_path))
                try:
                    if (self._host == "windows_host"):
                        ret = subprocess.check_output("cd " + str(self._local_cpld_host_path) + "&& curl -u " + str(
                            self._atf_username) + ":" + str(self._atf_password) + " --ssl-no-revoke -X GET " + str(
                            Atf_img_path) + " --output " + str(self._cpldfile), shell=True)
                    else:
                        ret = subprocess.check_output("cd " + str(self._local_cpld_host_path) + "; curl -u " + str(
                            self._atf_username) + ":" + str(self._atf_password) + " --ssl-no-revoke -X GET " + str(
                            Atf_img_path) + " --output " + str(self._cpldfile), shell=True)
                    self._cpld_pkg_name = self._cpldfile
                except:
                    self._log.error("Failed Due To Path From Artifactory Not Given {0}".format(sys.exc_info()[0]))
                    return False
            time.sleep(5)
            Dnd_img_path = str(self._local_cpld_host_path) + str(self._cpld_pkg_name)
            if (self._local_cpld_host_mode != True):
                self._log.info(Dnd_img_path)

                if (path.exists(Dnd_img_path) == True):
                    if (zipfile.is_zipfile(Dnd_img_path) == True):
                        self._log.info("CPLD .zip Image Download Successful")
                    elif ".pof" in Dnd_img_path:
                        self._log.info("CPLD .pof Image Download Successful")
                    else:
                        self._log.error(
                            "Downloaded File Is Not A CPLD ZIP Image File,Artifactory Path Is Not Given Properly")
                        return False
                else:
                    self._log.error("Download Didn't Happen, File DoesNot Exists")
                    return False
                time.sleep(5)
                if ".zip" in Dnd_img_path:
                    with zipfile.ZipFile(Dnd_img_path, 'r') as zip_ref:
                        zip_ref.extractall(str(self._local_cpld_host_path))
                elif ".pof" in Dnd_img_path:
                    head, tail = ntpath.split(Dnd_img_path)
                    self._cpld_pkg_name = tail

                    if (".pof" in self._cpld_pkg_name):
                        if (path.exists(self._local_cpld_host_path) == True):
                            self._log.info("User Has Given Extracted CPLD-zip and Given Direct .pof Image {0}".format(
                                self._cpld_pkg_name))
                            self._cpldfile = self._cpld_pkg_name
                        else:
                            self._log.error(
                ...
                if (self._host == "windows_host"):
                    try:
                        if ".zip" in str(Dnd_img_path):
                            subprocess.check_output("del /f " + str(Dnd_img_path), shell=True)
                            self._log.info("CPLD Zip Image Deleted After Extraction")
                    except:
                        self._log.error("CPLD Zip/.pof file Couldn't Be Delted After Extraction {0}".format(sys.exc_info()[0]))
                        return False
                elif (self._host == "rpi_linux_host"):
                    try:
                        if ".zip" in str(Dnd_img_path):
                        ...
                        self._log.error("CPLD Zip/.pof file Couldn't Be Delted After Extraction")
                        return False
                else:
                    self._log.error(
                        "Currently Supported Host Machines Are Microsoft Based OS and Debian Raspberry pi OS")

                self._cpldfile = self.get_cpld_pof_name(self._local_cpld_host_path, "Main")
            self._log.info("CPLD Image {0} Is Going To Get Flashed On Platform".format(self._cpldfile))

            # Flashing Image
            if (self._flash.flash_image(self._local_cpld_host_path, self._cpldfile,target="cpld1") == True):
                self._log.info("CPLD Image {0} Flashing Successfull".format(self._cpldfile))
            else:
                self._log.info("Unable To Do Flashing CPLD Image")
                return False
    ...
    def platform_secondary_cpld_flashing(self):
        if ((str(self._enable_cpld_flashing)).lower() in ["true", "yes", "on"]):
            # Downloading and Extracting CPLD-Image Alone
            if (self._direct_cpld_pkg_download == True):  # cpld DOWNLAOD from ATF
                try:
                    self._log.info("Method-1 Direct Download Of CPLD Package From Artifactory {0}".format(
                        self._direct_cpld_pkg_download))
                    self._log.info("This Is the location of CPLD Package Artifactory Location {0}".format(
                        self._direct_atf_cpld_sec_path))
                    head, tail = ntpath.split(self._direct_atf_cpld_sec_path)
                    self._cpld_pkg_name = tail
                    '''
                    #Checking and Removing Older version of same name if any
                    try:
                    ...
                    if (self._host == "windows_host"):
                        ret = subprocess.check_output("cd " + str(self._local_cpld_host_path) + "&& curl -u " + str(
                            self._atf_username) + ":" + str(self._atf_password) + " --ssl-no-revoke -X GET " + str(
                            self._direct_atf_cpld_sec_path) + " --output " + str(self._cpld_pkg_name), shell=True)
                    else:
                        ret = subprocess.check_output("cd " + str(self._local_cpld_host_path) + "; curl -u " + str(
                            self._atf_username) + ":" + str(self._atf_password) + " --ssl-no-revoke -X GET " + str(
                            self._direct_atf_cpld_sec_path) + " --output " + str(self._cpld_pkg_name), shell=True)
            ...
            if (self._local_cpld_host_mode != True):
                ...
                if (path.exists(Dnd_img_path) == True):
                    if zipfile.is_zipfile(Dnd_img_path) == True:
                        self._log.info("CPLD .zip Image Download Successfull")
                    elif ".pof" in Dnd_img_path:
                        self._log.info("CPLD .pof Image Download Successfull")
                    else:
                        self._log.error(
                            "Downloaded File Is Not A CPLD ZIP Image File,Artifactory Path Is Not Given Properly")
                        return False
                ...
                if (self._host == "windows_host"):
                    try:
                        ...
                        self._log.error(
                            "CPLD Zip/.pof file Couldn't Be Delted After Extraction {0}".format(sys.exc_info()[0]))
                        return False
                elif (self._host == "rpi_linux_host"):
                    try:
                        if ".zip" in str(Dnd_img_path):
                        ...
                        self._log.error("CPLD Zip Couldn't Be Delted After Extraction")
                        return False
                else:
                    self._log.error(
                        "Currently Supported Host Machines Are Microsoft Based OS and Debian Raspberry pi OS")
                self._log.info("CPLD Zip Image Deleted After Extraction")
                self._cpldfile = self.get_cpld_pof_name(self._local_cpld_host_path, "Second")
            self._log.info("CPLD Image {0} Is Going To Get Flashed On Platform".format(self._cpldfile))

            # Flashing Image
            if (self._flash.flash_image(self._local_cpld_host_path, self._cpldfile,target="cpld2") == True):
                self._log.info("CPLD Image {0} Flashing Successfull".format(self._cpldfile))
            else:
                self._log.info("Unable To Do Flashing CPLD Image")
                return False
    ...
    def platform_ac_power_on(self):
        ...
        if self._ac.get_ac_power_state() == True:
            ...
            self._log.error("Failed To Platform(SUT) AC-power TURNED-ON Confrimation")
            return False
    ...
    def enter_into_bios(self):
        ...
        ret=self.setupmenu.wait_for_entry_menu(4000)
        for i in range(0, 10):
    ...
    def bios_cpld_ver_verification(self):
        """
        Function to verify the flashed CPLD image in BIOS.
        """
        ...
        start_match = self._cpld_pkg_name.split(".")[0]
        end_match = self._cpld_pkg_name.split(".")[-2].split("_")[0]

        cpld_ver_bios = ret_bios_info.result_value[0][2].split()[0]
        bios_name = cpld_ver_bios.split(".")

        start_bios_match = bios_name[0]
        temp_end_bios_match = bios_name[-1]
        end_bios_match = temp_end_bios_match[int("-{}".format(len(end_match))):]
        self._log.info("CPLD image version flashed from local: {}".format(self._cpld_pkg_name))

        self._log.info("CPLD image version from BIOS page is: {}".format(cpld_ver_bios))

        if (str(start_match).lower() in str(start_bios_match).lower()):
            if (str(end_match).lower() in str(end_bios_match).lower()):
                self._log.info("Successfully verified the flashed CPLD image version..")
                return True
        self._log.error("CPLD image version verification failed..")
        return False

    def get_cpld_pof_name(self, cpld_oss_pkg_path, chip_name):
        """
        Function to find the correct .pof file from the OSS site under CPLD ingredient
        """
        if cpld_oss_pkg_path:
            list_file_names = os.listdir(cpld_oss_pkg_path)
            for file in list_file_names:
                if re.search(r"\.pof$", file):
                    file_name = file
                    return file_name
                else:
                    self._log.error("Multiple .pof Images Found")
        else:
            self._log.error("CPLD Image Path Issue")


    def execute(self):
        self._log.info("{0} {1} {2} {3} {4}".format(self._ostype, self._cpldpath, self._cpldfile, self._imagetype,
                                                    self._driverpkgpath))
        self._log.info(
            "{0} {1} {2} {3} {4}".format(self._enable_cpld_flashing, self._imagefile, self._local_cpld_host_path,
        ...
        for i in range(0,5):
            ...
            time.sleep(10)
            # Flashing CPLD Image
            ret = self.platform_main_cpld_flashing()
            if (ret == True):
                self._log.info("CPLD_Image Flashing on Main Chip Was Successful")
            elif (ret == "Flashing_Disabled"):
                self._log.info("CPLD-Flashing Parameter is Given as False, CPLD_Image Flashing Will not Happen")
            else:
                self._log.info("CPLD-Flashing Failed")
                return False

            # Flashing CPLD Image
            ret = self.platform_secondary_cpld_flashing()
            if (ret == True):
                self._log.info("CPLD_Image Flashing on Secondary chip Was Successful")
            elif (ret == "Flashing_Disabled"):
                self._log.info("CPLD-Flashing Parameter is Given as False, CPLD_Image Flashing Will not Happen")
            else:
                self._log.info("CPLD-Flashing Failed")
            ...
            if (self.enter_into_bios() != True):
                self._log.error(
                    "CPLD Image Flashed But Didn't Enter Into Bios_Setup Page,Stuck at Post-code 0.2")
                self._log.error("Trying CPld Flashing {0} Attempt since it Failed To BOOT".format(i))
                continue
            return True
