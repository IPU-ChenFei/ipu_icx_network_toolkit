...
version 1.0 01-july-2022 :- Created Os Installation Script
...
from src.lib.tools_constants import SysUser   #username password from environement variable pickup
...
class cpld_Flashing(BaseTestCase):
    def __init__(self, test_log, arguments, cfg_opts):
        super(cpld_Flashing, self).__init__(test_log, arguments, cfg_opts)
    ...
    def add_arguments(cls, parser):
        super(cpld_Flashing, cls).add_arguments(parser)
        ...
        parser.add_argument('--ATF_PATH_CPLD', action="store", dest="atf_cpld_path", default=None,
                            help="InOrder TO Directly Download Ifwi-Package From ATF Make It True,by Default False")
                        ...
                        if (str(platform.architecture()).find("WindowsPE") != -1):
                            if not os.path.exists(cwd):
                                ...
                                opt = subprocess.Popen(
                                    "curl -X GET " + str(
                                        self._curl_server) + "/files/wimimager.zip --output wimimager.zip",
    ...
    def platform_cpld_flashing(self):
            ...
            if (self._local_cpld_host_mode != True):
                ...
                if ".7z" in Dnd_img_path:
                    with py7zr.SevenZipFile(Dnd_img_path, mode='r') as z:
                        z.extractall(str(self._local_cpld_host_path))
                ...
                # latest_cpld_img = glob.glob(str(self._local_cpld_host_path) + "*.bin")
                # self._cpldfile = max(latest_cpld_img, key=os.path.getctime)
                head, tail = ntpath.split(Dnd_img_path)
                path_cpld= self._local_cpld_host_path+"\\"+tail.strip(".zip") #expected a folder
                if (os.path.exists(path_cpld) == True):
                    self._log.info("After Extraction Folder Was present")
                else:
                    self._log.info("After Extraction No Folder Was present Direct Firmware files only")
                    path_cpld = self._local_cpld_host_path  #didn't expect any folder
                ret=[]
                ret=self.get_cpld_oob_bin_name(path_cpld)
                self._log.info("cpld Image {0} from Path {1} Is Going To Get Flashed On Platform".format(ret,path_cpld))
                # Flashing Image
                if (self._flash.flash_image(path_cpld, ret, target="cpld") == True):
                    self._log.info("cpld Image {0} Flashing Successfull".format(ret))
            #post flashing wake up platform
            time.sleep(60)
    ...
    def get_cpld_oob_bin_name(self, cpld_oss_pkg_path):
        """
        Function to find the correct .oob.bin file from the OSS site under cpld ingredient
        """
        if cpld_oss_pkg_path is not None:
            list_file_names = os.listdir(cpld_oss_pkg_path)
            k = []
            for file in list_file_names:
                if re.search(r"capsule", file):
                    file_name1 = file
                    if (".bin" in str(file_name1)):
                        s1 = file_name1
                        return (s1)
            self._log.error(
                "No cpld Capsule Firmware File Found for mentioned keytags capsule in this Location {0}".format(
                    cpld_oss_pkg_path))
            return False
    ...
    def platform_ac_power_off(self):
        ...
        if self._phy.get_power_state() == "G3":
    ...
    def platform_ac_power_on(self):
        if self._ac.ac_power_on() == True:
            ...
            return False
        time.sleep(70)
        if self._phy.get_power_state() == "S0":
    ...
    def verify_cpld_version(self):
        time.sleep(60)
        ret = self._flash.current_cpld_version_check()
        if (ret[0] == True):
            self._log.info("Current cpld version {0} ".format(ret[1]))
            self._log.info("Flashed cpld version {0}".format(self._cpldfile))

            flashed_cpld_version = re.findall("egs-\d*.\d*-\d*-", ret[1])
            local_cpld_version = re.findall("egs-\d*.\d*-\d*-", self._cpldfile)

            if str(flashed_cpld_version[0]) != str(local_cpld_version[0]):
                self._log.error("Flashed cpld version is not same as the current version..")
                return False

            self._log.info("Successfully verified the Flashed cpld version with the current version..")

        else:
            self._log.error("cpld version verification Failed..")
            return False
    ...
    def execute(self):
        ...
        # Flashing cpld Image
        try:
            ret = self._flash.get_current_firmware_version(firmware="cpld")
            self._log.info("Before Flashing cpld Image {0}".format(ret[1]))
        except:
            self._log.error("Either Bmc is not Confiured for Expected debuguser Credential or IPissue")
            return False
        ret = self.platform_cpld_flashing()
        if (ret == True):
            self._log.info("cpld Capsule Image Flashing TO Platform Was Succesful")
            ret = self._flash.get_current_firmware_version(firmware="cpld")
            self._log.info("After Flashing cpld Image {0}".format(ret[1]))
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if cpld_Flashing.main() else Framework.TEST_RESULT_FAIL)
