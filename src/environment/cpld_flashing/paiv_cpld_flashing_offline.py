...
class Os_Installation(BaseTestCase):
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        self._direct_cpld_pkg_download = False  # whether to download cpld from ATF given by user #True or False
        self._direct_atf_cpld_path = arguments.atf_cpld_path  # full atf cpld_pkg path along with file name extension usually .zip
        # target location where it needs to be copied
        self._local_cpld_host_mode = True
        self._local_cpld_main_host_path = arguments.cpld_main_img_dwnd_host_path # In this passed location cpld Main package resides pre-downloaded
        self._local_cpld_sec_host_path = arguments.cpld_sec_img_dwnd_host_path  # In this passed location cpld Sec package resides pre-downloaded
    ...
    def prepare(self):
        ...
        if ((str(self._local_cpld_host_mode)).lower() in ["true", "yes", "on", "enable"]):
            ...
            if (self._local_cpld_host_mode and self._direct_cpld_pkg_download):
                ...
                return False
            if not self._local_cpld_main_host_path:
        ...
        if ((str(self._direct_cpld_pkg_download)).lower() in ["true", "yes", "on", "enable"]):
            ...
            if not self._direct_atf_cpld_path:
    ...
    def add_arguments(cls, parser):
        ...
        if (str(platform.architecture()).find("WindowsPE") != -1):
            host = "windows_host"
            cpld_main_img_hostpath = "C:\CPLD_Image\\"
            cpld_sec_img_hostpath = "C:\CPLD_Image\\"
            sft_and_baseos = "C:\OS\\"
            ...
            host = "rpi_linux_host"
            cpld_main_img_hostpath = "/home/pi/Public/firmware/chipwrite/"
            cpld_sec_img_hostpath = "/home/pi/Public/firmware/chipwrite/"
        ...
        parser.add_argument('--ATF_PATH_CPLD_PKG', action="store", dest="atf_cpld_path", default=None,
                            help="InOrder TO Directly Download Ifwi-Package From ATF Make It True,by Default False")
        ...
        parser.add_argument('--LOCAL_CPLD_MAIN_IMG_PATH', action="store", dest="cpld_main_img_dwnd_host_path",
                            default=cpld_main_img_hostpath,
                            help="Incase already Exisitng CPLD Image on Local Drive Needs To Be Used Give Full cpld path Location")  # offline Main file flashing
        parser.add_argument('--LOCAL_CPLD_SECONDARY_IMG_PATH', action="store", dest="cpld_sec_img_dwnd_host_path",
                            default=cpld_sec_img_hostpath,
                            help="Incase already Exisitng CPLD Image on Local Drive Needs To Be Used Give Full cpld path Location")  # offline Secondary file flashing
    ...
    def platform_main_cpld_flashing(self):
        if ((str(self._enable_cpld_flashing)).lower() in ["true", "yes", "on"]):
            ...
            if (self._direct_cpld_pkg_download == True):  # cpld DOWNLAOD from ATF
                try:
                    ...
                    self._log.info("This Is the location of CPLD Package Artifactory Location {0}".format(
                        self._direct_atf_cpld_path))
                    head, tail = ntpath.split(self._direct_atf_cpld_path)
                    self._cpld_pkg_name = tail
                    '''
                    #Checking and Removing Older version of same name if any
                    try:
                        if(self._host == "windows_host"):
                            subprocess.check_output("del /f "+str(self._local_cpld_main_host_path)+str(self._cpld_pkg_name),shell=True)
                        elif(self._host == "rpi_linux_host"):
                            subprocess.check_output("sudo rm -rf "+str(self._local_cpld_main_host_path)+str(self._cpld_pkg_name),shell=True)
                    '''
                    if (self._host == "windows_host"):
                        ret = subprocess.check_output("cd " + str(self._local_cpld_main_host_path) + "&& curl -u " + str(
                            self._atf_username) + ":" + str(self._atf_password) + " -X GET " + str(
                            self._direct_atf_cpld_path) + " --output " + str(self._cpld_pkg_name), shell=True)
                    else:
                        ret = subprocess.check_output("cd " + str(self._local_cpld_main_host_path) + "; curl -u " + str(
                            self._atf_username) + ":" + str(self._atf_password) + " -X GET " + str(
                            self._direct_atf_cpld_path) + " --output " + str(self._cpld_pkg_name), shell=True)
                ...
                self._log.info(
                    "Method-2 Pre-Downloaded CPLD Package in Local Machine {0}".format(self._local_cpld_main_host_path))
                head, tail = ntpath.split(self._local_cpld_main_host_path)
                self._cpld_pkg_name = tail
                self._local_cpld_main_host_path = head + "\\"
                if (".pof" in self._cpld_pkg_name):
                    if (path.exists(self._local_cpld_main_host_path) == True):
                        self._log.info("User Has Given Extracted CPLD-zip and Given Direct .pof Image {0}".format(
                            self._cpld_pkg_name))

                        if "Main".lower() in str(self._cpld_pkg_name).lower():
                            self._cpldfile = self._cpld_pkg_name
                            Dnd_img_path = str(self._local_cpld_main_host_path) + str(self._cpld_pkg_name)
                            self._log.info(Dnd_img_path)
                        else:
                            self._log.error("Unable to find Main .pof file In CPLD Zip File")
                            return False
                    else:
                        self._log.error(
                            "User-Given Location {0} CPLD File Deosn't Exists, Provide Correct Location Of CPLD Image".format(
                                self._local_cpld_main_host_path))
                        return False
                    ...
                    if (path.exists(self._local_cpld_main_host_path) == True):
                        with zipfile.ZipFile(str(self._local_cpld_main_host_path) + str(self._cpldfile), 'r') as zip_ref:
                            zip_ref.extractall(str(self._local_cpld_main_host_path))

                        self._cpldfile = self.get_cpld_pof_name(self._local_cpld_main_host_path, "Main")
            ...
            if (self._local_cpld_host_mode != True):
                ...
                if (path.exists(Dnd_img_path) == True):
                    if (zipfile.is_zipfile(Dnd_img_path) == True):
                        self._log.info("CPLD Image Download Successfull")
                    else:
                        self._log.error(
                ...
                if (self._host == "windows_host"):
                    try:
                        ...
                        self._log.error("CPLD Zip Couldn't Be Delted After Extraction {0}".format(sys.exc_info()[0]))
                        return False
                elif (self._host == "rpi_linux_host"):
                    try:
                ...
                latest_cpld_img = glob.glob(str(self._local_cpld_main_host_path) + "*.pof")
                self._cpldfile = max(latest_cpld_img, key=os.path.getctime)
                head, tail = ntpath.split(self._cpldfile)
                self._cpldfile = tail
            self._log.info("CPLD Image {0} Is Going To Get Flashed On Platform".format(self._cpldfile))

            # Flashing Image
            if (self._flash.flash_image(self._local_cpld_main_host_path, self._cpldfile,target="cpld1") == True):
                self._log.info("CPLD Image {0} Flashing Successful".format(self._cpldfile))
            else:
        ...
        if ((str(self._enable_cpld_flashing)).lower() in ["true", "yes", "on"]):
            ...
            if (self._direct_cpld_pkg_download == True):  # cpld DOWNLAOD from ATF
                try:
                    ...
                    try:
                        if(self._host == "windows_host"):
                            subprocess.check_output("del /f "+str(self._local_cpld_sec_host_path)+str(self._cpld_pkg_name),shell=True)
                        elif(self._host == "rpi_linux_host"):
                            subprocess.check_output("sudo rm -rf "+str(self._local_cpld_sec_host_path)+str(self._cpld_pkg_name),shell=True)
                    '''
                    if (self._host == "windows_host"):
                        ret = subprocess.check_output("cd " + str(self._local_cpld_sec_host_path) + "&& curl -u " + str(
                            self._atf_username) + ":" + str(self._atf_password) + " -X GET " + str(
                            self._direct_atf_cpld_path) + " --output " + str(self._cpld_pkg_name), shell=True)
                    else:
                        ret = subprocess.check_output("cd " + str(self._local_cpld_sec_host_path) + "; curl -u " + str(
                ...
                self._log.info(
                    "Method-2 Pre-Downloaded CPLD Package in Local Machine {0}".format(self._local_cpld_sec_host_path))
                head, tail = ntpath.split(self._local_cpld_sec_host_path)
                self._cpld_pkg_name = tail
                self._local_cpld_sec_host_path = head + "\\"
                if (".pof" in self._cpld_pkg_name):
                    if (path.exists(self._local_cpld_main_host_path) == True):
                        self._log.info("User Has Given Extracted CPLD-zip and Given Direct .pof Image {0}".format(
                            self._cpld_pkg_name))
                        if "Second".lower() in self._cpld_pkg_name.lower():
                            self._cpldfile = self._cpld_pkg_name
                            Dnd_img_path = str(self._local_cpld_main_host_path) + str(self._cpld_pkg_name)
                            self._log.info(Dnd_img_path)
                        else:
                            self._log.error("Unable to find Secondary .pof file In CPLD Zip File")
                            return False
                    else:
                        self._log.error(
                            "User-Given Location {0} CPLD File Deosn't Exists, Provide Correct Location Of CPLD Image".format(
                    ...
                    self._log.info("User Has Given CPLD Image Zip File Directly {0}".format(self._cpld_pkg_name))
                    if (path.exists(self._local_cpld_sec_host_path) == True):
                        with zipfile.ZipFile(str(self._local_cpld_sec_host_path) + str(self._cpldfile), 'r') as zip_ref:
                            zip_ref.extractall(str(self._local_cpld_sec_host_path))

                        self._cpldfile = self.get_cpld_pof_name(self._local_cpld_sec_host_path, "Second")
                        self._log.info("CPLD Image Method-2 {0} ".format(self._cpldfile))
                    else:
                        self._log.error(
                            "User-Given Location {0} CPLD File Deosn't Exists, Provide Correct Location Of CPLD Image".format(
                                self._local_cpld_sec_host_path))
                        return False
                else:
                    self._log.error(
                        "User Has Given only the file location {0} No specific file was Given, Please Given Full File path also with the File Name".format(
                ...
                try:
                    ...
                    return False
            time.sleep(5)
            Dnd_img_path = str(self._local_cpld_sec_host_path) + str(self._cpld_pkg_name)
            if (self._local_cpld_host_mode != True):
                self._log.info(Dnd_img_path)
                ...
                if (self._host == "windows_host"):
                    ...
                    self._log.error(
                        "Currently Supported Host Machines Are Microsoft Based OS and Debian Raspberry pi OS")
                self._log.info("CPLD Zip Image Deleted After Extraction")
                # latest_cpld_img = glob.glob(str(self._local_cpld_sec_host_path) + "*.pof")
                # self._cpldfile = max(latest_cpld_img, key=os.path.getctime)
                # head, tail = ntpath.split(self._cpldfile)
                # self._cpldfile = tail
            self._log.info("CPLD Image {0} Is Going To Get Flashed On Platform".format(self._cpldfile))
            ...
            # Flashing Image
            if (self._flash.flash_image(self._local_cpld_sec_host_path, self._cpldfile,target="cpld2") == True):
                self._log.info("CPLD Image {0} Flashing Successful".format(self._cpldfile))
            else:
                self._log.info("Unable To Do Flashing CPLD Image")
                return False
    ...
    def get_cpld_pof_name(self, cpld_oss_pkg_path, chip_name):
        ...
        """
        if cpld_oss_pkg_path is not None:
            list_file_names = os.listdir(cpld_oss_pkg_path)
            for file in list_file_names:
                if chip_name.lower() in file.lower():
    ...
    def execute(self):
        self._log.info("{0} {1} {2} {3} {4}".format(self._ostype, self._cpldpath, self._cpldfile, self._imagetype,
                                                    self._driverpkgpath))
        self._log.info(
            "{0} {1} {2} {3} {4}".format(self._enable_cpld_flashing, self._imagefile, self._local_cpld_main_host_path,
