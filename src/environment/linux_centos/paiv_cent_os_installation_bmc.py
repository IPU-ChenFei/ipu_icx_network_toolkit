...
class Os_Installation(BaseTestCase):
    ...
    def add_arguments(cls, parser):
        ...
        parser.add_argument('--ATF_ISO_PATH', action="store", dest="atf_iso_path", default="https://ubit-artifactory-ba.intel.com/artifactory/dcg-dea-srvplat-local/Automation_Tools/SPR/CentOS_8_4_v5.iso",
    ...
    def bios_path_navigation1(self, path):
        ...
        try:
            ...
            for i in range(len(path)):
                ...
                self.setupmenu.enter_selected_item(ignore=False, timeout=10)
                self._log.info("Entered into {0} ".format(path[i]))
            self._log.info("Entered into Install CentOS 8")
            self.setupmenu.enter_selected_item(ignore=False, timeout=10)
            return True
        except Exception as ex:
            self._log.error("{0} Issues Observed".format(ex))
            ...
            for i in range(0, 100):
                try:
                    ...
                    if (self._os.is_alive() == True):
                        ...
                        self._log.info("CENT Os Installation Had Issues, Waited For 70Minutes ")
                        return False
                    else:
                        self._log.debug("CENT OS Installation Is IN-Progress")
                except:
                    continue
