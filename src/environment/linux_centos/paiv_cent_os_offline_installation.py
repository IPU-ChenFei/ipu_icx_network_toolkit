"""
version 1.0 25-february-2022 :- Created Os Installation Script
...
class Os_Installation(BaseTestCase):
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        self.inventory_filename = arguments.inventory_filename
        if not self.inventory_filename:
            self.inventory_filename = None

        self.os_params = self._os_pre_req_lib.get_sut_inventory_data("centos", self.inventory_filename)
    ...
    def add_arguments(cls, parser):
        ...
        parser.add_argument('--LOCAL_OS_PACKAGE_MODE', action="store", dest="pre_downloaded_os_pkg_mode", default=True,
        ...
        # --------------------- Inventory filename parameter
        parser.add_argument('--INVENTORY_FILENAME', action="store", dest="inventory_filename",
                            default=False,
                            help="To get the inventory from local")
    ...
    def automation_post_installation_enabler(self):
        target_rep_disk = None
        if 'raid' not in self._rhel_cfg_file_name:
            if self._hardisk_drive_name not in "UEFI Misc Device":
                drive_name = self.storage_common_obj.get_drive_name(self._hardisk_drive_name)
                target_rep_disk = "targetdisk={}".format(drive_name)

        grup_file_name = "S:\\EFI\\BOOT\\grub.cfg"
        if (os.path.isfile("S:\\" + str(self._rhel_cfg_file_name)) == True):
            print("This is RHEL OS")
            if 'raid' in self._rhel_cfg_file_name:
                uefi_rep_str = self._rhel_cfg_file_name.replace("-raid","")
                if (os.path.isfile(grup_file_name) == True):
                    with open(grup_file_name, 'r') as gb_file:
                        file_data = gb_file.read()
                        time.sleep(4)
                    file_data = file_data.replace(uefi_rep_str,self._rhel_cfg_file_name)
                    with open(grup_file_name, 'w', encoding="utf-8") as gb_file1:
                        gb_file1.write(file_data)

            with open("S:\\" + str(self._rhel_cfg_file_name), 'r') as file:
                filedata = file.read()
                time.sleep(4)
            if 'raid' not in self._rhel_cfg_file_name:
                if self._hardisk_drive_name not in "Misc Device":
                    filedata = re.sub('targetdisk=sda.*?done', target_rep_disk, filedata, flags=re.DOTALL)
        ...
        if (os.path.isfile("S:\\" + str(self._centos_cfg_file_name)) == True):
            self._log.info("This is Cent OS")
