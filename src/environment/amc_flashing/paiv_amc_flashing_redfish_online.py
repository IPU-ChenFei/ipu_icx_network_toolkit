...
class AMC_Flashing(BaseTestCase):
    def __init__(self, test_log, arguments, cfg_opts):
        super(AMC_Flashing, self).__init__(test_log, arguments, cfg_opts)
        ...
        self._os_install = False  # for flashing amc image alone
        ...
        self._enable_amc_flashing = True
        self._amcpath = arguments.AMCPATH
        self._amcfile = arguments.AMCFILE
        ...
        # self._amc_dnd_extract = arguments.img_dnd_extract    #True or Flase by default True
        self._direct_amc_pkg_download = True  # whether to download amc from ATF given by user #True or False
        self._direct_atf_amc_path = arguments.atf_amc_path  # full atf amc_pkg path along with file name extension usually .zip
        # target location where it needs to be copied
        self._local_amc_host_mode = False
        self._local_amc_host_path = arguments.amc_img_dwnd_host_path  # In this passed location amc package resides pre-downloaded
    ...
    def prepare(self):
        # validation on amc package mode
        if ((str(self._local_amc_host_mode)).lower() in ["true", "yes", "on", "enable"]):
            self._local_amc_host_mode = True
            if (self._local_amc_host_mode and self._direct_amc_pkg_download):
                self._log.error(
                    "Either Already Available Package or Newly Download From Artifactory Either one option can be Made True")
                return False
            if not self._local_amc_host_path:
                self._log.error("Give In The lOCATION OF Pre-Downloaded amc ZIP LOCATION")
                return False

        if ((str(self._direct_amc_pkg_download)).lower() in ["true", "yes", "on", "enable"]):
            self._direct_amc_pkg_download = True
            if not self._direct_atf_amc_path:
    ...
    def add_arguments(cls, parser):
        super(AMC_Flashing, cls).add_arguments(parser)
        # caf based throw and fetch approach
        ostype = os.environ.get("OSTYPE")
        amcpath = os.environ.get("AMCPATH")
        amcfile = os.environ.get("AMCFILE")
        ...
        if (str(platform.architecture()).find("WindowsPE") != -1):
            host = "windows_host"
            amc_img_hostpath = "C:\AMC_Image\\"
            sft_and_baseos = "C:\OS\\"
            # host sanity check for folders
            if (path.exists("C:\AMC_Image") != True):
                subprocess.check_output("mkdir C:\AMC_Image", shell=True)
            if (path.exists("C:\os") != True):
                subprocess.check_output("mkdir C:\os", shell=True)
        elif (str(platform.architecture()).find(r"ELF") != -1):
            host = "rpi_linux_host"
            amc_img_hostpath = "/home/pi/Public/firmware/chipwrite/"
        ...
        ##amc PACKAGE
        # --------------------- Whether To Download and Use ALready Existing AMC Package
        parser.add_argument('--ENABLEFLASHING', action="store", dest="ENABLE_AMC_FLASHING", default=enableflashing,
                            help="Incase amc .bin Needs To Be Flashed On Platform Give True By Default False it WIll Be False")
        parser.add_argument('--AMCPATH', action="store", dest="AMCPATH", default=amcpath)  # owr
        parser.add_argument('--AMCFILE', action="store", dest="AMCFILE", default=amcfile)  # owr
        # --------------------- FOR DIRECT Download of amc PACKAGE
        parser.add_argument('--DIRECT_AMC_PACKAGE_DOWNLOAD_MODE', action="store", dest="direct_amc_download",
                            default=False,
                            help="InOrder TO Directly Download Ifwi-Package From ATF Make It True,by Default False")
        parser.add_argument('--ATF_PATH_AMC', action="store", dest="atf_amc_path", default=None,
                            help="InOrder TO Directly Download Ifwi-Package From ATF Make It True,by Default False")
        # --------------------- FOR Custom RE-Use of amc PACKAGE In Local Machine
        parser.add_argument('--LOCAL_AMC_PACKAGE_MODE', action="store", dest="pre_downloaded_amc_pkg_mode",
                            default=None,
                            help="Inorde TO Make Use Of Already Existing Predownloaded amc Package From Local Hostmachine, Make It True, By Default it will be False")  # true will fetch the image location along sft img_name passed by user
        parser.add_argument('--LOCAL_AMC_IMG_PATH', action="store", dest="amc_img_dwnd_host_path",
                            default=amc_img_hostpath,
                            help="Incase already Exisitng AMC Image on Local Drive Needs To Be Used Give Full amc path Location")  # offline flashing
        # parser.add_argument('--DND_EXTRACT', action = "store", dest = "img_dnd_extract",default = True,help="Extract amc Image TO USB,By Default It will be True")
        ...
        # --amcFILE Name of the amc Image with .bin Extension
        # For Direct Parameter passing no need of Artifactory usage  --amc_IMG_PATH xxxxx --amcFILE xxxxx --Dnd_Extract False
    ...
    def platform_amc_flashing(self):
        if ((str(self._enable_amc_flashing)).lower() in ["true", "yes", "on"]):
            # Downloading and Extracting amc-Image Alone
            if (self._direct_amc_pkg_download == True):  # amc DOWNLAOD from ATF
                try:
                    self._log.info("Method-1 Direct Download Of amc Package From Artifactory {0}".format(
                        self._direct_amc_pkg_download))
                    self._log.info("This Is the location of amc Package Artifactory Location {0}".format(
                        self._direct_atf_amc_path))
                    head, tail = ntpath.split(self._direct_atf_amc_path)
                    self._amc_pkg_name = tail
                    if (self._host == "windows_host"):
                        ret = subprocess.check_output("cd " + str(self._local_amc_host_path) + "&& curl -u " + str(
                            self._atf_username) + ":" + str(self._atf_password) + " -X GET " + str(
                            self._direct_atf_amc_path) + " --output " + str(self._amc_pkg_name), shell=True)
                    else:
                        ret = subprocess.check_output("cd " + str(self._local_amc_host_path) + "; curl -u " + str(
                            self._atf_username) + ":" + str(self._atf_password) + " -X GET " + str(
                            self._direct_atf_amc_path) + " --output " + str(self._amc_pkg_name), shell=True)
                    self._log.info("Direct Download Of amc Package From Artifactory Successfull")
                except Exception as ex:
                    self._log.error(
                        "{0} Artifactory Path Is Not Given Properly and Please Check ATF Usernme {1} and Password {2} {3}".format(
                            self._direct_amc_pkg_download, self._atf_username, self._atf_password, ex))
                    return False
            elif (self._local_amc_host_mode == True):  # for making usee of already #pre downlaoded package
                self._log.info(
                    "Method-2 Pre-Downloaded amc Package in Local Machine {0}".format(self._local_amc_host_path))
                head, tail = ntpath.split(self._local_amc_host_path)
                self._amc_pkg_name = tail
                if (".bin" in self._amc_pkg_name):
                    if (path.exists(self._local_amc_host_path) == True):
                        self._log.info("User Has Given Extracted amc-zip and Given Direct .bin Image {0}".format(
                            self._amc_pkg_name))
                        self._amcfile = self._amc_pkg_name
                    else:
                        self._log.error(
                            "User-Given Location {0} amc File Deosn't Exists, Provide Correct Location Of amc Image".format(
                                self._local_amc_host_path))
                        return False
                elif (".zip" in self._amc_pkg_name):
                    self._log.info("User Has Given amc Image Zip File Directly {0}".format(self._amc_pkg_name))
                    if (path.exists(self._local_amc_host_path) == True):
                        with zipfile.ZipFile(str(self._local_amc_host_path), 'r') as zip_ref:
                            zip_ref.extractall(head)
                        if (self._host == "windows_host"):
                            latest_amc_file = str(head) + "\*.bin"
                        else:
                            latest_amc_file = str(head) + "/*.bin"
                        latest_amc_img = glob.glob(latest_amc_file)
                        self._amcfile = max(latest_amc_img, key=os.path.getctime)
                        head, file_name = ntpath.split(self._amcfile)
                        self._amcfile = file_name
                        self._local_amc_host_path = head
                        self._log.info("AMC Image Method-2 {0} ".format(self._amcfile))
                elif (".7z" in self._amc_pkg_name):
                    self._log.info("User Has Given AMC Image 7z File Directly {0}".format(self._amc_pkg_name))
                    self._log.info("AMC Image Method-3 {0} ".format(self._amcfile))
                else:
                    self._log.error(
                        "User Has Given only the file location {0} No specific file was Given, Please Given Full File path also with the File Name".format(
                            self._local_amc_host_path))
                    return False
                self._log.info("Pre-Downloaded AMC From Local Computer {0}".format(self._amc_pkg_name))
            else:
                # Download Begins For AMC IMG
                self._log.info("Method -3 OWR Triggerd and Downloaded Software Package FROM Artifactory")
                Atf_img_path = str(self._basepath) + str(self._amcpath) + str(
                    self._amcfile)  # if future amc path Change is Required
                self._log.info("This Is the location of AMC Artifactory Location {0}".format(Atf_img_path))
                try:
                    if (self._host == "windows_host"):
                        ret = subprocess.check_output("cd " + str(self._local_amc_host_path) + "&& curl -u " + str(
                            self._atf_username) + ":" + str(self._atf_password) + " -X GET " + str(
                            Atf_img_path) + " --output " + str(self._amcfile), shell=True)
                    else:
                        ret = subprocess.check_output("cd " + str(self._local_amc_host_path) + "; curl -u " + str(
                            self._atf_username) + ":" + str(self._atf_password) + " -X GET " + str(
                            Atf_img_path) + " --output " + str(self._amcfile), shell=True)
                    self._amc_pkg_name = self._amcfile
                except:
                    self._log.error("Failed Due To Path From Artifactory Not Given {0}".format(sys.exc_info()[0]))
                    return False
            time.sleep(5)
            Dnd_img_path = str(self._local_amc_host_path) + str(self._amc_pkg_name)
            if (self._local_amc_host_mode != True):
                self._log.info(Dnd_img_path)

                if (path.exists(Dnd_img_path) == True):
                    if (zipfile.is_zipfile(Dnd_img_path) == True) or py7zr.is_7zfile(Dnd_img_path):
                        self._log.info("AMC Image Download Successfull")
                    else:
                        self._log.error(
                            "Downloaded File Is Not A AMC ZIP Image File,Artifactory Path Is Not Given Properly")
                        return False
                else:
                ...
                if ".7z" in Dnd_img_path:
                    with py7zr.SevenZipFile(Dnd_img_path, mode='r') as z:
                        z.extractall(str(self._local_amc_host_path))
                elif ".zip" in Dnd_img_path:
                    with zipfile.ZipFile(Dnd_img_path, 'r') as zip_ref:
                        zip_ref.extractall(str(self._local_amc_host_path))
                ...
                if (self._host == "windows_host"):
                    try:
                        ...
                        self._log.error("AMC Zip Couldn't Be Delted After Extraction {0}".format(sys.exc_info()[0]))
                        return False
                elif (self._host == "rpi_linux_host"):
                    try:
                        subprocess.check_output("sudo rm -rf " + str(Dnd_img_path), shell=True)
                    except:
                        self._log.error("AMC Zip Couldn't Be Delted After Extraction")
                        return False
                else:
                    self._log.error(
                        "Currently Supported Host Machines Are Microsoft Based OS and Debian Raspberry pi OS")
                self._log.info("AMC Zip Image Deleted After Extraction")
                # latest_amc_img = glob.glob(str(self._local_amc_host_path) + "*.bin")
                # self._amcfile = max(latest_amc_img, key=os.path.getctime)
                head, tail = ntpath.split(Dnd_img_path)
                path_amc= self._local_amc_host_path+"\\"+tail.strip(".zip")
                ret=[]
                ret=self.get_amc_oob_bin_name(path_amc)
                self._log.info(ret)
                if(ret[0].find("dual") != -1):
                    self._log.info(
                        "AMC Image has 2-Staged IMAGE Detected {0} {1} Is Going To Get Flashed On Platform".format(
                            ret[1][0], ret[1][1]))
                    for i in range(0,2):
                        # Flashing Image
                        if (self._flash.flash_image(path_amc, ret[1][i], target="amc") == True):
                            self._log.info("AMC Image {0} Flashing Successfull".format(ret[1][i]))
                        else:
                            self._log.info("Unable To Do Flashing for AMC Image")
                            return False
                else:
                    self._log.info("AMC Image {0} Is Going To Get Flashed On Platform".format(ret))
                    # Flashing Image
                    if (self._flash.flash_image(path_amc, ret, target="amc") == True):
                        self._log.info("AMC Image {0} Flashing Successfull".format(ret))
    ...
    def get_amc_oob_bin_name(self, amc_oss_pkg_path):
        """
        Function to find the correct .oob.bin file from the OSS site under amc ingredient
        """
        if amc_oss_pkg_path is not None:
            list_file_names = os.listdir(amc_oss_pkg_path)
            list_file_names = os.listdir(amc_oss_pkg_path)
            k = []
            for file in list_file_names:
                if re.search(r"pldm", file):
                    file_name1 = file
                    if ("stage1" in str(file_name1)):
                        s1 = file_name1
                        k.append(s1)
                    elif (str("stage2") in str(file_name1)):
                        s2 = file_name1
                        k.append(s2)
                        return "dual", k
                    if not k:
                        if (re.search("pldm", file)):  # add by AT
                            file_name = file
                            if not "stage" in str(file_name):
                                return (file_name)
            self._log.error(
                "No AMC Firmware File Found for mentioned keytags stage and pldm in this Location {0}".format(
                    amc_oss_pkg_path))
            return False
    ...
    def bios_amc_ver_verification(self):
        """
        Function to verify the flashed AMC image in BIOS.
        """
        ...
        start_match = self._amc_pkg_name.split(".")[0]
        end_match = self._amc_pkg_name.split(".")[-2].split("_")[0]

        amc_ver_bios = ret_bios_info.result_value[0][2].split()[0]
        bios_name = amc_ver_bios.split(".")

        start_bios_match = bios_name[0]
        temp_end_bios_match = bios_name[-1]
        end_bios_match = temp_end_bios_match[int("-{}".format(len(end_match))):]
        self._log.info("amc image version flashed from local: {}".format(self._amc_pkg_name))

        self._log.info("amc image version from BIOS page is: {}".format(amc_ver_bios))

        if (str(start_match).lower() in str(start_bios_match).lower()):
            if (str(end_match).lower() in str(end_bios_match).lower()):
                self._log.info("Successfully verified the flashed AMC image version..")
                return True
        self._log.error("AMC image version verification failed..")
        return False



    def verify_amc_version(self):
        time.sleep(60)
        ret = self._flash.current_amc_version_check()
        if (ret[0] == True):
            self._log.info("Current AMC version {0} ".format(ret[1]))
            self._log.info("Flashed AMC version {0}".format(self._amcfile))

            flashed_amc_version = re.findall("egs-\d*.\d*-\d*-", ret[1])
            local_amc_version = re.findall("egs-\d*.\d*-\d*-", self._amcfile)

            if str(flashed_amc_version[0]) != str(local_amc_version[0]):
                self._log.error("Flashed AMC version is not same as the current version..")
                return False

            self._log.info("Successfully verified the Flashed AMC version with the current version..")

        else:
            self._log.error("AMC version verification Failed..")
            return False

        return True

    def execute(self):
        self._log.info("{0} {1} {2} {3} {4}".format(self._ostype, self._amcpath, self._amcfile, self._imagetype,
                                                    self._driverpkgpath))
        self._log.info(
            "{0} {1} {2} {3} {4}".format(self._enable_amc_flashing, self._imagefile, self._local_amc_host_path,
        ...
        # Flashing AMC Image
        ret = self._flash.get_current_firmware_version(firmware="amc")
        self._log.info("Before Flashing AMC Image {0}".format(ret[1]))
        ret = self.platform_amc_flashing()
        if (ret == True):
            self._log.info("AMC_Image Flashing TO Platform Was Succesful")
            ret = self._flash.get_current_firmware_version(firmware="amc")
            self._log.info("After Flashing AMC Image {0}".format(ret[1]))
        elif (ret == "Flashing_Disabled"):
            self._log.info("AMC-Flashing Parameter is Given as False, AMC_Image Flashing Will not Happen")

        else:
            self._log.info("AMC-Flashing Failed")
            return False
        return True


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if AMC_Flashing.main() else Framework.TEST_RESULT_FAIL)
