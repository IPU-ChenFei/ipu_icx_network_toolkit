...
class Os_Installation(BaseTestCase):
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        self._ifwi_verification = arguments.IFWI_VERIFICATION
        ...
        self._direct_ifwi_pkg_download = False  # whether to download ifwi from ATF given by user #True or False
    ...
    def add_arguments(cls, parser):
        ...
        parser.add_argument('--IFWI_VERIFICATION', action="store", dest="IFWI_VERIFICATION",
                            default=True,
                            help="Whether to request BIOS menu verification after flashing the IFWI")
        ...
        if ((str(self._enable_ifwi_flashing)).lower() in ["true", "yes", "on"]):
            ...
            if (self._direct_ifwi_pkg_download == True):  # ifwi DOWNLAOD from ATF
                ...
                if (".bin" in self._ifwi_pkg_name):
                    ...
                    string = str(self._ifwi_pkg_name)[:-4]
                    string = str(self._local_ifwi_host_path) + "\\" + string + "\\"

                    if (path.exists(str(self._local_ifwi_host_path) + str(self._ifwi_pkg_name)) == True):
                        with zipfile.ZipFile(str(self._local_ifwi_host_path+ str(self._ifwi_pkg_name)), 'r') as zip_ref:
                            zip_ref.extractall(string)
                        try:
                            if (self._host == "windows_host"):
                                latest_ifwi_file = str(string) + "\*.bin"
                            else:
                                latest_ifwi_file = str(string) + "/*.bin"
                            self._log.info("Their Is was a SubFolder in The Extracted Zip Logic Taken Care")
                        except:
                            if (self._host == "windows_host"):
            ...
            if (self._local_ifwi_host_mode != True):
                ...
                try:
                    ...
                    self._ifwifile = tail
                    self._log.error("Their Is was a SubFolder in The Extracted Zip Logic Taken Care")
                    folder_name = ''.join(self._ifwifile.split())
                    folder_name = (folder_name[:-4])+"\\"
                    self._local_ifwi_host_path += folder_name
                    print("sdfasdfdfg",self._local_ifwi_host_path)
                except:
                    self._log.error("Their Is No SubFolder in The Extracted Zip")
            ...
            if (self.platform_ac_power_on() != True):
                self._log.error("Failed To Platform Ac-Power OFF")
                return False
            if self._ifwi_verification == True:
