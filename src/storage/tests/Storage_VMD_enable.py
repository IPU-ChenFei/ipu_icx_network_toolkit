...
def test_steps(sut, my_os):
    ...
    Case.step('set VMD enable')
    print('=======================================================')
    print(sut.socket_name)
    print('========================================================')
    if sut.socket_name == 'GNR':
        vmd_knobs = set_vmd('bhs_bios.xml')
    elif sut.socket_name == 'SPR':
        vmd_knobs = set_vmd('egs_bios.xml', vmd_list=[4, 5], weight=8)
    else:
        vmd_knobs = set_vmd('wht_bios.xml')
    set_bios_knobs_step(sut, *vmd_knobs)
    Case.sleep(5)

    Case.step('check vmd setting')
    ret = check_bios_knobs_step(sut, *vmd_knobs)
    Case.expect('check vmd enable successful', ret)
