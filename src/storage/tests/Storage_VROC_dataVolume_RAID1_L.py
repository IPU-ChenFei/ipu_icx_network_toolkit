...
def test_steps(sut, my_os):
    ...
    Case.step('create data volume RAID1')
    raid_devices_list = get_raid_devices(sut)
    errmsg_list = []
    raid_cmd1 = rf'echo y | mdadm -C /dev/md/imsm0 {raid_devices_list[0]} {raid_devices_list[1]} -n 2 -e imsm'
    raid_cmd2 = rf'mdadm -C /dev/md/md0 /dev/md/imsm0 -n 2 -l 1'
    try:
        ret_code1 = sut.execute_shell_cmd(raid_cmd1, cwd=log_dir)[0]
        ret_code2 = sut.execute_shell_cmd(raid_cmd2, cwd=log_dir)[0]
        Case.expect('create RAID 1 success', (ret_code1 or ret_code2) == 0)

        Case.step('check raid info')
        check_raid_cmd = r'mdadm -D /dev/md/md0 | tee RAID1.txt'
        stdout = sut.execute_shell_cmd(check_raid_cmd, cwd=log_dir)[1]
        Case.expect('check raid 1 info success', 'raid1' in stdout)
        ...
        Case.expect('stop RAID 1 success', ret_code4 == 0 and expect_str in stderr2)
