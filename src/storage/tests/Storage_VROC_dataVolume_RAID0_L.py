...
def test_steps(sut, my_os):
    ...
    raid_cmd2 = rf'mdadm -C /dev/md/md0 /dev/md/imsm0 -n 2 -l 0'
    try:
        ret_code1 = sut.execute_shell_cmd(raid_cmd1, cwd=log_dir)[0]
        ret_code2 = sut.execute_shell_cmd(raid_cmd2, cwd=log_dir)[0]
        Case.expect('create RAID 0 success', (ret_code1 or ret_code2) == 0)

        Case.step('check raid info')
        check_raid_cmd = r'mdadm -D /dev/md/md0 | tee RAID0.txt'
        ...
        Case.expect('stop RAID 0 success', ret_code4 == 0 and expect_str in stderr2)
