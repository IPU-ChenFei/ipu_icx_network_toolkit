...
def test_steps(sut, my_os):
    ...
    for i in range(hd_num):
        fio_stress_cmd = rf"fio -filename={devices_list[i]} -direct=1 -iodepth=16 -thread -rw=rw -rwmixread=70 -rwmixwrite=30 -ioengine=libaio -bs=64k \
        -size=300G -numjobs=1 -runtime={stress_time} -time_based -group_reporting -name=randrw_70read_64k > {devices_list[i].split('/')[-1]}.txt"
