...
def test_steps(sut, my_os):
    ...
    for i in range(hd_num):
        fio_stress_cmd = rf"fio -filename={devices_list[i]} -direct=1 -iodepth=16 -thread -ioengine=libaio -bs=64k \
        -size=100G -numjobs=1 -runtime={stress_time} -time_based -group_reporting -name=seq_100read64k > {devices_list[i].split('/')[-1]}.txt"
