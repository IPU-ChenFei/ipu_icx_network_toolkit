...
#    print(f"fio -filename={disk} -direct=1 -iodepth 1 -thread -rw=randrw -rwmixread=70 -ioengine=psync -bs=4k -size=300G -numjobs=50 -runtime=180 -group_reporting -name=randrw_70read_4k -output={log}")
    print(f"fio -filename={disk} -direct=1 -iodepth 16 -thread -rw=write -ioengine=libaio -bs=64k -size=300G -numjobs=1 -runtime=180 -time_based -group_reporting -name=seq_100write64k -output={log}")
    os.system(f"fio -filename={disk} -direct=1 -iodepth 16 -thread -rw=write -ioengine=libaio -bs=64k -size=300G -numjobs=1 -runtime=180 -time_based -group_reporting -name=seq_100write64k -output={log}")
