...
class StorageIOStressSataLinux(ContentBaseTestCase):
    """
    HPALM : H82247-PI_Storage_StressIO_SATA_L
    ...
    """
    TEST_CASE_ID = ["H82247", "PI_Storage_StressIO_SATA_L"]
    CMD_FDISK = 'fdisk -l'
    IP_ADDRESS_SUT = "ip route get 1.2.3.4 | awk '{print $7}'"
    ROOT_PATH = "/root"
    DISK_INFO_REGEX = ".*\/dev\/sda:\s(\S+.\S+)"
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        """
        super(StorageIOStressSataLinux, self).__init__(test_log, arguments, cfg_opts)
    ...
    def prepare(self):
        ...
        """
        super(StorageIOStressSataLinux, self).prepare()
    ...
    def execute(self):
        ...
        disk_info = re.findall(self.DISK_INFO_REGEX, fdisk_info)
        self._log.debug("Disk /dev/sda size is : {}".format(disk_info))
        if not disk_info:
            raise content_exceptions.TestFail("Failed to verify disk information {} in {}".format(disk_info,
                                                                                                  self.CMD_FDISK))
    ...
    def cleanup(self, return_status):
        ...
        """
        super(StorageIOStressSataLinux, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if StorageIOStressSataLinux.main() else Framework.TEST_RESULT_FAIL)
