...
class StorageStressIONVMeWindows(StorageCommon):
    """
    HPALM : H82238-PI_Storage_StressIO_NVMe_W
    Drive stress IO on all the drives for one hour:
    1. On Windows SUT, run 'iometer.exe' under cmd shell for one hour
    """
    TEST_CASE_ID = ["H82238", "PI_Storage_StressIO_NVMe_W"]
    step_data_dict = {1: {'step_details': 'Check all the drives in This Computer-->Manage-->Disk Management',
                      ...
                      2: {'step_details': 'IDrive stress IO on all the drives for one hour using Iometer tool command '
                          ...
                          'expected_results': 'IOMeter tool installed and executed successfully without IOErrors'}}
    CSV_FILE = "result.csv"
    DEVICE_TYPE = "NVMe"

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new StorageStressIONVMeWindows object

        :param test_log: Used for debug and info messages
        :param arguments:
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(StorageStressIONVMeWindows, self).__init__(test_log, arguments, cfg_opts)
    ...
    def prepare(self):
        ...
        """
        super(StorageStressIONVMeWindows, self).prepare()
    ...
    def execute(self):
        """
        1. Check OS is booted from NVMe or not
    ...
    def cleanup(self, return_status):
        ...
        """
        super(StorageStressIONVMeWindows, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if StorageStressIONVMeWindows.main() else Framework.TEST_RESULT_FAIL)
