...
class U2NvmeEfiShellColdReset(TxtBaseTest):
    """  Phoenix ID : 16013539771 U.2 NVMe-EFI shell reset(Cold)

    This Testcase aims at Efi shell cold reset on U.2 Nvme
    """
    TEST_CASE_ID = ["16013539771", "U.2 NVMe-EFI shell reset(Cold)"]
    ROOT_DIR = "/root"
    WAIT_TIME = 60
    COLD_RESET_ZIP_FILE = "coldreset.zip"
    LSSCSI = "lsscsi | grep nvme | grep INTEL"
    REGEX_FOR_SSD = "/dev/\S+"
    step_data_dict = {
        1: {'step_details': 'Copy the startup.nsh file to the U.2 nvme from the EFI shell.',
            'expected_results': 'Successfully copied the startup.nsh file to U.2 nvme from EFI shell.'},
        ...
        3: {'step_details': 'Execute the startup.nsh for 100 cycles',
            'expected_results': 'Successfully performed 100 cycles of cold reset'}}

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new U2NvmeEfiShellColdReset object

        :param test_log: Used for debug and info messages
        :param arguments:
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(U2NvmeEfiShellColdReset, self).__init__(test_log, arguments, cfg_opts)
    ...
    def prepare(self):
        ...
        :return None
        """
        super(U2NvmeEfiShellColdReset, self).prepare()

    def execute(self):
        """
        This method functionality is to execute EFI shell cold reset( reset -c) to
        be triggered on u.2 nvme.
        ...
        if len(ssd_list) < 16:
            raise content_exceptions.TestFail("Please connect 16 U.2 nvme to hsbp in the setup to go ahead!")

        # Download the file from artifactory and copy it to SUT
        self._os.execute("mkdir /mnt/", self._command_timeout)
        zip_file_path = self._install_collateral.download_and_copy_zip_to_sut(self.ROOT_DIR + "/reset", self.COLD_RESET_ZIP_FILE)
        ...
        self._log.info("Executing '{}' command to perform cold reset".format(startup_cmd))
        count = 1
        while count < cycles:
            for usb_drive in usb_drive_list:
                ...
                try:
                    ...
                    pass
                self._log.info("Cold Reset successfull!!")
                time.sleep(timeout)
                ...
                self._log.info("Cold reset count : {} times".format(count))
                if count < cycles:
    ...
    def cleanup(self, return_status):  # type: (bool) -> None
        ...
        try:
            ...
            raise ex
        super(U2NvmeEfiShellColdReset, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if U2NvmeEfiShellColdReset.main() else Framework.TEST_RESULT_FAIL)
