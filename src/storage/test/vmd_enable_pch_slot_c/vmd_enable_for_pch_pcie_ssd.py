...
class VMDEnableForPchPcieSsd(ContentBaseTestCase):
    """
    Phoenix ID : P16013615499-Verify system booting with VMD enable for PCH-Pciessd for Centos
                 P16013615596-Verify system booting with VMD enable for PCH-Pciessd for Windows
    Verify booting of the system with OS after enabling the VMD of slot C .
    """
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new VMDEnableForPchPcieSsd object
        ...
        super(VMDEnableForPchPcieSsd, self).__init__(test_log, arguments, cfg_opts, self.bios_config_file)
    ...
    def prepare(self):
        ...
        :return None
        """
        super(VMDEnableForPchPcieSsd, self).prepare()

    def execute(self):
        """
        Verify system with OS  installed boots successfully with PCH slot for HHHL is enabled in bios
        ...
        self._log.info("PCIE SSD Drive connected to Slot C is {}".format(slot_c_info[self.DEVICE_NAME]))
        ...
        self._log.info("System boot to OS successfully with PCH slot enabled")

        return True


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if VMDEnableForPchPcieSsd.main() else Framework.TEST_RESULT_FAIL)
