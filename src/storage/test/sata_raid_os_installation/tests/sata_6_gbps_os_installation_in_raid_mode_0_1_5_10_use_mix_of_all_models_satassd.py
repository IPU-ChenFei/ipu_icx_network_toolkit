...
class OsInstallationSataRaid(ContentBaseTestCase):
    """
    Phoenix ID : 16013558163 - SATA-6 GBPS OS installation in RAID Mode(0,1,5,10) -use mix of all Models SATASSD
    1509391748- SATA SSD-OS and Driver installation check in RAID Mode

    this class used for create RAID using two SATA drives,
    Install RHEL OS on RAID and delete RAID.
    """

    TEST_CASE_ID = ["16013558163", "SATA_6_GBPS_OS_installation_in_RAID_Mode_0_1_5_10_use_mix_of_all_Models_SATASSD",
                    "1509391748", "SATA_SSD_OS_and_Driver_installation_check_in_RAID_Mode"]

    step_data_dict = {
                      1: {'step_details': 'create the RAID0/RAID1/RAID/5/RAID10 using SATA Drives',
                          'expected_results': 'RAID0 Creation successful'},
                      2: {'step_details': 'Install RHEL OS on RAID0/RAID1/RAID/5/RAID10 and boot to  OS',
                          'expected_results': 'Successfully installed OS'},
                      3: {'step_details': 'Verify OS is successfully installed in RAID0/RAID1/RAID/5/RAID10',
                          'expected_results': 'Verified OS installation is successful in RAID0/RAID1/RAID/5/RAID10'},
                      4: {'step_details': 'delete RAID0/RAID1/RAID/5/RAID10 Volume',
                          'expected_results': 'Successfully deleted RAID0/RAID1/RAID/5/RAID10 Volume'}}

    def __init__(self, test_log, arguments, cfg_opts):
        """
         Creates a new OsInstallationSataRaid object
        ...
        super(OsInstallationSataRaid, self).__init__(test_log, arguments, cfg_opts)
        ...
        self.raid_levels_supported = list()
    ...
    def prepare(self):
        ...
        if self.os.os_subtype.lower() == LinuxDistributions.RHEL.lower():
            self._common_content_lib.update_sut_inventory_file(SutInventoryConstants.SATA_RAID,
                                                               SutInventoryConstants.RHEL)
        elif self.os.os_subtype.lower() == LinuxDistributions.CentOS.lower():
            self._common_content_lib.update_sut_inventory_file(SutInventoryConstants.SATA_RAID,
                                                               SutInventoryConstants.CENTOS)
        else:
            content_exceptions.TestFail("{} Not supported.Please check the configuration".format(self.os.os_subtype))
    ...
    def execute(self):
        ...
        """
        # initialize package counter to extract the OS and software package only once and to skip the extraction step
        ...
        self._log.info("RAID Levels to test : {}".format(self.raid_levels_test))
        if len(self.raid_levels_test) < 2:
            raise content_exceptions.TestFail("For Creating a RAID need at least two Empty  are required, "
        ...
        for raid_level in self.raid_levels_test:
            ...
            if package_extract_counter > 0:
                if self.os.os_subtype.lower() == LinuxDistributions.RHEL.lower():
                    ret_val.append(
                        self._os_installation_lib.rhel_os_installation(os_extract=False, software_extract=False))
                elif self.os.os_subtype.lower() == LinuxDistributions.CentOS.lower():
                    ret_val.append(
                        self._os_installation_lib.centos_os_installation(installation_mode="offline", os_extract=False,
                                                                         software_extract=False))
                else:
    ...
    def cleanup(self, return_status):
        ...
        super(OsInstallationSataRaid, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if OsInstallationSataRaid.main() else Framework.TEST_RESULT_FAIL)
