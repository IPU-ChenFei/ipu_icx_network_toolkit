...
class StorageBootingToU2NvmeVMDAlderstreamLinux(ContentBaseTestCase):
    """
    Phoenix ID: 18016909970	PI_Storage_BootingTo_U.2_VMD_Alderstream_L
                16013668995 - PCie NVMe-OS installation in VMDEnable -use mix of all Models centos

    Install RHEL/CENTOS OS on connected NVMe U.2/PCIE card and boot into it,
    """
    TEST_CASE_ID = ["18016909970", "PI_Storage_BootingTo_U.2_VMD_Alderstream_L",
                "16013668995", "PCie NVMe-OS installation in VMDEnable -use mix of all Models"]

    name_ssd = None

    step_data_dict = {1: {'step_details': 'Enable the VMD BIOS knobs',
                          'expected_results': 'successfully Enabled VMD BIOS knobs'},
                      2: {'step_details': 'install Linux OS to U.2 device',
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new StorageBootingToU2NvmeVMDAlderstreamLinux object
        ...
        super(StorageBootingToU2NvmeVMDAlderstreamLinux, self).__init__(test_log, arguments, cfg_opts)
    ...
    def prepare(self):
        ...
        with open(sut_inv_file_path, "r") as fh:
            for line in fh.readlines():
                if (SutInventoryConstants.NVME_SSD_NAME_RHEL in line and self.os.os_subtype.lower() in
                    SutInventoryConstants.NVME_SSD_NAME_RHEL) or (SutInventoryConstants.NVME_SSD_NAME_CENTOS in line and
                        self.os.os_subtype.lower() in SutInventoryConstants.NVME_SSD_NAME_CENTOS):
                    self.name_ssd = line
                    break
    ...
    def execute(self):
        """
        This method is used to install the RHEL OS
        """
    ...
    def cleanup(self, return_status):
        """Test Cleanup"""
        super(StorageBootingToU2NvmeVMDAlderstreamLinux, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if StorageBootingToU2NvmeVMDAlderstreamLinux.main() else Framework.TEST_RESULT_FAIL)
