...
class StorageBootingToU2NvmeLinux(BaseTestCase):
    """
    HPALM : H80272-PI_Storage_BootingTo_U.2_L

    Install RHEL OS on connected NVMe U.2 card and boot into it,
    """
    TEST_CASE_ID = ["H80272", "PI_Storage_BootingTo_U.2_L"]
    NVME_SSD_NAME_RHEL = "nvme_ssd_name_rhel"
    name_ssd = None

    step_data_dict = {1: {'step_details': 'install Linux OS to U.2 device',
                          'expected_results': 'OS installed successfully'},
                      2: {'step_details': 'Boot into OS',
                          'expected_results': 'Successfully Booted into OS without any errors'}}

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new StorageBootingToU2NvmeLinux object
        ...
        super(StorageBootingToU2NvmeLinux, self).__init__(test_log, arguments, cfg_opts)
    ...
    def add_arguments(cls, parser):
        super(StorageBootingToU2NvmeLinux, cls).add_arguments(parser)
        # Use add_argument
    ...
    def cleanup(self, return_status):
        ...
        super(StorageBootingToU2NvmeLinux, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if StorageBootingToU2NvmeLinux.main() else Framework.TEST_RESULT_FAIL)
