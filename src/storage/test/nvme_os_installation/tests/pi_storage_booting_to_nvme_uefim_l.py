...
class StorageBootingToNVMeUEFIMLinux(BaseTestCase):
    """
    HPALM : H80021-PI_Storage_BootingToNVMe_UEFIM_L.
    ...
    TEST_CASE_ID = ["H80021", "PI_Storage_BootingToNVMe_UEFIM_W"]
    ...
    step_data_dict = {1: {'step_details': 'Copy and extract os and software packages into USB',
                      ...
                      2: {'step_details': 'Change 1st BOOT ORDER to NVME drive and select UEFI shell for grub install',
                          'expected_results': 'Successfully changed BOOT ORDER to NVME and selected UEFI shell for '
                                              'grub install'},
                      ...
                      4: {'step_details': 'Verify OS is successfully installed in NVME or not',
                          'expected_results': 'Verified OS installation is successful in NVME or not'}
                      }

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new StorageBootingToNVMeUEFIMLinux object
        ...
        super(StorageBootingToNVMeUEFIMLinux, self).__init__(test_log, arguments, cfg_opts)
    ...
    def add_arguments(cls, parser):
        super(StorageBootingToNVMeUEFIMLinux, cls).add_arguments(parser)
        # Use add_argument
    ...
    def execute(self):
        ...
        3. Switch USB to DUT
        4. Changing first boot order to NVME
        5. Executing commands in UEFI shell for os installation
        6. Verifying os installation in NVME
        ...
        # Enter into BIOS SETUP Page to change BIOS BOOTORDER AND Select NVME For OS-Installation
        ...
        self._log.info(
            "Entering Into Bios SETUP Page To Change BIOS BOOTORDER AND Select NVME For OS-Installation")
        if self._os_installation_lib.bios_path_navigation(path=self._os_installation_lib.bios_path):
            ...
            if self._os_installation_lib.bios_path_navigation(path=self._os_installation_lib.boot_select_uefi_path):
                self._log.info("Selecting NVME To Proceed with Boot is Successful")
            else:
    ...
    def cleanup(self, return_status):
        ...
        super(StorageBootingToNVMeUEFIMLinux, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if StorageBootingToNVMeUEFIMLinux.main() else Framework.TEST_RESULT_FAIL)
