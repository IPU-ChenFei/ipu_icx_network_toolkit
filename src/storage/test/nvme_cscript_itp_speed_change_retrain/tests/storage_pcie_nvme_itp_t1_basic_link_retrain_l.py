...
class StorageNvmeItpT1BasicLinkRetrainLinux(ContentBaseTestCase):
    """
    GLASGOW ID : G56622, NVMe_HHHL_PCIe_CScripts_ITP_T1_Basic Link Retrain
    ...
    TEST_CASE_ID = ["G56622", "NVMe_HHHL_PCIe_CScripts_ITP_T1_Basic Link Retrain"]
    LTSSM_TEST_NAME = "linkRetrain"
    step_data_dict = {
        ...
        3: {'step_details': 'Check NVME device Link speed Width Retrain the tool and run the Ltssm tool on'
                            ' OS and PythonSV',
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a StorageNvmeItpT1BasicLinkRetrainLinux object

        :param test_log: Used for debug and info messages
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(StorageNvmeItpT1BasicLinkRetrainLinux, self).__init__(test_log, arguments, cfg_opts)
    ...
    def prepare(self):
        # type: () -> None
        """preparing the setup"""
        super(StorageNvmeItpT1BasicLinkRetrainLinux, self).prepare()
    ...
    def cleanup(self, return_status):
        ...
        super(StorageNvmeItpT1BasicLinkRetrainLinux, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if StorageNvmeItpT1BasicLinkRetrainLinux.main() else Framework.TEST_RESULT_FAIL)
