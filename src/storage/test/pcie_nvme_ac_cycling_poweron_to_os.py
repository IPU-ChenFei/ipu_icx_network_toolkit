...
class PcieNvmeAcCycles(ResetBaseTest):
    """
    ...
    This TestCase is Used to Verify MCE Logged Status by Performing AC Cycles and verifying the status of u.2
     NVME in Every Cycle.
    """
    TEST_CASE_ID = ["16013568694", "Pcie NVMe-AC cycling (poweron to OS)"]
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        """
        super(PcieNvmeAcCycles, self).__init__(test_log, arguments, cfg_opts)
    ...
    def prepare(self):
        ...
        super(PcieNvmeAcCycles, self).prepare()
    ...
    def execute(self):
        ...
        for cycle_number in range(self._common_content_configuration.get_num_of_ac_cycles()):
            ...
            if before_reboot_device_list != after_reboot_device_list:
                raise content_exceptions.TestFail("Before AC cycle and After AC cycle PCIe NVMe device list is not same")
            self._log.info("Before AC cycle and After AC cycle PCIe NVMe devices are same")
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if PcieNvmeAcCycles.main() else Framework.TEST_RESULT_FAIL)
