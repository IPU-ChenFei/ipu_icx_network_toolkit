...
class SataEfiShellColdReset(TxtBaseTest):
    """  Phoenix ID : 16013535852 SATA - EFI shell reset(cold)

    This Testcase aims at Efi shell cold reset on SATA SSD
    """
    TEST_CASE_ID = ["16013535852", "SATA - EFI shell reset(cold)"]
    ROOT_DIR = "/root"
    WAIT_TIME = 60
    COLD_RESET_ZIP_FILE = "coldreset.zip"
    LSSCSI = "lsscsi | grep INTEL"
    REGEX_FOR_SSD = "/dev/\S+"
    step_data_dict = {
        1: {'step_details': 'Copy the startup.nsh file to sata ssd from EFI shell',
            'expected_results': 'Successfully copied the startup.nsh file to sata ssd from EFI shell.'},
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new SataEfiShellColdReset object

        :param test_log: Used for debug and info messages
        :param arguments:
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(SataEfiShellColdReset, self).__init__(test_log, arguments, cfg_opts)
    ...
    def prepare(self):
        ...
        :return None
        """
        super(SataEfiShellColdReset, self).prepare()

    def execute(self):
        """
        This method functionality is to execute EFIshell cold reset( reset -c) to be triggered on satassd

        :return: True
        :raise: If cold reset failed raise content_exceptions.TestFail
        """
    ...
    def cleanup(self, return_status):  # type: (bool) -> None
        ...
        try:
            ...
            raise ex
        super(SataEfiShellColdReset, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if SataEfiShellColdReset.main() else Framework.TEST_RESULT_FAIL)
