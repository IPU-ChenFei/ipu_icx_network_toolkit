...
class U2AcCycles(ResetBaseTest):
    """
    ...
    This TestCase is Used to Verify MCE Logged Status by Performing AC Cycles and verifying the status of u.2
     NVME in Every Cycle.
    """
    TEST_CASE_ID = ["16013590194", "U.2 NVMe-AC cycling (poweron to OS)"]
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        """
        super(U2AcCycles, self).__init__(test_log, arguments, cfg_opts)
    ...
    def execute(self):
        ...
        for cycle_number in range(self._common_content_configuration.get_num_of_ac_cycles()):
            ...
            if before_reboot_device_list != after_reboot_device_list:
                raise content_exceptions.TestFail("Before AC cycle and After AC cycle u.2 nvme device list is not same")
            self._log.info("Before AC cycle and After AC cycle u.2 nvme devices are same")
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if U2AcCycles.main() else Framework.TEST_RESULT_FAIL)
