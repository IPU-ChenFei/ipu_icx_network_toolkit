...
def test_steps(sut, sutos):
    ...
    tools.get('fio_l_storage').uncompress_to(method='zip')

    # install
    filename = 'FIO-linux-storage-new'
    cmd = f'sh fio_install.sh'
    print('============================cmd==========================')
    print(cmd)
    # sut.execute_shell_cmd(cmd, timeout=10 * 60, cwd=SUT_TOOLS_LINUX_STORAGE)
    sut.execute_shell_cmd(cmd, timeout=10 * 60, cwd=f'{tools.fio_l_storage.ipath}/{filename}')
    # check
    sutos.execute_cmd(sut, 'fio -v')
