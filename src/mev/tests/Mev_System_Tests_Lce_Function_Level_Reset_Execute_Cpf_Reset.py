...
def test_steps(sut, my_os):
    ...
    try:
        ...
        Case.step(' Enable lce_flr on device on ACC')
        bdf = get_bdf(mev.execute_acc_cmd, 1456, 0)
        ret, _, _ = mev.execute_acc_cmd(f'echo 1 > /sys/bus/pci/devices/{bdf}/lce_flr')
        Case.expect('LCE flr successfully.', ret == 0)

        ret_, stdout, _ = mev.execute_acc_cmd('dmesg')
