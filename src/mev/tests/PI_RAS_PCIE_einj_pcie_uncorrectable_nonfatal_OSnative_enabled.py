...
from src.configuration.config.bios.egs_bios import ras_knobs_osnative
...
def test_steps(sut, my_os):
    ...
    try:
        ...
        changed = set_bios_knobs_step(sut, *ras_knobs_osnative)
        if changed:
            my_os.g3_cycle_step(sut)
        mev.general_bring_up()

        Case.step('Create 2 VMs and ping to DHCP')
        ...
        Case.step('Inject nonfatal error')
        sut.execute_shell_cmd('dmesg -C')
        sut.execute_shell_cmd('modprobe einj')
        ret, _ ,_ = sut.execute_shell_cmd('echo 0x80 > error_type', cwd='/sys/kernel/debug/apei/einj')
        ...
        ret_1, stdout, _ = sut.execute_shell_cmd('dmesg |grep -i Non-Fatal')
        Case.expect('Check error', 'Non-Fatal' in stdout)

        Case.step('check VFs connectivity after system recovery')
        mev.pass_vf_traffic(mev.vm_list[0])
        Case.expect('ping to each other successfully.',
                    MEVOp.ping_to_dhcp(mev.vm_list[0], mode='ipv4'))
        mev.pass_vf_traffic(mev.vm_list[1])
