...
def test_steps(sut, my_os):
    ...
    try:
        ...
        Case.step('Load AVF driver and trigger LCE VF flr')
        ret, _, _ = sut.execute_shell_cmd(f'echo 16 > /sys/bus/pci/devices/{bdf}/sriov_numvfs ')
        Case.expect('create LCE VF successfully.', ret == 0)
        sut.execute_shell_cmd('modprobe vfio-pci')
        sut.execute_shell_cmd(' echo 8086 1454 > /sys/bus/pci/drivers/vfio-pci/new_id')
        bdf_list = get_bdf(sut.execute_shell_cmd, '1454', -1)
        for vf_bdf in bdf_list:
            ret, _, _ = sut.execute_shell_cmd(f'echo 1 > /sys/bus/pci/devices/{vf_bdf}/reset')
            Case.expect('LCE flr successfully.', ret == 0)
        sut.execute_shell_cmd(f'echo 0 > /sys/bus/pci/devices/{bdf}/sriov_numvfs', timeout=60)
