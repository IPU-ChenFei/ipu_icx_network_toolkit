...
TASK = 'LINUX_CENTOS_INSTALLATION'


def main():
    cmd = ['python', 'main_wrapper.py', '-v', '-t', TASK] + sys.argv[1:]
    result = subprocess.run(cmd, shell=True, check=False, cwd=r'src\environment\provision_wrapper')
