...
class PowermanagementISSTDPConfig2(IntelSSTCommon):
    """
    This test case is used for Intel speed select feature validation with TDP Config2.
    Phoenix ID : 18014074320
    """
    TEST_CASE_ID = ["18014074320", "PI_Powermanagement_ISS_TDP_Config2_L"]
    TDP_CONFIG = IntelSSTConfig.Config2
    REGEX_SST_PP = r"Intel.*SST-PP.*is\ssupported"
    TDP_LEVEL = '4'
    _BIOS_CONFIG_FILE = "power_management_iss_tdp_config2.cfg"
    BIT_TIMEOUT = 5  # in minutes
    BURNING_50_WORKLOAD_CONFIG_FILE = "cpu_memory_100_workload.txt"

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create an instance of PowermanagementISSTDPConfig2
        ...
        super(PowermanagementISSTDPConfig2, self).__init__(test_log, arguments, cfg_opts,
    ...
    def prepare(self):
        # type: () -> None
        """preparing the setup"""
        super(PowermanagementISSTDPConfig2, self).prepare()

    def execute(self):
        """test main logic to check the functionality"""
        errors = []
        intel_sst_bios_details = self.get_intel_sst_info_bios(self.TDP_CONFIG)
        msr_values = self.get_msr_values([self.MSR_x35, self.MSR_64A], convert_to=NumberFormats.BINARY)
        ...
        tdp_config2 = self._common_content_lib.convert_binary_to_decimal(
            self._common_content_lib.get_binary_bit_range(msr_values[hex(self.MSR_64A)],
                                                          self.INDICES_16_23))
        self._log.info("MSR %s value is %s", hex(self.MSR_64A), tdp_config2)

        p1_ratio = self.get_p1_ratio(IntelSSTConfig.Config2, self.TDP_LEVEL)
        ...
        if int(p1_ratio) != int(intel_sst_bios_details[self.TDP_LEVEL][self.IntelSSTTable.P1_RATIO]):
            errors.append("PythonSV p1 ratio {} and Bios {} P1 ratios are not matching".format(p1_ratio,
                                                                                      intel_sst_bios_details[self.TDP_LEVEL][self.IntelSSTTable.P1_RATIO]))
        ...
        for number in range(socket_info):
            ...
            error_values = [float(value) for value in columun_values if
                            float(value) > float(tdp_value)]

            if error_values:
                errors.append("Few CPU{} Power values are greater than {}".format(number, float(
                    tdp_value)))
    ...
    def cleanup(self, return_status):
        # type: (bool) -> None
        """DTAF cleanup"""
        super(PowermanagementISSTDPConfig2, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if PowermanagementISSTDPConfig2.main() else Framework.TEST_RESULT_FAIL)
