...
CASE_DESC = [
    "To verify platform stability by Warm Reset flow ( Fast path )"
]
...
def test_steps(sut, my_os):
    ...
    Case.step('CHANGE BIOS KNOBS -->  Enable AttemptFastBoot and AttemptFastBootCold')
    ...
    for i in range(1, 3):
        ...
        Case.step(f'Issue "shutdown -r" command in OS to reboot-{i}')
        sut.execute_shell_cmd('shutdown -r')
        Case.sleep(90)
        Case.wait_and_expect('system back to OS', 600, sut.check_system_in_os)

        Case.step(f'Check the BIOS log for subBootMode = WarmBootFast-{i}')
        bios_log_path = sut.get_bios_log()
        knob_line_in_bios_log = find_lines('subBootMode = WarmBootFast', bios_log_path)[-1]
        Case.expect('subBootMode = WarmBootFast', knob_line_in_bios_log == 'subBootMode = WarmBootFast')
