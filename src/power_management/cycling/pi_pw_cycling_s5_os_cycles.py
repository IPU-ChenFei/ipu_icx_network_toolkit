...
class PwCyclingS5OSCycles(ResetBaseTest):
    """
    HPQC ID : H82180 - PI_PW_Cycling_S5_OS_Cycles_L
    PHOENIX ID : 16013747165 - PI_PW_Cycling_S5_OS_Cycles_W
    ...
    TEST_CASE_ID = ["H82180", "PI_PW_Cycling_S5_OS_Cycles_L", "16013747165", "PI_PW_Cycling_S5_OS_Cycles_W"]
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        """
        super(PwCyclingS5OSCycles, self).__init__(test_log, arguments, cfg_opts, self._BIOS_CONFIG_FILE)
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(PwCyclingS5OSCycles, self).prepare()
    ...
    def execute(self):
        ...
        """
        test_status = True
        s5_cycles = self._common_content_configuration.get_num_of_s5_dpmo_cycles()

        self._log.info("Number of S5 cycles to be executed: %d", s5_cycles)
        if self.health_check:
            self.baseline_pcie_health_check()

        self.cng_log.__exit__(None, None, None)
        # start of cycle test for loop
        for cycle_number in range(1, s5_cycles + 1):
            ...
            with ProviderFactory.create(self.cng_cfg, self._log) as cng_log:
                ...
                if not ret_val:
                    self._log.error("Terminating the S5 cycling test...")
                    break
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if PwCyclingS5OSCycles.main()
