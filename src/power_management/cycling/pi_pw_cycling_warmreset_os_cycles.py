...
class PwCyclingWarmResetOSCycles(ResetBaseTest):
    """
    HPQC ID : H82181 - PI_PW_Cycling_WarmReset_OS_Cycles_L
    PHOENIX_ID : 16013747384 - PI_PW_Cycling_WarmReset_OS_Cycles_W
    ...
    TEST_CASE_ID = ["H82181", "PI_PW_Cycling_WarmReset_OS_Cycles_L",
                    "16013747384", "PI_PW_Cycling_WarmReset_OS_Cycles_W"]
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        """
        super(PwCyclingWarmResetOSCycles, self).__init__(test_log, arguments, cfg_opts, self._BIOS_CONFIG_FILE)
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(PwCyclingWarmResetOSCycles, self).prepare()
    ...
    def execute(self):
        ...
        """
        test_status = True
        warm_reset_cycles = self._common_content_configuration.get_num_of_warm_reset_dpmo_cycles()
        self._log.info("Number of Warm Reset cycles to be executed: %d", warm_reset_cycles)
        if self.health_check:
            self.baseline_pcie_health_check()
        self.cng_log.__exit__(None, None, None)
        for cycle_number in range(1, warm_reset_cycles + 1):
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if PwCyclingWarmResetOSCycles.main()
