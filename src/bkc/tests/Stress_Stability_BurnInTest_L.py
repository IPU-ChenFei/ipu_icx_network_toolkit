...
def test_steps(tcd):
    ...
    # Tool Version [0.2.13]
    #ID/LINK: https://hsdes.intel.com/appstore/article/#/1509775292
    #TITLE: Stress_Stability_BurnInTest_L
    #DOMAIN: system
    ...
    if tcd.prepare("setup system for test"):
        ...
        # BurnIn stress runtime has several criterial: power on: 4h,  BKC : 12h,  Volume validation: 48h
    ...
    ## 4
    if tcd.step("4"):
        sutos.execute_cmd(sut, f'rm -rf /tmp/BiTLog2.log')
        ##################
        tcd.log("### Expected result ###")
        tcd.log("New")
        ### Notes ###
    ...
    ## 6
    if tcd.step("6"):
        sutos.execute_cmd(sut, f'python run_burnin.py', timeout=180)
        ##################
        tcd.log("### Expected result ###")
        tcd.log("New")
        ### Notes ###
    ...
    ## 7
    if tcd.step("7"):
        sutos.execute_cmd(sut, f'cat /tmp/BiTLog2.log')
        ##################
        tcd.log("### Expected result ###")
        tcd.log("New")
        ### Notes ###
    ...
    ## 8
    if tcd.step("8"):
        sutos.execute_cmd(sut, f'python check_burnIn_log.py')
        ##################
        tcd.log("### Expected result ###")
