...
CASE_DESC = [
    ...
    'This case is used to verify if the SUT could boot with 1S processor in Linux OS ',
...
def test_steps(sut, my_os):
    ...
    try:
        ...
        # Step 5
        Case.step('get pci nic&nvme information')
        get_nicinfo_cmd = r"lspci | grep -i net | awk '{print $1}' | xargs -n 1 lspci -vvv -s > nic.txt"
        sut.execute_shell_cmd(get_nicinfo_cmd, cwd=remote_path)
        get_ssdinfo_cmd = r"lspci | grep -i ssd | awk '{print $1}' | xargs -n 1 lspci -vvv -s > ssd.txt"
        sut.execute_shell_cmd(get_ssdinfo_cmd, cwd=remote_path)

        Case.step('check pci nic&nvme information')
        ret_code, stdout, stderr = sut.execute_shell_cmd('./nic_nvme_check.sh', cwd="/root/sut_scripts/bkc_uniq")
        Case.expect('nic nvme info check pass', 'PASS' in stdout)

        # Step 6
        Case.step('get memory information')
        get_meminfo_cmd = r'dmidecode -t memory > mem.txt'
        sut.execute_shell_cmd(get_meminfo_cmd, cwd=remote_path)

        Case.step('check memory information')
        ret_code, stdout, stderr = sut.execute_shell_cmd('./mem_check.sh', cwd="/root/sut_scripts/bkc_uniq")
