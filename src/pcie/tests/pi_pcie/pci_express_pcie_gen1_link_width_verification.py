...
class PCIExpressPcieGen1LinkWidthVerification(PcieCommon):
    """
    Glasgow : 10526-PCI_Express_PCIe_Gen1_Link_Width_verification

    To verify that each PCI express slot in the system can link a Gen1 adapter at all supported link widths.

    """
    TEST_CASE_ID = ["G10526", "PCIExpressPCIeGen1LinkWidthVerification"]

    step_data_dict = {
        1: {'step_details': 'Check require PCIe slot is populated or not',
            'expected_results': 'Required PCIe slot is populated as Expected'},
        2: {'step_details': 'Check Populated PCIe Card has Gen1 Link speed and width',
            'expected_results': 'Populated PCIe Card has Gen1 Link speed and Width as expected'}
    }
    GEN_1 = '2.5'

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a PCIExpressPcieGen1LinkWidthVerification object

        :param test_log: Used for debug and info messages
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(PCIExpressPcieGen1LinkWidthVerification, self).__init__(test_log, arguments, cfg_opts)
    ...
    def prepare(self):  # type: () -> None
        ...
        """
        super(PCIExpressPcieGen1LinkWidthVerification, self).prepare()
    ...
    def execute(self):
        ...
        3. Check Populated Gen1 PCIe slot has Gen1 Link Stat speed and Width.
        ...
        for each_slot in pcie_slot_device_list:
            if self.GEN_1 not in each_slot[PcieSlotAttribute.PCIE_DEVICE_SPEED_IN_GT_SEC]:
                raise content_exceptions.TestFail('Slot Name: {} is not a Gen1 slot'.format(
    ...
    def cleanup(self, return_status):
        super(PCIExpressPcieGen1LinkWidthVerification, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if PCIExpressPcieGen1LinkWidthVerification.main() else
