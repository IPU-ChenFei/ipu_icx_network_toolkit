...
class VerifyMinMaxPackagePowerRangeSku(ManageabilityCommon):
    """
    This test confirms the ITP &  PythonSV reads of CPU minimum and maximum TDP power match the values from the
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create an instance of VerifyMinMaxPackagePowerRangeSku

        :param test_log: Log object
        :param arguments: None
        :param cfg_opts: Configuration Object of provider
        """
        super(VerifyMinMaxPackagePowerRangeSku, self).__init__(test_log, arguments, cfg_opts)
    ...
    def execute(self):
        ...
        self._log.info("Record the Max and Min system wattages")

        nm_max_power = self.get_nm_capabilities_obj.get_max_power(self._USER_DOMAIN_ID)
        self._log.info("Max Power Wattage from Node Manager = {}".format(nm_max_power))

        nm_min_power = self.get_nm_capabilities_obj.get_min_power(self._USER_DOMAIN_ID)
        self._log.info("Min Power Wattage from Node Manager = {}".format(nm_min_power))

        self._log.info("Record the Max, Tdp and Min system wattages using cscripts!")

        tdp_power_from_cscripts = self._miv_cscripts_obj.get_cscripts_tdp_power_value()
        self._log.info("Max (TDP) Power Wattage from cscripts = {}".format(tdp_power_from_cscripts))

        min_power_from_cscripts = self._miv_cscripts_obj.get_cscripts_min_power_value()
        self._log.info("Min Power Wattage from cscripts = {}".format(min_power_from_cscripts))

        max_power_from_cscripts = self._miv_cscripts_obj.get_cscripts_max_power_value()
        self._log.info("Max Power Wattage from cscripts = {}".format(max_power_from_cscripts))

        if float(nm_max_power) != float(tdp_power_from_cscripts):
            log_error = "Max Power Wattage from Node Manager Command does not match with Max Power Wattage from ITP" \
                        " or PythonSV"
            self._log.error(log_error)
            raise RuntimeError(log_error)

        if float(nm_min_power) != float(min_power_from_cscripts):
            log_error = "Min Power Wattage from Node Manager Command does not match with Min Power Wattage from ITP" \
                        " or PythonSV"
            self._log.error(log_error)
            raise RuntimeError(log_error)

        self._log.info("Max and Min Power Wattages from Node Manager matches with Max and Min Power Wattages from"
                       " ITP or PythonSV")
    ...
    def cleanup(self, return_status):
        """Test Cleanup"""
        super(VerifyMinMaxPackagePowerRangeSku, self).cleanup(return_status=True)
        if os.path.isfile(self.get_nm_capabilities_obj.miv_log_file):
            os.remove(self.get_nm_capabilities_obj.miv_log_file)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if VerifyMinMaxPackagePowerRangeSku.main() else Framework.TEST_RESULT_FAIL)
