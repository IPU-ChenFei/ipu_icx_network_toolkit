...
class WarmResetCycleWithSandstoneTest(StressStabilityCommon):
    """
    This class is used to check Warm reset with sandstone stress
    Pheonix ID: 16014494148 - PI_Warm_Reset_Cycle_With_Sandstone_Test
    """
    TEST_CASE_ID = ["16014494148", "PI_Warm_Reset_Cycle_With_Sandstone_Test"]

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create an instance of WarmResetCycleWithSandstoneTest

        :param cfg_opts: Configuration Object of provider
        :param test_log: Log object
        :param arguments: None
        """
        super(WarmResetCycleWithSandstoneTest, self).__init__(test_log, arguments, cfg_opts)
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(WarmResetCycleWithSandstoneTest, self).prepare()
    ...
    def execute(self):
        ...
        for cycle in range(total_reboots):
            ...
            ipmi_reset_cmd = self.os.execute(SandstoneTestConstants.IPMI_TOOL_POWER_RESET, self._command_timeout)
            if ipmi_reset_cmd.cmd_failed():
                self._log.error("IPMI power reset command failed. Debug output: {}".format(ipmi_reset_cmd))
                raise content_exceptions.TestFail("ipmi command failed on iteration {} of {}.".format(cycle,
                                                                                                      total_reboots))
            self.os.wait_for_os(self.reboot_timeout)
            self._log.info("system is alive.")

        host_path = self._common_content_lib.copy_log_files_to_host("16014494148", SandstoneTestConstants.ROOT, ".log")
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if WarmResetCycleWithSandstoneTest.main() else Framework.TEST_RESULT_FAIL)
