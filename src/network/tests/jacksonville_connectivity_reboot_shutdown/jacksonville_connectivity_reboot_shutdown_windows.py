...
class JacksonvilleConnectivityRebootShutdownWindows(NetworkingCommon):
    """
    HPQC ID : H81653-PI_Networking_Jacksonville_ConnectivityReboot_shutdown_W

    This case is used to check the networking connectivity after Reboot SUT.
    """
    TEST_CASE_ID = ["H81653", "PI_Networking_Jacksonville_ConnectivityReboot_shutdown_W"]
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create an instance of JacksonvilleConnectivityRebootShutdownWindows.

        :param cfg_opts: Configuration Object of provider
        :param test_log: Log object
        :param arguments: None
        """
        super(JacksonvilleConnectivityRebootShutdownWindows, self).__init__(test_log, arguments, cfg_opts)
    ...
    def execute(self):  # type: () -> bool
        ...
        if not self.network_provider.is_sut_pingable():
            raise content_exceptions.TestFail("System is not Ping as Expected after graceful G3")
        ...
        for iteration in range(self.NUMBER_OF_ITERATIONS):
            ...
            self.perform_graceful_g3()
            if not self.network_provider.is_sut_pingable():
                raise content_exceptions.TestFail("System is not Ping as Expected after graceful G3 Iteration {}")
            self._log.debug("System is Pinging and Reachable after Iteration {}".format(iteration + 1))
        self._log.debug("System is Pinging and Reachable for all graceful G3 {} Iterations".
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if JacksonvilleConnectivityRebootShutdownWindows.main()
