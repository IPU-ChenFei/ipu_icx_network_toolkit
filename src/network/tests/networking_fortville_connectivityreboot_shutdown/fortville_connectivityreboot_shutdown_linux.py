...
class FortvilleConnectivityRebootShutdownLinux(NetworkingCommon):
    """
    HPQC ID : H93819-PI_Networking_Fortville_ConnectivityReboot_shutdown_linux

    This case is used to check the networking connectivity for Fortville Driver after Reboot SUT.
    """
    TEST_CASE_ID = ["H93819", "PI_Networking_Fortville_ConnectivityReboot_shutdown_linux"]
    ...
    STEP_DATA_DICT = {
        1: {'step_details': "Boot to Linux OS, make sure Network adapter driver has been installed successfully."
            ...
            'expected_results': "Network's driver has been installed currently."
        ...
        3: {'step_details': "Shutdown the SUT, manually power on SUT, then ping DHCP server's IP address.",
            'expected_results': "Network driver will auto loading after power on SUT."
                                "Boot to OS, ping DHCP server's IP address is successful."},
        4: {'step_details': "Shutdown and boot SUT five times,"
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create an instance of FortvilleConnectivityRebootShutdownLinux.

        :param cfg_opts: Configuration Object of provider
        :param test_log: Log object
        :param arguments: None
        """
        super(FortvilleConnectivityRebootShutdownLinux, self).__init__(test_log, arguments, cfg_opts)
    ...
    def prepare(self):  # type: () -> None
        ...
        preparing the setup
        """
        self.driver_provider.install_driver(NetworkDrivers.FORTVILLE_DRIVER_CODE,
    ...
    def execute(self):  # type: () -> bool
        ...
        self.network_provider.get_network_adapter_interfaces(assign_static_ip=True)
        if not self.network_provider.ping_network_adapter_ip(self.ip_address):
            raise content_exceptions.TestFail("Network Adapter is not Pinging as Expected after graceful G3")
        ...
        for iteration in range(self.NUMBER_OF_ITERATIONS):
            ...
            if not self.network_provider.ping_network_adapter_ip(self.ip_address):
                raise content_exceptions.TestFail(
                    "Network Adapter is not Ping as Expected after shutdown Iteration {}".format(iteration + 1))
            self._log.info("Network Adapter is Pinging and Reachable after Shutdown Iteration {}".format(iteration + 1))
        self._log.info("Network Adapter is Pinging and Reachable for all Shutdown {} Iterations".
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if FortvilleConnectivityRebootShutdownLinux.main()
