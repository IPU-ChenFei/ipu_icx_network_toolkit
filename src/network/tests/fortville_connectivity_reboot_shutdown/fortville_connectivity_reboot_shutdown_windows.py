...
class FortvilleConnectivityRebootShutdownWindows(NetworkingCommon):
    """
    HPQC ID : H93834-PI_Networking_Fortville_ConnectivityReboot_Shutdown_W

    This Class is used to check the networking connectivity after Reboot and Shutdown Cycles of SUT.
    """
    TEST_CASE_ID = ["H93834", "PI_Networking_Fortville_ConnectivityReboot_Shutdown_W"]
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create an instance of FortvilleConnectivityRebootShutdownWindows.

        :param cfg_opts: Configuration Object of provider
        :param test_log: Log object
        :param arguments: None
        """
        super(FortvilleConnectivityRebootShutdownWindows, self).__init__(test_log, arguments, cfg_opts)
    ...
    def prepare(self):  # type: () -> None
        """
        PreChecks if the Fortville Driver is installed in System.
        """
    ...
    def execute(self):  # type: () -> bool
        ...
        for iteration in range(self.NUMBER_OF_ITERATIONS):
            ...
            self.perform_graceful_g3()
            if not self.network_provider.ping_network_adapter_ip(self.ip_address):
                raise content_exceptions.TestFail("System is not Pinging after Shutdown Iteration {}")
            self._log.debug("System is Pinging and Reachable after Shutdown Iteration {}".format(iteration + 1))
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if FortvilleConnectivityRebootShutdownWindows.main()
