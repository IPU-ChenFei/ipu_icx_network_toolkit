...
def test_steps(sut, my_os):
    ...
    try:
        ...
        # Step2 - Make sure ATE kernel is loaded on XHC and working
        Case.step("Make sure ATE kernel is loaded on XHC and working")
        _, stdout, _ = sut.execute_shell_cmd('uname -r | cut -c "1-4"', 30)
        Case.expect('Check ATE kernel value is 5.10', "5.10" in stdout)

        # Step3 - check MEV device on SUT
        ...
        # Step4 - Uninstall the nvme drive
        Case.step('Uninstall the nvme drive and install  vfio-pci')
        return_code, _, _ = sut.execute_shell_cmd('modprobe -r nvme', 30)
        Case.expect('return value is 0', return_code == 0)

        # Step5 - install vfio-pci
        Case.step('install  vfio-pci')
        ...
        # Step7 - Get LAN CPF IOMMU group of device
        ...
        # Step12 - Get IP from DHCP for VF0
        Case.step('Get IP from DHCP for VF0 and ping from vm0 to DHCP server')
        vm0 = mev.vm_list[0]
        mev.pass_vf_traffic(vm0)
        Case.expect('ping to DHCP client successfully.', MEVOp.ping_to_dhcp(vm0, mode='ipv4'))

        # Step14 - Run in parallel non-stop ping from VF on VM and nvme_cpf_integration_test in a loop
        Case.step('Get NVME CPF  and NVME PF are  bdf')
        nvme_bdf_pf = get_bdf(sut.execute_shell_cmd, 1457, 0)
        nvme_bdf_cpf = get_bdf(sut.execute_shell_cmd, 1458, 0)
        Case.step('Run in parallel non-stop ping from VF on VM and nvme_cpf_integration_test in a loop')
        return_code, stdout, stderr = sut.execute_shell_cmd(
        ...
        # Step15 - Clear autoload.sh file
        Case.step("Clear autoload.sh file")
        return_code, _, _ = sut.execute_shell_cmd('rm -rf autoload.sh ', 30, cwd='/root')
        Case.expect('return value is 0', return_code == 0)
