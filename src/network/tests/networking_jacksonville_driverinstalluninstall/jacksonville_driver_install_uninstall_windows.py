...
class JacksonvilleDriverInstallUninstallWindows(NetworkingCommon):
    """
    HPQC ID : H80078-PI_Networking_JacksonVille_DriverInstallUninstall_W

    This Class is Used for Installation and Uninstallation of Jacksonville Network Driver for Windows OS.
    """
    TEST_CASE_ID = ["H80078", "PI_Networking_Jacksonville_DriverInstallUninstall_W"]
    STEP_DATA_DICT = {
        ...
        3: {'step_details': 'Install Jacksonville driver in sut using make',
            'expected_results': 'SUT is pinging successfully'}}

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create an instance of JacksonvilleDriverInstallUninstallWindows.

        :param cfg_opts: Configuration Object of provider
        :param test_log: Log object
        :param arguments: None
        """
        super(JacksonvilleDriverInstallUninstallWindows, self).__init__(test_log, arguments, cfg_opts)
    ...
    def execute(self):  # type: () -> bool
        ...
        2) Uninstall Jacksonville Driver and verifies the sut is not pinging.
        3) Install Jacksonville Driver and verifies the sut is pinging.
        ...
        self._driver_provider.copy_driver_files(self.phy)
        self.driver_provider.install_driver(self.driver_inf_file, self.driver_device_id)
        self._test_content_logger.end_step_logger(1, return_val=True)

        self._test_content_logger.start_step_logger(2)
        self._driver_provider.uninstall_driver(self.driver_inf_file, self.driver_device_id)
        if self.network_provider.is_sut_pingable():
            raise content_exceptions.TestFail("System is still Pinging after driver uninstallation")
        self._log.info("As expected sut is not pinging after uninstalling driver")
        self._test_content_logger.end_step_logger(2, return_val=True)

        self._test_content_logger.start_step_logger(3)
        self._driver_provider.install_driver(self.driver_inf_file, self.driver_device_id)
        ...
        if not self.network_provider.is_sut_pingable():
            raise content_exceptions.TestFail("System is not Pinging after driver is installed")
        self._log.info("SUT is pinging after driver is installed")
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if JacksonvilleDriverInstallUninstallWindows.main()
