...
CASE_DESC = [
    "this test case is used to verify dpdk working with different mtu"
]
...
def test_steps(sut, my_os):
    ...
    try:
        ...
        Case.step("start tcpdump")
        sut2.execute_shell_cmd_async(RUN_TCPDUMP.format(sut2_eth1_name),
                                     cwd=bhs_sut_tools.SUT_TOOLS_LINUX_NETWORK)

        Case.step("start testpmd test")
        ret, out, err = sut1.execute_shell_cmd_async(f'python3 dpdk_expect.py start_dpdk {tool_p}',
                                                     cwd=bhs_sut_tools.SUT_TOOLS_LINUX_NETWORK)
        Case.expect('testpmd should be launched and working without errors.', ret == 0)

        #sut2 need pip3 install pexpect --proxy=http://child-prc.intel.com:913
        Case.step("start scapy")
        sut2.execute_shell_cmd(f"tar -xzvf {tool}", cwd=bhs_sut_tools.SUT_TOOLS_LINUX_NETWORK)
        ret, out, err = sut2.execute_shell_cmd(
            f'python3 dpdk_expect.py send_packet1 {sut1_mac1_name} {sut1_eth1_name} {scapy_tool_path}', 60 * 5,
            cwd=bhs_sut_tools.SUT_TOOLS_LINUX_NETWORK)
        Case.expect('send packet 1 successful !', ret == 0)

        Case.step("start testpmd test two")
        ret, out, err = sut1.execute_shell_cmd_async(f'python3 dpdk_expect.py start_dpdk_2 {tool_p}',
                                                     cwd=bhs_sut_tools.SUT_TOOLS_LINUX_NETWORK)
        Case.expect('testpmd should be launched and working without errors.', ret == 0)

        Case.step("start scapy two")
        ret, out, err = sut2.execute_shell_cmd(
            f'python3 dpdk_expect.py send_packet2 {sut1_mac1_name} {sut1_eth1_name} {scapy_tool_path}', 60 * 5,
            cwd=bhs_sut_tools.SUT_TOOLS_LINUX_NETWORK)
        Case.expect('send packet 2 successful !', ret == 0)

        Case.step("start testpmd test three")
        ret, out, err = sut1.execute_shell_cmd_async(f'python3 dpdk_expect.py start_dpdk_3 {tool_p}',
                                                     cwd=bhs_sut_tools.SUT_TOOLS_LINUX_NETWORK)
        Case.expect('testpmd should be launched and working without errors.', ret == 0)

        Case.step("start scapy three")
        ret, out, err = sut2.execute_shell_cmd(
            f'python3 dpdk_expect.py send_packet3 {sut1_mac1_name} {sut1_eth1_name} {scapy_tool_path}', 60 * 5,
            cwd=bhs_sut_tools.SUT_TOOLS_LINUX_NETWORK)
        Case.expect('send packet 3 successful !', ret == 0)

        Case.step("download log")
        sut1.download_to_local(remotepath=f'{bhs_sut_tools.SUT_TOOLS_LINUX_NETWORK}/send_package1.log',
                              localpath=os.path.join(LOG_PATH, 'result'))
        sut1.download_to_local(remotepath=f'{bhs_sut_tools.SUT_TOOLS_LINUX_NETWORK}/send_package2.log',
                              localpath=os.path.join(LOG_PATH, 'result'))
        sut1.download_to_local(remotepath=f'{bhs_sut_tools.SUT_TOOLS_LINUX_NETWORK}/send_package3.log',
                               localpath=os.path.join(LOG_PATH, 'result'))
        sut2.download_to_local(remotepath=f'{bhs_sut_tools.SUT_TOOLS_LINUX_NETWORK}/tcpdump.log',
                              localpath=os.path.join(LOG_PATH, 'result'))
