...
def test_steps(sut, my_os):
    ...
    nic1 = valos.get_ether_name(sut1, sut_pci_type, 0)
    for i in range(1):
        try:
            #ret = sut1.ssh_connection(60, sut1.check_ssh_status)
            ret = sut1.execute_shell_cmd("nmcli device disconnect {}".format(nic1))
            Case.expect("nmcli disconnect successful", ret[0] == 0)
            Case.sleep(5)
            #ret = sut1.ssh_connection(60, sut1.check_ssh_status)
            ret, out, err = sut1.execute_shell_cmd("ping -c 6 {}".format(conn.port2.ip))
            Case.sleep(5)
            if ret == 2:
                Case.expect("Network is unreachable", ret == 2)
            else:
                lost_data = re.search(r'(\d+)% packet loss', out)
                Case.expect("ping failed", int(lost_data.group(1)) == 100)

            Case.step("enable ipv4")
            #ret = sut1.ssh_connection(60, sut1.check_ssh_status)
            ret = sut1.execute_shell_cmd("nmcli device connect {}".format(nic1))
            Case.expect("nmcli connect successful", ret[0] == 0)
            Case.sleep(5)
            #ret = sut1.ssh_connection(60, sut1.check_ssh_status)
            ret = sut1.execute_shell_cmd("ping -c 20 {}".format(conn.port2.ip))
            Case.expect("ping successful", ret[0] == 0)
        except Exception as e:
            #ret = sut1.ssh_connection(60, sut1.check_ssh_status)
            ret = sut1.execute_shell_cmd("ifup {}".format(nic1))
            Case.sleep(5)
            ret = sut1.execute_shell_cmd("nmcli device connect {}".format(nic1))
            Case.sleep(5)
            raise
