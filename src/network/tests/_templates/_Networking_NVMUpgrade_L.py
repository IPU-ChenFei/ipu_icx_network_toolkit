...
CASE_DESC=[
    'connect sut1 fortville nic port to sut2 fortville nic port cable',
...
def __firmware_update(sut, path, mac):
    sut.execute_shell_cmd(f'chmod 777 nvmupdate64e', cwd=path)
    ret_code, stdout, stderr = sut.execute_shell_cmd('./nvmupdate64e -u -m {} -l -o update.log -c nvmupdate.cfg'.format(mac), timeout=60 * 30, cwd=path)
    ...
    ret_code, stdout, stderr = sut.execute_shell_cmd('cat update.log', cwd=path)
...
def test_steps(sut, my_os):
    ...
    flag = tool.split("_")[0]
    tool_path = "{0}/{1}Upgrade/{1}/Linux_x64".format(valos.tool_path, flag)
    ret = valos.unzip_tools(sut1, tool, "{}Upgrade".format(flag))
    Case.expect("unzip sut1 update tool", ret)
    ret = valos.unzip_tools(sut2, tool, "{}Upgrade".format(flag))
