...
from src.network.lib.utility import execute_host_cmd
...
class VM(GenericComponent):
    ...
    def create(self, mem_sz=2048, cpu_num=2, hugepage=False):
        qemu_cmd = "qemu-img create -f qcow2 -F qcow2 -b {}/centos8_vm.qcow2 " \
                   "/var/lib/libvirt/images/{}.qcow2".format(sut_tool('SUT_TOOLS_LINUX_MEV'), self.__name)
    ...
    def __create_sriov_xml(self, bdf):
        ...
        slot = bdf[8:10]
        function = bdf[11:]
        xml_path = f"{sut_tool('HOST_TMP_MEV')}\\pci_device.xml"
        ...
        xml_tmp = f"{sut_tool('HOST_TMP_MEV')}\\xml_tmp"
        if not os.path.exists(xml_tmp):
            os.makedirs(xml_tmp)
        vf_file = f'{xml_tmp}\\pci_device_{self.__name}_{bus}_{slot}_{function}.xml'
        conf.write(vf_file)
        self._sut.upload_to_remote(vf_file, f"{sut_tool('SUT_TOOLS_LINUX_MEV')}/xml_tmp")
        execute_host_cmd(f'del /s /q {vf_file}')
        return f"{sut_tool('SUT_TOOLS_LINUX_MEV')}/xml_tmp/pci_device_{self.__name}_{bus}_{slot}_{function}.xml"
