...
def get_vm_ip(vm_name, net_interface):
    ...
    """
    # net_interface = _get_network_interface()
    ...
    log.info("vm net domain returns: \n{}".format(cmd))
...
def _pexpect_ssh_cmd(host, cmd):
    log.info("pexpect spawning cmd: ssh root@{} {} ".format(host, cmd))
    p_exp = pexpect.spawn('ssh root@{} {}'.format(host, cmd))
...
if __name__ == '__main__':
    ...
    sut_no, vm_name, net_inter, d = None, None, None, False
    opts, args = getopt.getopt(sys.argv[1:], shortopts="d",
                               longopts=["sut_no=", "vm_name=", "net_inter="])
    for opt, val in opts:
        if opt in ('-d', '--delete'):
            d = True
        elif opt == '--sut_no':
    ...
    # Clear VM data when delete is set
    if d:
        # ping sut1 from sut2 before deleting
        if sut_no == 'sut2':
            sut_vm_ip = get_vm_ip(vm_name, net_inter)
            ssh_and_ping(vm_ip=sut_vm_ip, vm2_ip=f'192.168.13.100')

        clear_vm(vm_name)
        sys.exit()

    # Create_vm(vm, 2, 15, 2048)
    create_vm_from_qcow2(vm_name, 2048, 2)
    ...
    if sut_no == 'sut1':
        ...
        _pexpect_ssh_cmd(sut_vm_ip, 'ifconfig {} 192.168.13.101/24 up'.format(vm_interface))

    # disable firewall on vm
    _handle_vm_firewall(sut_vm_ip)

    if sut_no.lower() == 'sut1':
        # pinging sut2 from sut1
        ssh_and_ping(vm_ip=sut_vm_ip, vm2_ip='192.168.13.101')

    else:
        log.info("Sut2 script done, added port configured to ip 192.168.13.101")
        sys.stdout.write("192.168.13.101")
