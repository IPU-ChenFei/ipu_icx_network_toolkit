...
if __name__ == '__main__':
    vm_name, net_inter = None, None
    opts, args = getopt.getopt(sys.argv[1:],  shortopts="",
                               longopts=["vm_name=", "net_inter="])
    for opt, val in opts:
        if opt == '--net_inter':
            net_inter = val
        elif opt == '--vm_name':
    ...
    _pexpect_ssh_cmd(sut_vm_ip, r'./t /pkg {package} /reconfig /pc {configuration} /flow {flow} /run /minutes {duration} /V -1 /quit')
