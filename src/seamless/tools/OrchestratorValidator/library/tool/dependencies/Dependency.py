...
class Dependency:
    ...
    class Tags:
        ...
        defaultTag = "default"
        targetPropertyTag = "target_property"
    ...
    def _parse_properties_dict(self, properties):
        ...
        if self.Tags.targetPropertyTag in properties:
            tag_content = properties[self.Tags.targetPropertyTag]
            try:
                self.target_property = self.Targets(tag_content)
            except ValueError:
                raise DependencyException(f"{tag_content} target property is not supported", self)
        if self.Tags.calculateTag in properties:
            self.calculate = properties[self.Tags.calculateTag]
