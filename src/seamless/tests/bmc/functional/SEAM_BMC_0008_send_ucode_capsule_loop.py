...
class SEAM_BMC_0008_send_ucode_capsule_loop(SeamlessBaseTest):
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        self.bmc_timeout_min = 30
        self.bmc_timeout_max = 40
    ...
    def add_arguments(cls, parser):
        ...
        parser.add_argument('--ac_while_staging', action='store_true',
                            help="Add this argument if need to to AC while staging")
    ...
    def get_current_version(self, echo_version=True):
        ...
        version = None
        if (self.HDE_16015195100):
            time.sleep(random.randint(self.bmc_timeout_min,self.bmc_timeout_max))
    ...
    def trigger_update(self, slot1_path, slot2_path):
        if self.update_type == 'inband':
            ...
            if self.ac_while_staging:
                ...
                return self.send_capsule_ac_while_staging(slot1_path, self.CAPSULE_TIMEOUT, self.start_workload)
            else:
                self._log.info("Sending upgrade capsule...")
            ...
            if self.ac_while_staging:
                ...
                return self.send_capsule_ac_while_staging(slot1_path, self.CAPSULE_TIMEOUT, self.start_workload, capsule_path2=slot2_path, update_type='ucode')
            else:
                self._log.info("Sending downgrade capsules...")
    ...
    def execute(self):
        ...
        if self.ac_while_staging:
            for x in range(self.loop_count):
                self._log.info("\n")
                self._log.info("Executing Update 1 on Loop # {}".format(x))
                self.summary_log.info("\n\n")
                self.summary_log.info("Results of Update 1 on Loop # {}".format(x))
