...
class SEAM_BMC_0015_smm_update(SeamlessBaseTest):
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        self.dmesg_enable = "echo -n 'file pfr_update.c +p; file pfr_telemtry.c +p' > /sys/kernel/debug/dynamic_debug/control"
    ...
    def execute(self):
        ...
        if self._os_type == OperatingSystems.LINUX:
            ...
            self._log.info(f"checking kernel version: {self.run_ssh_command('uname -r')}")
        ...
        try:
            if (self.smm_command == "get_version"):
                ...
                cmd = 'queryCapability'
                if self._os_type == OperatingSystems.LINUX:
                    cmd = '/root/smm_update/pfrut -q /| tee /root/smm_update/SSTS/SSTS.log'
                ...
                for x in range(self.CSR_loop ):
                    if self._os_type == OperatingSystems.LINUX:
                        ...
                        cmd =  "/root/smm_update/pfrut -l " + self.linux_capsule_path + self.smm_capsule+ " -u -R /| tee /root/smm_update/SSTS/SSTS.log"
                        result = self.run_ssh_command(cmd)
                    ...
                    if ('Execution Succeeded' in output1):
                        ...
                        if self._os_type == OperatingSystems.LINUX:
                            self.run_ssh_command(self.dmesg_c,log_output=False)
                            cmd =  "/root/smm_update/pfrut -l " + self.linux_capsule_path + self.smm_capsule2+ " -u -R /| tee /root/smm_update/SSTS/SSTS.log"
                            result = self.run_ssh_command(cmd)
                            if 'Access Denied' in result.stdout or 'Unsupported' in result.stdout or 'Invalid Parameter' in result.stdout:
...
# need to add code injection status also KPI check
                ...
                for x in range(self.loop):
                    ...
                    if self.update_type == "negative":
                        ...
                        if (str(self.expected_ver) == str(post_ver)):
                            ...
                            result = False
                        if self._os_type == OperatingSystems.LINUX:
                            cmd =  "/root/smm_update/pfrut -D " + str(self.log_rev)
                            result = self.run_ssh_command(cmd)
                        ...
                        if 'Execution Succeeded' in command1:
                            ...
                            if self._os_type == OperatingSystems.LINUX:
                                cmd =  "/root/smm_update/pfrut -T " + str(self.log_type)
                                result = self.run_ssh_command(cmd)
                                if 'Invalid' in result.stdout: #or '' in result.stdout:
                                    result = False
                        ...
                        if self._os_type == OperatingSystems.LINUX:
                            self.linux_log_level()
                            cmd =  "/root/smm_update/pfrut -L " + str(self.log_level)
                            result = self.run_ssh_command(cmd)
                ...
                if self._os_type == OperatingSystems.LINUX:
                    self.run_ssh_command(self.dmesg_c,log_output=False)
                    cmd =  "/root/smm_update/pfrut -l " + self.linux_capsule_path + self.smm_capsule+ " -s -R /| tee /root/smm_update/SSTS/SSTS.log"
                    result = self.run_ssh_command(cmd)
                    if 'Access Denied' in result.stdout or 'Unsupported' in result.stdout or 'Invalid Parameter' in result.stdout:
                        result = False
                ...
                if self._os_type == OperatingSystems.LINUX:
                    self.run_ssh_command(self.dmesg_c,log_output=False)
                    #cmd = 'echo 2 > /sys/firmware/smm/start'
                    cmd =  "/root/smm_update/pfrut -l " + self.linux_capsule_path + self.smm_capsule+ " -a -R /| tee /root/smm_update/SSTS/SSTS.log"
                    result = self.run_ssh_command(cmd)
                    if 'Access Denied' in result.stdout or 'Unsupported' in result.stdout or 'Invalid Parameter' in result.stdout:
                        result = False
                        raise RuntimeError("Code injection returned error")
