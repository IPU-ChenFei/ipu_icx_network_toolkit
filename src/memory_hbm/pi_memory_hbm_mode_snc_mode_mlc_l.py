...
class PiMemoryHBMSNCMlcLinux(MemoryHbmCommon):
    """
     Pheonix ID: 16014743114 - PI_Memory_HBM_Mode_SNC_Mode_MLC_L

    """

    TEST_CASE_ID = ["16014743114", "PI_Memory_HBM_Mode_SNC_Mode_MLC_L"]
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create a new PiMemoryHBMSNCMlcLinux object.
        ...
        super(PiMemoryHBMSNCMlcLinux, self).__init__(test_log, arguments, cfg_opts, bios_config_file)
    ...
    def prepare(self):
        ...
        super(PiMemoryHBMSNCMlcLinux, self).prepare()
    ...
    def execute(self):
        ...
        for each_socket in range(0, sockets_count):
            value = self.SV.get_by_path(self.SV.UNCORE, PlatformConfiguration.MEMORY_KTILK_SNC_CONFIG[self._silicon_family],
                                        socket_index=each_socket)
            self._log.info("Socket {} value is :{}".format(each_socket, value))

            if self.SNC_EXPECTED_VALUE not in hex(value):
    ...
    def cleanup(self, return_status):
        ...
        """DTAF cleanup"""
        self._stress_provider.kill_stress_tool(MLCToolConstants.MLC_TOOL_NAME, self.MLC_COMMAND)
        super(PiMemoryHBMSNCMlcLinux, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if PiMemoryHBMSNCMlcLinux.main() else Framework.TEST_RESULT_FAIL)
