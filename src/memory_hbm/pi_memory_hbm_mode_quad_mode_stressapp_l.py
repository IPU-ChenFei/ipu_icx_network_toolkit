...
class PiMemoryHBMModeQuadStressAppLinux(ContentBaseTestCase):
    """
    Pheonix id : 16014743004 - PI_Memory_HBM_Mode_QUAD_Mode_Stressapp_L

    """

    TEST_CASE_ID = ["16014743004", "PI_Memory_HBM_Mode_QUAD_Mode_Stressapp_L"]
    ...
    HBM_QUAD_BIOS_CONFIG_FILE = "one_lm_quad_mode_bios_knob.cfg"
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        :raises: None
        """
        self.hbm_quad_bios_config_file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                                       self.HBM_QUAD_BIOS_CONFIG_FILE)

        # calling base class init
        super(PiMemoryHBMModeQuadStressAppLinux, self).__init__(test_log, arguments, cfg_opts)
    ...
    def execute(self):
        ...
        1. Set and verify SNC Bios knob to SNC4 and verify PythonSv register values snc_config
        2. run the StressAppTest tool for 1 Hours.
        ...
        self.snc_check_with_pythonsv_values(self.hbm_quad_bios_config_file_path, self.QUAD_EXPECTED_VALUE)
    ...
    def snc_check_with_pythonsv_values(self, bios_file_path, expected_reg_value):
        ...
        for each_socket in range(0, sockets_count):
            ...
            value = self.SV.get_by_path(
                self.SV.UNCORE, PlatformConfiguration.MEMORY_SNC_CONFIG[self._silicon_family],
    ...
    def cleanup(self, return_status):
        ...
        """DTAF cleanup"""
        self._stress_provider.kill_stress_tool(StressAppTestConstants.STRESS_APP_TOOL_NAME, self.STRESS_APP_COMMAND)
        super(PiMemoryHBMModeQuadStressAppLinux, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if PiMemoryHBMModeQuadStressAppLinux.main() else Framework.TEST_RESULT_FAIL)
