...
class PiMemoryHBMModeS0S5S0Windows(MemoryHbmCommon):
    """
    Pheonix ID:16014744132 - PI_Memory_HBM_Mode_PM_S0_S5_S0_W

    """

    TEST_CASE_ID = ["16014744132", "PI_Memory_HBM_Mode_PM_S0_S5_S0_W"]
    ...
    step_data_dict = {1: {'step_details': 'To verify SUT has no DDR memory configuration ...',
                      ...
                      3: {'step_details': 'Check Mce errors, Dc cycle the system and verify for Number of nodes '
                                          'and node memory',
                          'expected_results': 'Verified Number of Nodes and node memory'}}

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create a new PiMemoryCacheModeS0S5S0Linux object.
        ...
        super(PiMemoryHBMModeS0S5S0Windows, self).__init__(test_log, arguments, cfg_opts, bios_config_file_path)

        self._s5_object = ResetBaseTest(self._log, arguments, cfg_opts)
    ...
    def prepare(self):
        ...
        super(PiMemoryHBMModeS0S5S0Windows, self).prepare()
    ...
    def execute(self):
        ...
        2. DC cycle and check the number of nodes and node memory.
        ...
        for cycle_number in range(1, self.NUMBER_OF_CYCLES + 1):
            self._log.info("DC cycle %d", cycle_number)
            ...
            self.verify_number_of_nodes(memory_mode=PlatformMode.HBM_MODE,
    ...
    def cleanup(self, return_status):
        # type: (bool) -> None
        """DTAF cleanup"""
        super(PiMemoryHBMModeS0S5S0Windows, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if PiMemoryHBMModeS0S5S0Windows.main() else Framework.TEST_RESULT_FAIL)
