...
class PiMemoryHBMModeG3CycleWindows(MemoryHbmCommon):
    """
    Pheonix ID:16014743954 -PI_Memory_HBM_Mode_G3_Cycle_W
    """

    TEST_CASE_ID = ["16014743954", "PI_Memory_HBM_Mode_G3_Cycle_W"]
    BIOS_CONFIG_FILE = "one_lm_bios_knob.cfg"
    G3_CYCLES = 10

    step_data_dict = {1: {'step_details': 'To verify SUT has no DDR memory configuration ...',
                          'expected_results': 'SUT has no DDR expected memory configuration ...'},
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create a new PiMemoryHBMModeG3CycleWindows object.
        ...
        super(PiMemoryHBMModeG3CycleWindows, self).__init__(test_log, arguments, cfg_opts, bios_config_file_path)
    ...
    def prepare(self):
        ...
        super(PiMemoryHBMModeG3CycleWindows, self).prepare()
    ...
    def cleanup(self, return_status):
        # type: (bool) -> None
        """DTAF cleanup"""
        super(PiMemoryHBMModeG3CycleWindows, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if PiMemoryHBMModeG3CycleWindows.main() else Framework.TEST_RESULT_FAIL)
