...
class SldLoggingUefiEdpcDisable(PcieSurpriseLinkDownBaseTest):
    """
    ...
    """
    BIOS_CONFIG_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                    "sld_logging_edpc_disable_bios_knobs.cfg")

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a SldLoggingUefiEdpcDisable object

        :param test_log: Used for debug and info messages
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(SldLoggingUefiEdpcDisable, self).__init__(test_log, arguments, cfg_opts, self.BIOS_CONFIG_FILE)
    ...
    def prepare(self):  # type: () -> None
        ...
        prepare
        """
        super(SldLoggingUefiEdpcDisable, self).prepare()
    ...
    def execute(self):
        ...
        try:
            ...
            if not self.run_sld_edpc(sdp=self.sdp_obj, socketnum_pxpport_list=self._pxp_port_list,
                                                      loop=4, edpc_enable=False):
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if SldLoggingUefiEdpcDisable.main() else Framework.TEST_RESULT_FAIL)
