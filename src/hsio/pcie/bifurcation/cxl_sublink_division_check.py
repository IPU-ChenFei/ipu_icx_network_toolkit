...
class CxlSubLinkDivisionCheck(BifurcationBaseTest):
    """
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a CxlSubLinkDivisionCheck object

        :param test_log: Used for debug and info messages
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(CxlSubLinkDivisionCheck, self).__init__(test_log, arguments, cfg_opts)

    def prepare(self):  # type: () -> None
        """
        prepare
        """
        super(CxlSubLinkDivisionCheck, self).prepare()
    ...
    def execute(self):
        ...
        if self._common_content_configuration.get_pcie_bifurcation_auto_discovery():
            self.validate_speed_width_in_os(protocol=PxpInventory.IOProtocol.CXL)
        else:
            self.validate_speed_and_width_in_os(protocol=PxpInventory.IOProtocol.CXL)
        return True


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if CxlSubLinkDivisionCheck.main() else
