...
class CxlCtgBaselineCacheOp4(CxlCtgCommon):
    """
    hsdes_id :  16015657097

    Objective:
    Generate CXL.cache traffic using CTG tool.  Perform baseline check on BW throughput to see
    if the CXL device is meeting expected BW (within 10% tolerance).

    This is a base line functional test to see that a CXL end point to CPU CXL tiles is functioning correctly and
    that the cache Opcode 4  BW Test (WrInv/WrInv)   transfer BW is within a reasonable range of the expected
    performance levels.  This test will run OpCode selected.
    Expected numbers for cache Opcode 4  BW Test (WrInv/WrInv) is  85% of 39.5 GB/s with 10% tolerance level
    ...
    Note ; Above scores are only SKU's recommended by performance team (QDF : Q03K)
    """
    CMD = "./ctg -v -p {} --sc 0 --vm {} --s1 4 --sc 1 --vm {} --s1 4 -t {} > cache_ctg_log.txt"
    EXPECTED_BW = 39.5
    ...
    def __init__(self, test_log, arguments, cfg_opts, bios_config_file=None):
        ...
        """
        super(CxlCtgBaselineCacheOp4, self).__init__(test_log, arguments, cfg_opts, bios_config_file)

    def prepare(self):
        """
        To check whether the host is alive or not.
        """
        super(CxlCtgBaselineCacheOp4, self).prepare(self.sdp)
    ...
    def execute(self):
        ...
        dimm_1_txt = self.create_and_copy_dimm_vm_on_sut(dimm_name=dimm_name_1, ctg_tool_path=self.ctg_tool_sut_path,
                                                         csp=self.csp, no_of_files=1)
        dimm_2_txt = self.create_and_copy_dimm_vm_on_sut(dimm_name=dimm_name_2, ctg_tool_path=self.ctg_tool_sut_path,
                                                         csp=self.csp, no_of_files=1)

        execution_time_in_sec = self._common_content_configuration.get_cxl_stress_execution_runtime(tool="ctg")

        traffic_cmd = self.CMD.format(int(bus, 16), dimm_1_txt, dimm_2_txt, execution_time_in_sec)
        ...
        self.verify_bw_number_in_ctg_output(file_name="cache_ctg_log.txt", tolerance_percent=10,
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if CxlCtgBaselineCacheOp4.main() else Framework.TEST_RESULT_FAIL)
