...
class CxlCtgBaselineMemOp12(CxlCtgCommon):
    """
    hsdes_id :  22014181151
    ...
    and that the memory OpCode transfer BW is within a reasonable range of the expected performance levels.
    This test will run OpCode selected.
    Expected numbers for mem opcode 10 (MemRd + MemWr ) is  85% of 39.5 GB/s with 10% tolerance level
    ...
    def __init__(self, test_log, arguments, cfg_opts, bios_config_file=None):
        ...
        """
        super(CxlCtgBaselineMemOp12, self).__init__(test_log, arguments, cfg_opts, bios_config_file)

    def prepare(self):
        """
        To check whether the host is alive or not.
        """
        super(CxlCtgBaselineMemOp12, self).prepare(self.sdp)
    ...
    def execute(self):
        ...
        self.verify_bw_number_in_ctg_output(file_name="socket_0_core_to_hdm0_memory.txt", tolerance_percent=10,
                                            expected_bw=79.1 * 0.85)

        return True


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if CxlCtgBaselineMemOp12.main() else Framework.TEST_RESULT_FAIL)
