...
class CxlMlcMemBwScanCheckType3Linux(CxlCommon):
    """
    hsdes_id :  22014456697 CXL.mem - MLC Memory Bandwidth Scan for Type 3 card – Linux OS (rev 1.0)
    ...
    def __init__(self, test_log, arguments, cfg_opts, bios_config_file=CXL_BIOS_KNOBS):
        ...
        """
        super(CxlMlcMemBwScanCheckType3Linux, self).__init__(test_log, arguments, cfg_opts, bios_config_file)
    ...
    def prepare(self):
        ...
        """
        super(CxlMlcMemBwScanCheckType3Linux, self).prepare()
    ...
    def execute(self):
        ...
        Memory Bandwidth Scan work load onto cxl Type 3 test card.   Is a Linux OS based stress test.
        Test case uses a Type 3 device.  Other commercially available Type 3 CXL cards may be used.
        ...
        for socket, port, dax_device in zip(self.dax_socket_list, self.dax_port_list, self.dax_device):
            ...
            self.os.execute_async(self.cxl_mlc_mem_bw_scan_run_cmd.format(int(node_num)), self.mlc_tool_path)
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if CxlMlcMemBwScanCheckType3Linux.main() else
