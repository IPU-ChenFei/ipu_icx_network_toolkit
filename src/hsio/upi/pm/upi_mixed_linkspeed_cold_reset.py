...
class UpiMixedLinkSpeedColdReset(HsioUpiCommon):
    """
    hsdes id: 1509117665 upi_non_ras_mixed_link_speed_cold_reset_cycling
    This test modifies BIOS to ensure mixed links speeds are set and verifies through Cscripts during each cold reset
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new UpiMixedLinkSpeedColdReset object

        :param test_log: Used for debug and info messages
        :param arguments:
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(UpiMixedLinkSpeedColdReset, self).__init__(test_log, arguments, cfg_opts)
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(UpiMixedLinkSpeedColdReset, self).prepare()

    def execute(self):

        return self.run_upi_cycle_tests(self._upi_checks.MIXED_LINK_SPEED, self._upi_checks.COLD_RESET)


if __name__ == "__main__":

    sys.exit(Framework.TEST_RESULT_PASS if UpiMixedLinkSpeedColdReset.main()
