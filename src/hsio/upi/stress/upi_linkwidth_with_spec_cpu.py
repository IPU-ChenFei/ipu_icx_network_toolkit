...
class UpiLinkwidthdWithSpecCpu(HsioUpiCommon):
    """
    hsdes_id :  22013653789 upi_linkwidth with spec CPU

    This test verifies link width is stable and all lanes are active under load
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new UpiLinkwidthdWithSpecCpu object

        :param test_log: Used for debug and info messages
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(UpiLinkwidthdWithSpecCpu, self).__init__(test_log, arguments, cfg_opts, self._BIOS_CONFIG_FILE)
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(UpiLinkwidthdWithSpecCpu, self).prepare()

    def execute(self):
        """
        This method is used to execute verify_upi_linkwidth_with_spec_cpu.

        :return: True or False based on the Output of upi_with_spec_cpu
        """

        return self.verify_upi_with_spec_cpu(test_type=self._upi_checks.LANE)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if UpiLinkwidthdWithSpecCpu.main() else Framework.TEST_RESULT_FAIL)
