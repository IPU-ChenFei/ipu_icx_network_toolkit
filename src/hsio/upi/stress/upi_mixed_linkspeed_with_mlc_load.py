...
class UpiMixedLinkspeedWithMLCLoad(HsioUpiCommon):
    """
    hsdes_id :  22012968661 upi_mixed_link_speed with MLC load
    HSD ID: 18016910109 PI_Verify_UPI_Mixed_Link_Speed_at_OS_with_concurrent_stress_MLC
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new UpiMixedLinkspeedWithMLCLoad object

        :param test_log: Used for debug and info messages
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(UpiMixedLinkspeedWithMLCLoad, self).__init__(
    ...
    def prepare(self):
        ...
        """
        try:
            super(UpiMixedLinkspeedWithMLCLoad, self).prepare()
        except:
    ...
    def execute(self):
        """
        This method is used to execute verify_upi_mixed_linkspeed_with_mlc to verify that bandwidth is >=85% after MLC

        :return: True or False based on the Output of upi_with_mlc
        """

        return self.verify_upi_with_mlc(test_type=self._upi_checks.MIXED_LINK_SPEED)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if UpiMixedLinkspeedWithMLCLoad.main() else Framework.TEST_RESULT_FAIL)
