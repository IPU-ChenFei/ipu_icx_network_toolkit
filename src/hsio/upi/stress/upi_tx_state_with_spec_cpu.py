...
class UpiTxStateWithSpecCpu(HsioUpiCommon):
    """
    hsdes_id :  22013653888 upi_tx_state with MLC load

    This test verifies upi trx port state in L0 and ready under load
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new UpiTxStateWithSpecCpu object

        :param test_log: Used for debug and info messages
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(UpiTxStateWithSpecCpu, self).__init__(test_log, arguments, cfg_opts, self._BIOS_CONFIG_FILE)
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(UpiTxStateWithSpecCpu, self).prepare()

    def execute(self):
        """
        This method is used to execute verify_upi_tx_state_with_Spec_Cpu

        :return: True or False based on the Output of upi_with_spec_cpu
        """

        return self.verify_upi_with_spec_cpu(test_type=self._upi_checks.TX_STATE)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if UpiTxStateWithSpecCpu.main() else Framework.TEST_RESULT_FAIL)
