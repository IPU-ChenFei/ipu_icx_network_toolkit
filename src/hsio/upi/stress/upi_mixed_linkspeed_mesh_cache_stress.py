...
class UpiMixedLinkSpeedMeshCacheStress(HsioUpiCommon):
    """
    hsdes id: 1509631675 upi_non_ras_mixed_linkspeed_cache_stress_ss_mlc
    This test modifies BIOS to ensure mixed UPI link speed is set and
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Creates a new UpiMixedLinkSpeedMeshCacheStress object

        :param test_log: Used for debug and info messages
        :param arguments:
        :param cfg_opts: xml.etree.ElementTree.Element of configuration options for execution environment.
        """
        super(UpiMixedLinkSpeedMeshCacheStress, self).__init__(test_log, arguments, cfg_opts)
    ...
    def prepare(self):
        ...
        :return: None
        """
        super(UpiMixedLinkSpeedMeshCacheStress, self).prepare()

    def execute(self):

        return self.run_upi_mesh_stress_test(self._upi_checks.MIXED_LINK_SPEED, self.MESH_TEST_CACHE_STRESS)


if __name__ == "__main__":

    sys.exit(Framework.TEST_RESULT_PASS if UpiMixedLinkSpeedMeshCacheStress.main()
