...
class VirtualizationCommon(ContentBaseTestCase):
    ...
    def create_vm_generic(self, vm_name, vm_type, vm_parallel=None, vm_create_async=None, mac_addr=None, pool_id=None,
        ...
        if vm_parallel is not None:
            ...
            self.__create_vm_generic(vm_name=vm_name, vm_type=vm_type, vm_parallel=vm_parallel, vm_create_async=vm_create_async,
                                mac_addr=mac_addr, pool_id=pool_id, pool_vol_id=pool_vol_id, cpu_core_list = cpu_core_list)
    ...
    def create_vmware_vm(self, vm_name, vm_type, mac_addr=None, vm_parallel=None, use_powercli=None):
        """
        This method is to create VM according to the given specification
        ...
        esxi_guestos_id = self._common_content_configuration.get_vm_esxi_guestos_identifier(self._sut_os, vm_type)
        ...
        if vm_parallel is not None and mac_addr is not None:
            self._vm_provider.create_vm(vm_name, os_variant=esxi_guestos_id, no_of_cpu=no_of_cpu, disk_size=disk_size,
                                        memory_size=memory_size, vm_creation_timeout=2700,
                                        vm_create_async=vm_parallel, mac_addr=mac_addr, os_version=os_version,
    ...
    def install_hqm_dpdk_library(self, common_content_lib = None):
        """
        ...
        self._install_collateral.install_hqm_dpdk_driver(common_content_lib = common_content_lib)

    def install_hqm_driver_library(self, common_content_lib = None):
        """
        This function installing the hqm driver library in sut
        """
        #install patch
        ...
        self._install_collateral.install_hqm_driver(common_content_lib)
    ...
    def yum_virt_install(self, package_group, common_content_lib=None, flags=None, cmd_path=None, group=False):
        ...
        cmd_result = common_content_lib.execute_sut_cmd("rpm --import /etc/pki/rpm-gpg/RPM-GPG-KEY*",
        ...
        for no_attempts in range(max_attempts):
            try:
                ...
                if not group:
                    install_result = common_content_lib.execute_sut_cmd("yum -y install {} --allowerasing".format(package_name),
    ...
    def windows_vm_object(self, vm_name, vm_type):
        ...
        try:
            ...
            self.create_ssh_vm_object(vm_name, copy_open_ssh=False)
            ipconfig_output = vm_os_obj.execute("ipconfig", self._command_timeout)
            self._log.info("IPCONFIG- {}".format(ipconfig_output.stdout))
        return vm_os_obj
    ...
    def enable_proxy_vm(self, common_content_lib=None):
        ...
        if http_proxy_reference_vm == "iind" or http_proxy_reference_vm == "chain":
            proxy_commands_sh_vm = "echo export http_proxy=http://proxy-{}.intel.com:911 >> /etc/profile.d/dtaf_proxy.sh;" \
                      ...
                      "echo export HTTPS_PROXY=http://proxy-{}.intel.com:911 >> /etc/profile.d/dtaf_proxy.sh;".format(
            ...
            proxy_commands_env_vm = "echo export http_proxy=http://proxy-{}.intel.com:911 >> /etc/environment;" \
                 ...
                 "echo export HTTPS_PROXY=http://proxy-{}.intel.com:911 >> /etc/environment;".format(
                  http_proxy_reference_vm, http_proxy_reference_vm, http_proxy_reference_vm, http_proxy_reference_vm)
        else:
 ...
 #           cmd_result = self._common_content_lib.execute_sut_cmd(self.REBOOT_VM_COMMAND
  ...
  #                     .format(vm_name, cmd_result))
    ...
    def create_virtual_function_dlb(self, no_of_vf):
        """
        ...
        self._common_content_lib.execute_sut_cmd("echo {} > /sys/class/dlb2/dlb0/device/sriov_numvfs"
                                                 .format(no_of_vf),
        ...
        vf_no = self._common_content_lib.execute_sut_cmd("cat /sys/class/dlb2/dlb0/device/sriov_numvfs",
    ...
    def get_split_hex_bdf_values_from_dbdf(self, dbsf_value):
        ...
        # slot_value = int(dbsf_value.split(":")[2].split(".")[0], 16)
        # function_value = int(dbsf_value.split(":")[2].split(".")[1], 16)
        domain_value = dbsf_value.split(":")[0]
        bus_value = dbsf_value.split(":")[1]
        slot_value = dbsf_value.split(":")[2].split(".")[0]
        function_value = dbsf_value.split(":")[2].split(".")[1]
        return domain_value, bus_value, slot_value, function_value
    ...
    def _enable_yum_repo_in_cent_vm(self, vm_os_obj):
        ...
        vm_install_collateral = InstallCollateral(self._log, vm_os_obj, self._cfg)
        # Move repo.zip file to SUT and extract with folder RHEL_REPOS
        sut_repo_folder_path = vm_install_collateral.download_and_copy_zip_to_sut(new, repo_file_path)
    ...
    def attach_pci_nw_device_to_vm(self, pcie_device, vm_name):
         ...
         attach_result = self._common_content_lib.execute_sut_cmd("virsh attach-device {} /root/{}".format(vm_name,
    ...
    def execute_iperf_client_esxi_vm(self,exec_time, port, iperf_server_ip, common_content = None):
        """
        This Method is Used to Set as Iperf Client and verify whether there is any data loss at server side..
        ...
        self.vm_ip = common_content.execute_sut_cmd(cmd,"Get the IP of VM",self._command_timeout)
        exec_time_min = 100
        if exec_time_min < exec_time:
            exec_time_min = exec_time
        cmd_output = common_content.execute_sut_cmd(self.cmd_for_iperf_client_with_port_esxi.format(iperf_server_ip, port, exec_time_min),
                                                    self.cmd_for_iperf_client_with_port_esxi.format(iperf_server_ip, port, exec_time_min),exec_time_min)
        if re.search(self.REGEX_FOR_DATA_LOSS, cmd_output):
            raise content_exceptions.TestFail("Data Loss is Observed at Client Side")
        self._log.info("There is No Data Loss at Client Side")
        self._log.debug("Sut is Successfully Set as Iperf Client and Iperf Response from Server is : {}".format(cmd_output))
    ...
    def attach_pci_device_using_dbdf_to_vm(self, vm_name, domain_value, bus_value, device_value, function_value):
        ...
        self._log.info("Copying the {} file to SUT".format(self.PCI_DEVICE_XML_FILE_NAME))
        self.os.copy_local_file_to_sut(self.PCI_DEVICE_XML_FILE_NAME, self.ROOT_PATH)
        detach_result = self._common_content_lib.execute_sut_cmd("virsh attach-device {} /root/{}".format(vm_name,
                                                                                                          self.PCI_DEVICE_XML_FILE_NAME),
    ...
    def host_vfio_accel_driver_unbind(self, dbsf_value, common_content_object=None):
        ...
        slot_value = dbsf_value.split(":")[2].split(".")[0]
        function_value = dbsf_value.split(":")[2].split(".")[1]
        command = "echo {} > /sys/bus/pci/devices/{}\:{}\:{}.{}/driver/unbind".format(dbsf_value, domain_value,
