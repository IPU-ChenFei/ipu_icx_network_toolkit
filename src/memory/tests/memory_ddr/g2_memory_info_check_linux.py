...
class G2MemoryInfoCheck(DDRCommon):
    """
    HP QC ID: 80044
    ...
    step_data_dict = {1: {'step_details': 'Clear OS logs and load default bios knobs',
                      ...
                      2: {'step_details': 'Check memory information.',
                          'expected_results': 'Make sure the memory total approximates the total amount of RAM  '
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        super(G2MemoryInfoCheck, self).__init__(test_log, arguments, cfg_opts)
    ...
    def execute(self):
        ...
        memtotal_with_variance = (self._post_mem_capacity_config - (self._post_mem_capacity_config *
                                                                    self._variance_percent))

        self._log.info("Total DDR capacity as per configuration with variance - {}".format(
            memtotal_with_variance))

        if memory_in_gib < int(memtotal_with_variance) or memory_in_gib > self._post_mem_capacity_config:
    ...
    def cleanup(self, return_status):
        # type: (bool) -> None
        """DTAF cleanup"""
        super(G2MemoryInfoCheck, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if G2MemoryInfoCheck.main()
