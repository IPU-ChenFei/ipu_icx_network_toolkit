...
class CRProvisioning2LmMixedMode50InterleavedUefi(CrProvisioningTestCommon):
    """
    Glasgow ID : 57628.11
    ...
    2. All installed DCPMM(s) are configured with 50% of capacity as volatile memory and the remainder as regions
    ...
    BIOS_CONFIG_FILE = "cr_provisioning_2lm_mixed_mode_50_50_interleaved_uefi.cfg"
    TC_ID = "G57628"
    ...
    step_data_dict = {1: {'step_details': 'Copy ipmctl efi file from host to usb and Set & Verify BIOS knobs'
                          ...
                          'expected_results': 'Ipmct.efi copied from host to usb and BIOS setup options are updated'
                      ...
                      2: {'step_details': 'Boot to UEFI shell, Launch ipmctl tool to check modes supported and dimm '
                          ...
                          'expected_results': 'Ipmctl launches successfully, Memory & App Direct Modes '
                                          'are both supported, All installed DCPMMs are listed with IDs and capacities '
                                          'and are healthy. The correct DCPMM FW Version is displayed.'},
                      3: {'step_details': 'Delete pcd data.',
                          'expected_results': 'Successfully deleted pcd data.'},
                      4: {'step_details': 'Check whether the DIMMs have pre-existing namespaces.',
                          'expected_results': 'Commands complete without errors, Pre-existing namespaces are '
                                              'identified and removed from DIMMs.'},
                      5: {'step_details': 'Create goal of 50% Memory mode and rest as appdirect mode, display goal '
                                          'information and restart the SUT ',
                      ...
                      6: {'step_details': 'After SUT restated the goal was applied, Confirm namespaces were '
                      ...
                      7: {'step_details': 'Show all the attributes for the newly created namespace,pmem block device'
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create a new CRProvisioning2LM 50% memory mode and appdirect .
        ...
        :raises: None
        """
        super(CRProvisioning2LmMixedMode50InterleavedUefi, self).__init__(test_log, arguments, cfg_opts,
    ...
    def execute(self):
        ...
        2. All installed DCPMM(s) 50% of capacity as volatile memory and the remainder as regions
         of persistent memory.
        ...
        dcpmm_disk_goal = self._uefi_util_obj.execute_cmd_in_uefi_and_read_response(
            "ipmctl.efi create -f -goal memorymode=50 persistentmemorytype=appdirect")

        dcpmm_disk_goal = ' '.join([str(elem) for elem in dcpmm_disk_goal])

        if not dcpmm_disk_goal:
            raise content_exceptions.TestFail("Failed to provision crystal ridge DIMMS with 50% Memory Mode and 50% "
                                              "Appdirect more ...")
        ...
        self._log.info("Successfully provisioned crystal ridge DIMMS as 50% Memory Mode... Rebooting to apply the "
                       "changes ...")
        ...
        #  To confirm that this goal is with an allocation of 50% volatile memory and remaining capacity is allocated
        #  a "AppDirect" interleaved persistent memory
        ...
        self._memory_common_lib.verify_region_default_attribs_mode(region_data_with_a_param, mode="AppDirect")
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if CRProvisioning2LmMixedMode50InterleavedUefi.main()
