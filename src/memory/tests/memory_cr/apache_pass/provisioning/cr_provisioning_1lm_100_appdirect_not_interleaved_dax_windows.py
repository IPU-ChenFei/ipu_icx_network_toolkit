...
class CRProvisioning1LM100AppDirectNotInterleavedDaxWindows(CrProvisioningTestCommon):
    """
    Glasgow ID: 56954
    Verification of the platform supported DCPMM capabilities in a Microsoft Windows OS environment.

    1. Setup and verify DCPMMs in 1LM 100% AppDirect, Not Interleaved mode using Linux provisioning tools.
    2. Create an explicit 100% App Direct goal with persistent memory AppDirect not interleaved capacity in 1LM mode.
    3. All DCPMMs will be provisioned as 100% persistent memory. Each individual DCPMM persistent address range will
    be its own interleave set.

    """

    BIOS_CONFIG_FILE = "cr_provisioning_1lm_100_appdirect_not_interleaveios_dax_bios_knob.cfg"
    TEST_CASE_ID = "G56954"
    _ipmctl_executer_path = None
    IPMCTL_EXE_INSTALLER = "ipmctl.zip"

    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create a new CRProvisioning2LM50AppDirect50MemoryMode object.
        ...
        super(CRProvisioning1LM100AppDirectNotInterleavedDaxWindows, self).__init__(test_log,
                                                                                    arguments, cfg_opts,
    ...
    def execute(self):
        ...
        2. Create a 2LM mixed mode goal with persistent memory, 100% AppDirect Mode.
        ...
        #  Configure the capacity on all installed DCPMM(s) with 100% App Direct mode
        dcpmm_disk_goal = self.dcpmm_configuration(
            ...
            cmd_str="with 100% App Direct Mode")
        ...
        return_value.append(self.create_partition_dcpmm_disk(convert_type="gpt", disk_lists=disk_list, mode='dax'))
...
if __name__ == "__main__":
    sys.exit(
        Framework.TEST_RESULT_PASS if CRProvisioning1LM100AppDirectNotInterleavedDaxWindows.main() else
