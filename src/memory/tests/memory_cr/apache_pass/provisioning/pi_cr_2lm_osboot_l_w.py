...
class PICR2lmOsBoot(CrProvisioningTestCommon):
    """
    HP QC  ID: 79523 (linux) and H82158 (Windows)
    2LM Basic functionality check,verify if system is able to boot into OS with CR DIMMs installed.
    """

    BIOS_CONFIG_FILE = "pi_cr_2lm_osboot_bios_knob.cfg"
    TEST_CASE_ID = "H79523/H82158-PI_CR_2LM_OSBoot"
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        super(PICR2lmOsBoot, self).__init__(test_log, arguments, cfg_opts, self.BIOS_CONFIG_FILE)
    ...
    def prepare(self):
        ...
        compare_config_slot_population = self._memory_provider.get_slot_configuration_list()

        # Verification between platform configuration and golden configuration info of populated dimms.
        dict_config_match = [self._memory_provider.verify_slot_population(compare_config_slot_population[config])
                             for config in range(len(compare_config_slot_population))]
        ...
        if not any(dict_config_match):
            raise content_exceptions.TestFail("Configuration not set correctly on this platform to support this test "
                                              "case.. please check the dimm population and try again..")
    ...
    def execute(self):
        ...
        sum_dcpmm_dimm_capacity_os = sum(self._memory_common_lib.get_total_dimm_memory_data(dimm_show))

        self._log.info("Total dimm capacity shown from OS level - {}".format(int(sum_dcpmm_dimm_capacity_os)))
        ...
        if int(sum_dcpmm_dimm_capacity_os) < dcpmm_memtotal_with_variance_config or int(sum_dcpmm_dimm_capacity_os) > \
                self._dcpmm_mem_capacity:
            raise content_exceptions.TestFail("Total dcpmm dimm Capacity is not same as installed Capacity.")

        self._log.info("Total dcpmm dimm Capacity is same as the installed Capacity.")
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if PICR2lmOsBoot.main()
