...
class PICR1lmAppDirectPMS0S5S0(CrProvisioningTestCommon):
    """
    HP QC  ID: 79529 (Linux), 82164 (Windows)

    To check system power sequence with 1LM CR: so->s5->s0
    """

    TEST_CASE_ID = "H79529/H82164-PiCR1lmAppDirectPMS0S5S0"
    ...
    step_data_dict = {1: {'step_details': 'To provision the crystal ridge with 1LM 100% AppDirect mode',
                      ...
                      2: {'step_details': 'Copying the 5GB TEST file from local HDD to the CR BLK disk',
                          'expected_results': 'calculate the checksum by using md5sum command'},
                      3: {'step_details': 'shutdown and power ON the SUT',
                          'expected_results': 'The SUT has to boot to OS'},
                      4: {'step_details': 'Calculate the checksum 5GB TEST file after shutdown by using md5sum command',
                          'expected_results': 'Calculated checksum before shutdown and after shutdown are same'}
                      }
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        super(PICR1lmAppDirectPMS0S5S0, self).__init__(test_log, arguments, cfg_opts, bios_config_file=None)
        ...
        if OperatingSystems.LINUX == self._os.os_type:
            self.pmem_list = self._cr_provisioning_100appdirect.mount_list
    ...
    def execute(self):
        ...
        2. Calculate the checksum value using md5sum before shutdown.
        3. Shutdown the SUT and power ON.
        4. Calculate the checksum value using md5sum after shutdown.
        ...
        for each_data in self.pmem_list:
            if OperatingSystems.LINUX == self._os.os_type:
                ...
                test_file_path = os.path.join(each_data, MemoryCommonLib.TEST_FILE_NAME)
            checksum_list_values_before_s5.append(
                self._memory_common_lib.calculate_checksum(test_file_path))

        self._log.info("checksum values before shutdown are : {}".format(checksum_list_values_before_s5))

        #  Step logger end for Step 2
        self._test_content_logger.end_step_logger(2, return_val=checksum_list_values_before_s5)
        ...
        # To get the checksum values after the shutdown
        for each_data in self.pmem_list:
            if OperatingSystems.LINUX == self._os.os_type:
                test_file_path = Path(os.path.join(each_data, MemoryCommonLib.TEST_FILE_NAME)).as_posix()
                ...
                test_file_path = os.path.join(each_data, MemoryCommonLib.TEST_FILE_NAME)
            checksum_list_values_after_s5.append(
                self._memory_common_lib.calculate_checksum(test_file_path))

        self._log.info("checksum values are after S5 : {}".format(checksum_list_values_before_s5))

        # compare checksum values before shutdown and after shutdown
        if checksum_list_values_before_s5 != checksum_list_values_after_s5:
            self._log.info("check sum values before shutdown and after shutdown are not same")
            return_value = False

        self._log.info("check sum values before shutdown and after shutdown are same")
