...
from src.memory.tests.memory_cr.crow_pass.pi_cr_2lm_memory_mode_basic_l_w import PiCR2LMMemoryModeBasic


class PICR2LMMemoryResourceCheck(CrProvisioningTestCommon):
    """
    HP QC ID: H101110-PI_CR_2LM_MemoryResourceCheck_L and H101155-PI_CR_2LM_MemoryResourceCheck_W

    To Verify Memory information for CR DIMMs is correct in linux
    """

    TEST_CASE_ID = ["H101110", "PI_CR_2LM_MemoryResourceCheck_L", "H101155", "PI_CR_2LM_MemoryResourceCheck_W"]
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create a new PICR2LMMemoryResourceCheck object.
        ...
        self._cr_2lm_memory_mode_basic = PiCR2LMMemoryModeBasic(test_log, arguments, cfg_opts)
        self._cr_2lm_memory_mode_basic.prepare()
        self._cr_2lm_memory_mode_basic_result = self._cr_2lm_memory_mode_basic.execute()

        # calling base class init
        super(PICR2LMMemoryResourceCheck, self).__init__(test_log, arguments, cfg_opts, bios_config_file=None)

        if self._cr_2lm_memory_mode_basic_result:
            self._log.info("Memory mode basic check has been done successfully!")
        else:
            raise content_exceptions.TestFail("Memory mode basic check has failed!")

        # Step logger end for Step 1
        self._test_content_logger.end_step_logger(1, return_val=self._cr_2lm_memory_mode_basic_result)
    ...
    def execute(self):
        ...
        self._memory_common_lib.verify_memory_resource_information(mem_resources_data, MemoryTopology.TWO_LM,
                                                                   percent=100)
        ...
        self._memory_common_lib.verify_socket_information(socket_op, cpu_locators, mode=MemoryTopology.TWO_LM)
    ...
    def cleanup(self, return_status):
        ...
        super(PICR2LMMemoryResourceCheck, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if PICR2LMMemoryResourceCheck.main() else Framework.TEST_RESULT_FAIL)
