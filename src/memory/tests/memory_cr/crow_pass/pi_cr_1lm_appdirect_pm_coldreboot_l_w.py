...
class PiCR1LMAppDirectPMColdReboot(ContentBaseTestCase):
    """
    HP QC  ID: 101124-PI_CR_1LM_AppDirect_PM_ColdReboot_L and H101169-PI_CR_1LM_AppDirect_PM_ColdReboot_W

    To check system power sequence with 1LM CR: Cold reboot
    """
    TEST_CASE_ID = ["H101124", "PI_CR_1LM_AppDirect_PM_ColdReboot_L",
                    "H101169", "PI_CR_1LM_AppDirect_PM_ColdReboot_W"]
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create a new PiCR1LMAppDirectPMColdReboot8by8 object.
        ...
        self._cold_reboot = ResetBaseTest(self._log, arguments, cfg_opts=cfg_opts)
    ...
    def execute(self):
        ...
        3. Install IOPort tool in SUT, cold Reboot the SUT.
        ...
        for cycle in range(self.num_of_cycle):
            ...
            # To get the checksum values after the cold reboot
            for each_data in self.pmem_list:
                if OperatingSystems.LINUX in self.os.os_type:
                    test_file_path = Path(os.path.join(each_data, self._memory_common_lib.TEST_FILE_NAME)).as_posix()
                elif OperatingSystems.WINDOWS in self.os.os_type:
