...
class PiCR2LMAppDirectFilesReadWrite(ContentBaseTestCase):
    """
    HP QC  ID: H101112-PI_CR_2LM_AppDirect_FilesReadWrite_L and H101157-PI_CR_2LM_AppDirect_FilesReadWrite_W

    Basic read/write functionality is working normally on CR DIMMs.
    """

    TEST_CASE_ID = ["H101112", "PI_CR_2LM_AppDirect_FilesReadWrite_L",
                    "H101157", "PI_CR_2LM_AppDirect_FilesReadWrite_W"]
    return_value = []
    NUMBER_OF_TIMES_COPY = 5
    ROOT = "/root"
    C_DRIVE_PATH = "C:\\"
    step_data_dict = {1: {'step_details': 'To provision the crystal ridge with 2LM 50% AppDirect, 50% Memory mode',
                          'expected_results': 'Successfully provisioned crystal ridge with 2LM 50% AppDirect, '
                                              '50% Memory mode'},
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        super(PiCR2LMAppDirectFilesReadWrite, self).__init__(test_log, arguments, cfg_opts)
        ...
        provisioning_result = self._cr_provisioning_mixed_mode.execute()

        if provisioning_result:
            self._log.info("Provisioning of DCPMM with 2LM mixed mode has been done successfully ...")
        else:
        ...
        for count in range(0, self.NUMBER_OF_TIMES_COPY):
            ...
            if not self.os.is_alive():
                raise RuntimeError("SUT is not responding while copying 5GB test file from hard disk "
                                   "to pmem drive, count is {}".format(count + 1))
