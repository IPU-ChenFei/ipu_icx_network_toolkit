...
class PiCR2LMMemoryAppDirectStorageResourceCheck(CrProvisioningTestCommon):
    """
    HP QC ID: H101114:PI_CR_2LM_Memory_AppDirect_StorageResourceCheck_L and
    H101159: PI_CR_2LM_Memory_AppDirect_StorageResourceCheck_W
    ...
    TEST_CASE_ID = ["H101114", "PI_CR_2LM_Memory_AppDirect_StorageResourceCheck_L",
                    "H101159", "PI_CR_2LM_Memory_AppDirect_StorageResourceCheck_W"]
    return_value = []
    ...
    step_data_dict = {1: {'step_details': 'Clear OS logs and Set the bios knobs, Restart the system, Boot to OS '
                      ...
                      2: {'step_details': 'Create Memory Allocation Goal on dimm,  reboot the system and goal '
                                          'verification',
                          'expected_results': 'system successfully boot to OS and goal is successfully verified'},
                      3: {'step_details': 'Check if CPS DIMM memory resource allocation is correct.',
                          'expected_results': 'CPS 50% is AppDirectCapacity and 50% is Memory Mode'},
                      4: {'step_details': 'Verify region data.',
                          'expected_results': 'Region exists with assigned ID, FreeCapacity = capacity'},
                      5: {'step_details': 'Create namespace on AppDirect',
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        """
        Create a new PI_CR_2LM_Memory_AppDirect_StorageResourceCheck_L object.
        ...
        super(PiCR2LMMemoryAppDirectStorageResourceCheck, self).__init__(test_log, arguments, cfg_opts,
                                                                         mode=MemoryTopology.TWO_LM)
    ...
    def execute(self):
        ...
        1. Create goal Appdirect = 50% and Memory mode = 50%
        ...
        # Configure the capacity on all installed DCPMM(s) with 50% as persistent memory and 50% Memory Mode
        dcpmm_disk_goal = self._ipmctl_provider.dcpmm_configuration(
            cmd=self.IPMCTL_CMD_FOR_50_AD_50_MEMORY_MODE, cmd_str="with 50% persistent and 50% volatile memory")

        # Verify the memory mode mode 50% provisioning
        ...
        # Verify the App Direct mode mode 50% provisioning
        self.return_value.append(
        ...
        self._ipmctl_provider.verify_pmem_device_presence_cap(namespace_info, appdirect_percent=50)
    ...
    def cleanup(self, return_status):
        ...
        super(PiCR2LMMemoryAppDirectStorageResourceCheck, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if PiCR2LMMemoryAppDirectStorageResourceCheck.main() else
