...
class PiCR1LMOsBoot(CpsTestCommon):
    """
    HP QC ID: H101106-PI_CR_1LM_OSBoot_L and H101151-PI_CR_1LM_OSBoot_W

    1LM Basic functionality check,verify if system is able to boot into OS with CR DIMMs installed.
    """
    TEST_CASE_ID = ["H101106", "PI_CR_1LM_OSBoot_L", "H101151", "PI_CR_1LM_OSBoot_W"]
    ...
    step_data_dict = {1: {'step_details': 'verify SUT has 8+8 (CPS + memory) in SUT.Clear OS logs and Set the bios '
                          ...
                          'expected_results': 'verified 8+8 config. Clear ALL the system Os logs and BIOS setting done '
    ...
    def __init__(self, test_log, arguments, cfg_opts):
        ...
        :return: None
        """
        super(PiCR1LMOsBoot, self).__init__(test_log, arguments, cfg_opts, mode=MemoryTopology.ONE_LM)
    ...
    def cleanup(self, return_status):
        # type: (bool) -> None
        """DTAF cleanup"""
        super(PiCR1LMOsBoot, self).cleanup(return_status)


if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if PiCR1LMOsBoot.main() else Framework.TEST_RESULT_FAIL)
