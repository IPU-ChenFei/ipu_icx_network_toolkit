...
def test_steps(tcd):
    ...
    # Tool Version [0.2.1]
    # ID: 15010585526
    # TITLE: PI_Memory_DDR5_Single_NUMA_Configuration _L
    # DOMAIN: memory
    ...
    if tcd.prepare("setup system for test"):
        ...
        ## Set BIOS knob: SncEn=0x0, NumaEn=0x1
        set_cli = not sut.xmlcli_os.check_bios_knobs("SncEn=0x0, NumaEn=0x1")
        if set_cli:
            sut.xmlcli_os.set_bios_knobs("SncEn=0x0, NumaEn=0x1")
            sutos.reset_cycle_step(sut)
            tcd.expect("double check bios knobs", sut.xmlcli_os.check_bios_knobs("SncEn=0x0, NumaEn=0x1"))
    ...
    try:
        ...
        if tcd.step("In OS, Check the numa node:"):
            ...
            dimm_info_check(sut, node_info_cmd, 'socket')
            ##################
            tcd.log("### Expected result ###")
            tcd.log("Make sure that the numbers of nodes are as expected")
            tcd.log("SNC2 should be:number of sockets")
            ### Notes ###
            # Step 4
            ##################
        ...
        if tcd.step("Run MLC for 12 hours: ./mlc --peak_injection_bandwidth -Z -t43200"):
            # Run MLC for 12 hours: ./mlc --peak_injection_bandwidth -Z -t43200
            # sutos.execute_cmd(sut, f'./{tools.mlc_l.ipath}/mlc --peak_injection_bandwidth -Z -t180', timeout=12 * 60)
