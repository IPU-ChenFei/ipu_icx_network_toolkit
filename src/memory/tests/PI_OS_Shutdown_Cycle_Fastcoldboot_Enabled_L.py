...
CASE_DESC = [
    'The objective of this test case is used to check OS shutdown cycle boot flow when fast boot is enabled.'
]


def test_steps(sut, my_os):
    Case.prepare('Boot to EDK11 Menu, change BIOS knobs then boot to OS.')
    boot_to_with_bios_knobs(sut, sut.default_os, *bios_knob('Attempt_Fast_Boot_enable_xmlcli'),
                            *bios_knob('Attempt_Fast_Boot_Cold_enable_xmlcli'))
    ...
    expect_str = 'subBootMode = ColdBootFast'
