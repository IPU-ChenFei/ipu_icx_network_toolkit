...
tool_menu.add_command(label='Utilities Folder')
...
step_output_frame = Frame(step_frame)
step_operation_frame = Frame(step_frame)

step_output_frame.grid(row=0, column=0, sticky='nwes')
step_operation_frame.grid(row=1, column=0, sticky='nwes', pady=2)

scroll_v = Scrollbar(step_output_frame)
scroll_v.pack(side='right', fill='y')

step_output = utils.TextMonitor(step_output_frame, width=60, height=20)
...
Label(step_operation_frame, text='TCD Name', anchor='w').grid(row=0, column=0, columnspan=2, padx=5, pady=2, sticky='we')
tcd_entry = Entry(step_operation_frame)
tcd_entry.grid(row=0, column=2, padx=5, pady=2, sticky='we')
tcd_entry.insert(0, 'Demo_TCD_Name')
stamp_button = Checkbutton(step_operation_frame, text='Time Stamp', variable=timestamp, onvalue=True, offvalue=False)
stamp_button.grid(row=0, column=3, padx=5, pady=2, sticky='we')

Button(step_operation_frame, text='H2L', width=23, command=partial(generated_step.h2l, tcd_entry, timestamp, step_output, dlog_output)).grid(row=1, column=0, padx=5, sticky='we')
Label(step_operation_frame, text='->', width=3).grid(row=1, column=1, padx=5, sticky='we')
Button(step_operation_frame, text='L2PY', width=23, command=partial(generated_step.l2py, dlog_output)).grid(row=1, column=2, padx=5, sticky='we')
Button(step_operation_frame, text='EXEC PYTHON', width=10, command=partial(generated_step.exec_python, dlog_output), background='green').grid(row=1, column=3, padx=5, sticky='we')
Button(step_operation_frame, text='STOP PYTHON', width=10, command=partial(generated_step.stop_python, dlog_output), background='red').grid(row=2, column=3, padx=5, pady=2, sticky='we')
Button(step_operation_frame, text='H2PY', command=partial(generated_step.h2py, tcd_entry, timestamp, step_output, dlog_output)).grid(row=2, column=0, padx=5, pady=2, columnspan=3, sticky='we')
