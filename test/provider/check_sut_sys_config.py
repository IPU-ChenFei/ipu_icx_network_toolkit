...
class CheckSutSysConfig(BaseTestCase):
    """
    Test Storage Provider APIs.
    """

    def __init__(self, test_log, arguments, cfg_opts):
        super(CheckSutSysConfig, self).__init__(test_log, arguments, cfg_opts)
    ...
    def execute(self):
        ...
        """
        self._log.info("Check Physical provider for SX State.")
        sx_state = self._common_content_lib.get_power_state(self._phy1)
        self._log.info("sx_state='{}'".format(sx_state))

        self._log.info("Check SUT SSH OS Provider.")
        try:
            result = self._os.execute("dir", 20)
            if result.cmd_passed():
                self._log.info("OS command succeeded. stdout={}".format(result.stdout))
            else:
                self._log.info("OS command failed. stderr={}".format(result.stderr))
        except Exception as ex:
            pass

        self._log.info("Check AC provider.")
        try:
            ac_state = self.ac.get_ac_power_state()
            self._log.info("AC State={}.".format(ac_state))
        except Exception as ex:
            pass

        platform_type = self._common_content_lib.get_platform_type()
        if platform_type not in [PlatformType.HPE, PlatformType.MSFT]:
            self._log.info("Check Physical provider for Post code.")
...
if __name__ == "__main__":
    sys.exit(Framework.TEST_RESULT_PASS if CheckSutSysConfig.main() else Framework.TEST_RESULT_FAIL)
